...
1. **Node version floor.** Node `>=20.6.0`, matching `@mariozechner/pi-coding-agent`'s `engines.node` floor at the pinned peer-dep version. The literal `>=20.6.0` is a *floor*, not a range — there is no upper bound. A Pi minor bump that moves that floor requires re-validating the loom range in the same edit, alongside the surface re-validation called out under **Pi SDK and capabilities** above. Violation is observable: the extension-factory capability probe (per [Pi Integration Contract — Extension entry point](./spec_topics/pi-integration-contract.md) step 0) compares `process.versions.node` against the floor and, on a sub-floor host, refuses every `pi.register*` call and emits `loom/load/host-incompatible` with `details.kind = "node-floor"`. The `package.json#engines.node` install-time check is package-manager-dependent and is **not** the enforcement point.
