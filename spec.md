...
*Session model.* A Pi extension instance is bound to exactly one active user session at a time. Session-swap semantics for in-flight invocations — the abort-and-await sequence, the per-invocation operator-visibility surface, partial-append fate, and the `invoke`-parent observation rule — are owned by [Pi Integration Contract — Session-swap behaviour for in-flight invocations](./spec_topics/pi-integration-contract.md#session-shutdown-semantics). Concurrent loom invocations within a session (parallel tool calls, sibling subagent sessions) are first-class and addressed by the `ActiveInvocationRegistry` and the per-invocation `loomAbort`; concurrent *user sessions* in the same host process are out of scope for V1 because Pi does not support them.
...
- **Source-language stability.** A `.loom` or `.warp` file that loads cleanly under V1.0 SHOULD load and behave identically under every V1.x release; the equivalence claim, the V1.0 review-vs-mechanical-gate posture, and the deferred-conformance-suite framing are owned by [Governance — GOV-13 / GOV-14](./spec_topics/governance.md#gov-13). The long-term migration framing (no major-version migration mechanism) is owned by [Future Considerations — Known V1 limitations (no seam expected)](./spec_topics/future-considerations.md). Migration across major versions is out of V1 scope.
...
- [Governance](./spec_topics/governance.md) — *spec-corpus governance, not implementation spec.* REQ-ID prefix table, retirement registry, and the GOV-1 through GOV-14 rules that govern REQ-ID coining, anchoring, retirement, and gating.
