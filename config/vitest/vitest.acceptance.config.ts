...
// acceptance suite (Phase 1 of the two-phase theta 1.0 release gate,
// real-host-smoke-gate.md).
//
// Invoked only by `npm run test:acceptance`; it SPAWNS the real `pi` binary in
// print mode (`pi -p --theta <dir> "/<name>"`, process-and-exit) against a live
...
// token-free red (the feature-theta fixture is absent) has already fired.
export default defineConfig({
  root: repoRoot,
  test: {
    include: ["tests/acceptance/**/*.test.ts"],
    environment: "node",
    // Each theta drives one or more live turns through a spawned `pi -p`
