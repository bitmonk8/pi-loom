...
# T26 - Compact-transcript byte-exact rendering — state the rationale
...
The *Compact-transcript format (normative)* sub-section (anchor
`compact-transcript-format-normative` in `docs/spec_topics/binder.md`)
pins role tags, the `: ` separator, the turn delimiter, the
`[tool-call …]` line form, the trailing-space contract on the
empty-content user message, and reference renderings A–D as
`MUST`-reproduce-exactly. The enclosing *System-prompt structure
(normative)* sub-section opens with "Wording may vary; the listed
tokens, line-prefixes, and conditional-presence rules are the contract",
yet the transcript rules pin the full byte sequence with no stated
reason for the stricter discipline. The *Determinism* section pins
`temperature: 0` and a per-loom FNV-1a seed but does not say whether
transcript byte wobble would defeat that determinism. An implementer or
later reviewer cannot tell whether reference renderings A–D are a hard
reproducibility contract or examples revisable for clarity.

## Solution approach

Under the `compact-transcript-format-normative` heading in
`docs/spec_topics/binder.md`, add a rationale paragraph explaining why
the transcript bytes are pinned even though the surrounding
system-prompt wording is non-normative, grounding the reason in the
*Determinism* section's (`#determinism`) `temperature: 0` plus FNV-1a
seed. Have the rationale address the asymmetry with *System-prompt
structure (normative)*'s "wording may vary" opener.

## Solution constraints

- Out of scope: weakening, demoting, or removing the existing byte-exact
  MUSTs or reference renderings A–D — they remain normative.

## Relationships

- T28 "Binder model — configured-string-to-`(provider, modelId)` parse rule" — same-cluster (same binder system-prompt construction surface; resolves independently).

---
# T27 - `Model<Api>` and `ModelRegistry` referenced by bare name, never pinned to their owning `.d.ts`
...
PIC pins every consumed SDK type to its declaring `.d.ts` path + symbol, but `Model<Api>` and `ModelRegistry` are referenced only by bare name at every site: the `ExtensionContext` member surface (`modelRegistry: ModelRegistry`, `model: Model<any> | undefined`), Host prerequisites item 3, SDK capability inventory item 7 (`id="sdk-cap-binder-llm-model"`), Pi version bump procedure step 7, `binder.md`'s strict-capability requirement, and the two `diagnostics.md` strict-capability rows. Two load-bearing claims rest on the unanchored types: the "`strictCapable` field absent at the loom 1.0 Pi-SDK pin" claim (diff-audited on each Pi minor bump) and the bump step-7 rename probe plus its build-time surface-inventory assertion. Neither can be diffed nor located without a declaring `.d.ts` path.

## Solution approach

Add a canonical `.d.ts` path + symbol pin for both `Model<Api>` and `ModelRegistry` at the `ExtensionContext` member surface block (`id="extensioncontext-interface"`), pinning each type to its own declaring file and carrying the standard re-validate-on-each-Pi-minor-bump obligation, and identify the declaring source of the `Api` type parameter. Clarify the relationship between the `Model<any>` instantiation on `ExtensionContext` and the `Model<Api>` form returned by `ModelRegistry.find`. Update the consumer sites — `binder.md`'s strict-capability requirement, the two `diagnostics.md` strict-capability rows, PIC Host prerequisites item 3, SDK capability inventory item 7, and Pi version bump procedure step 7 — to cite the pin by anchor rather than restating.
...
- T28 "Binder model — configured-string-to-`(provider, modelId)` parse rule" — co-resolve (the same PIC-pinning edit that lists `ModelRegistry.find` by its `dist/...` path should also pin `findExactModelReferenceMatch` and `getAvailable`, the symbols that finding depends on).
- T13 "Bare-name SDK types lack `.d.ts` pins at their PIC carrier sites" — same-cluster (identical anchoring gap, different types; same pin-then-forward-link recipe).
- T19 "`estimateTokens` signature names the wrong message type" — same-cluster (external-entity pinning family on the same page).
# T28 - Binder model — configured-string-to-`(provider, modelId)` parse rule
...
The strict-capability paragraph (anchor `strict-capability-requirement` in `binder.md`) resolves the binder model by calling `ctx.modelRegistry.find(provider, modelId)`, a two-argument call. Every configuration surface, however, carries a single free-form string — `bind_model:` in frontmatter, `looms.binderModel` in `settings.json`, and the resolution-chain prose all describe only "a model identifier", never a `(provider, modelId)` pair. No section specifies how that string is split into the two arguments, nor the behaviour when the string carries no provider component. Conforming implementers therefore diverge — one requires strict `provider/modelId`, another infers a provider for a bare id, a third calls `find("", id)` — so the frontmatter example `bind_model: claude-haiku` loads on some implementations and fails with `loom/load/binder-model-unresolved` on others.

## Solution approach

Rewrite the `strict-capability-requirement` paragraph in `binder.md` to define how the configured string parses into the `(provider, modelId)` pair that `ctx.modelRegistry.find` consumes, covering the no-provider-component (bare-id) case explicitly. Pi's `--model` flag already parses this shape via `findExactModelReferenceMatch` (`dist/core/model-resolver.d.ts`), which accepts both `provider/modelId` and a bare `modelId` and rejects cross-provider ambiguity; deferring to it avoids coining a loom-specific convention. Specify that an unmatched or ambiguous configured string produces `loom/load/binder-model-unresolved`. Add forward-links from the `bind_model:` row in `frontmatter.md` and the `looms.binderModel` entry in `discovery.md` to the binder-model parse rule.
...
- T27 "`Model<Api>` and `ModelRegistry` referenced by bare name" — co-resolve (the PIC-pinning edit should also pin `findExactModelReferenceMatch` and `getAvailable`, the symbols this finding depends on).
- T21 "`bind_context` and loom `model:` — out-of-set / unresolvable values are undefined" — must-precede (the loom `model:` resolution reuses this `(provider, modelId)` parse rule; resolve this first so T21 forward-links rather than re-states).
- T26 "Compact-transcript byte-exact rendering — state the rationale" — same-cluster (same binder system-prompt construction surface; resolves independently).
# T29 - Step 0 (d) peer-dep version probe relies on a subpath export the pinned packages do not all publish
...
Step 0 (d)'s "For each package in turn, read the installed version…" paragraph prescribes `createRequire(import.meta.url).resolve("<pkg>/package.json")` to read each lock-step `@earendil-works/*` package's installed version. Node 22's ESM resolver rejects subpaths not enumerated in a package's `exports` map with `ERR_PACKAGE_PATH_NOT_EXPORTED`; at the current `#pi-sdk-pin` range, two of the four pinned packages declare `exports` maps with no `./package.json` subpath and therefore throw. The *Self-failure* paragraph routes that throw to `kind: "peer-dep-malformed-version"`, so the probe emits `loom/load/host-incompatible` and skips every `pi.register*` call on a correctly-pinned, validly-installed host. The contract cannot load on the very SDK pin Step 0 is meant to authorise.

## Solution approach

Rewrite the resolution clause in Step 0 (d)'s "For each package in turn…" paragraph to read each package's `package.json` by an `exports`-independent mechanic: resolve the package main entry, then walk parent directories up to the first `package.json` whose `name` field equals the iteration's `<pkg>`. Extend the *Self-failure* paragraph's foreseeable-shape list so a parent walk that reaches the filesystem root without a name-matching `package.json` routes to `peer-dep-malformed-version` rather than `probe-failed`.

## Solution constraints

- The new parent-walk-reached-root failure case MUST route to the existing `peer-dep-malformed-version` kind; do not add a new member to the `kind` enumeration in *On failure: refusal and diagnostic* or to diagnostics.md's `loom/load/host-incompatible` `details.kind` list.
- Out of scope: the Step 0 (d) `#step-0-d-recommended-recipe` *Recommended recipe (non-normative)* paragraph, which stays scoped to SemVer semantics.
