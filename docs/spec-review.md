...
_Triage tally: 13 high, 13 medium retained; 31 low discarded; 4 low findings merged into 2 medium findings; 8 nit dropped; 0 false dropped._
...
---
# T26 — `semver` is referenced as a production dependency but is missing from `package.json`

**Source:** docs/reviews/spec-review/spec-20260507-064438-enriched.md
**Original headings:**
- `semver` library referenced as dependency but absent from `package.json`
- `semver` not declared as production dependency

**Original section:** spec.md — Orientation > Prerequisites > Pi SDK and capabilities; package.json
...
The current `package.json` declares no `semver` entry under `dependencies`, `devDependencies`, or `peerDependencies`. There is also no companion `@types/semver` entry, so a TypeScript build of the probe call site would fail typecheck against an untyped CommonJS module. A flat `node_modules` from npm or yarn would silently satisfy the import via the transitive copy that `@mariozechner/pi-coding-agent` carries, masking the gap; pnpm's strict resolution would reject the import outright.

Plan leaf H1 (`plan_topics/h1-scaffold.md`, *Tests* section) is the leaf chartered to add both the package entries and a literal-read manifest assertion that gates them, so the gap is "scheduled but not yet shipped" rather than "structurally missing from the plan". An implementer reading the spec today, however, sees a present-tense assertion ("pinned as a direct production dependency") that the live tree falsifies.
...
- `spec.md` — Orientation > Prerequisites > Host runtime, *Node version floor* obligation (edited)
- `spec_topics/diagnostics.md` — `loom/load/host-incompatible` row (edited)
- `package.json` — `dependencies`, `devDependencies` (edited)
...
The H1 leaf already declares that it adds `semver` and `@types/semver` as direct production dependencies and adds a manifest literal-read assertion against the `dependencies` entry; the body text needs at most a one-line clarification under the recommended fix that demotes the library prescription from PIC.
...
Land H1 as planned with `semver` + `@types/semver` and the literal-read manifest assertion exactly as planned, and in the same pass demote the library name to a recommended recipe in PIC Step 0 (a)/(d), with mirror edits in `spec.md` Host runtime obligation 1 and the `diagnostics.md` `loom/load/host-incompatible` row.

In `pi-integration-contract.md` Step 0 (a) and (d), restate the normative contract behaviourally ("compare `process.versions.node` against the pinned floor under SemVer ordering, treating prerelease tags as eligible") and move the `semver.satisfies(...)` literal call into a non-normative recipe paragraph. Mirror the change in `spec.md` (which already uses "the `semver`-based comparator" as a forward-link phrase) and in the `diagnostics.md` row (rephrase "compared via `semver.satisfies(…, \">=20.6.0\", { includePrerelease: true })`" as "compared under SemVer ordering against the pinned floor (prerelease tags eligible)").

In `package.json`, when H1 lands, add to `dependencies`:
...
This decouples the library identity from the normative contract; a future comparator swap touches H1 only; the H1 manifest assertion still gates the implementation. The behavioural restatement must explicitly preserve the `includePrerelease: true` semantic for Node-floor comparisons (nightly/RC builds remain eligible) and the caret-range semantic for peer-dep comparisons.
...
- T23 "Capability probe verifies only one of four lock-step peer-dep versions" — must-precede (the extended (d) check uses `semver.satisfies` against four packages instead of one; the missing `semver` `dependencies` entry must be added before that finding's check can run)
- T04 "`^X.Y.Z` prose label \"minor-version line\" only coincides with npm caret semantics" — same-cluster (adjacent `package.json` edit window; both touch the manifest pin block)
