...
**Original heading:** Single-active-session premise lacks a Pi-source citation in PIC (originally T22a, sub-split 2026-05-12 into T22a1 and T22a2, T22a2 ratified back into T22a1 2026-05-13 after Path-A SDK lookup against the V1 pin)
**Original section:** docs/spec_topics/pi-integration-contract.md — Host prerequisites
**Kind:** placement
**Importance:** medium
**Shape:** single
**State:** reduced
**Split from:** T22a (sub-split 2026-05-12 into T22a1 + T22a2; ratified back together 2026-05-13 under Path-A reshape — see header tally update — once an SDK lookup against `@mariozechner/pi-coding-agent ~0.72.1` `docs/extensions.md` confirmed the citation text and removed the human-gate that originally split T22a2 off).
...
Add a new sub-section in `docs/spec_topics/pi-integration-contract.md` under `**Host prerequisites.**` — placed between Host-prerequisites item 4 and the existing `<a id="degraded-state-host-prerequisites"></a>` sub-paragraph, matching the sibling sub-paragraph layout pattern used there — carrying the stable HTML anchor `id="session-binding-contract"`, the existing-paraphrase sentence, and a `Source of truth:` paragraph citing the V1 Pi-SDK pin. Then rewrite the opening sentence of the `id="session-model"` paragraph in `docs/spec.md` as a forward-link to that new anchor. The full sub-section body is:
...
The citation block adds exactly one `Source of truth:` paragraph in the same sub-section as the paraphrase. No fallback-condition prose, no normative MUST, no SDK-doc deep-link beyond the named section headings.

## Solution constraints

- The new PIC sub-section contains exactly the paraphrase sentence and the `Source of truth:` paragraph as written above — do not introduce a fallback-condition clause, an SDK-doc-page-unavailable backup, a normative MUST, or any other prose. (The 2026-05-12 sub-split's T22a2 had a fallback-condition clause; the Path-A reshape ratified T22a2 back into T22a1 *without* that clause because the live SDK lookup confirmed the named sections exist and are stable in the V1 pin, removing the original need for the fallback.)
- Do not pre-install the Future-Considerations contingency cross-link (T22b) or the Pi version-bump checklist item (T22c).
- The only `docs/spec.md` edit permitted is the opening-sentence forward-link rewrite; do not modify the closing sentence about concurrent user sessions (extraction work on it is owned by T15c).
...
- Do not edit `docs/spec_topics/future-considerations.md` from this finding; the V1 non-goals entry's existing `per the Session model paragraph cited below` clause becomes a circular reverse-cite once spec.md forward-links the new PIC anchor instead of FC, but the cleanup (rewriting FC's clause to point at PIC instead) is owned by T22b's resolution.
...
- The sub-section under that anchor contains the paraphrase sentence and a `Source of truth:` paragraph citing `@mariozechner/pi-coding-agent ~0.72.1` `docs/extensions.md` *Lifecycle Overview* and *Session Events* sections plus the closed `SessionShutdownEvent['reason']` set.
- The sub-section contains no fallback-condition clause, no normative MUST, and no SDK-doc deep-link beyond the named section headings.
