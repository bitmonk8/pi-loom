...
**Advisory notes (non-binding).** `hard-ceilings.md:75` and PIC-1 (d) are believed already aligned and best left untouched if a consistent fix is achievable without them. The other CIO-N rules in the same enumeration, and CIO-4's canonical "after the round's tool calls have completed and the slot count has been incremented" wording, are best left untouched unless a lens requires editing them. **Cluster note:** T11a's five "consumes one slot" prose sites are co-resolved with this finding under a single MULTI dispatch (see Relationships); the cluster's fixer is authorised to author both members' territories in one diff and SHOULD do so atomically to avoid the cap-counting-model contradiction that arises mid-edit.
...
- T11a "Forced-respond exemption: replace `consumes one slot` prose" — co-resolve (T11d's CIO-4 rewrite swaps the cap-counting model from "forced respond consumes one slot" to "exempt-routed terminator"; T11a's prose rewrite sweeps the five sibling "consumes one slot" sentences to match. Each member's edits invalidate the other's territory until both land; the pair MUST be dispatched as a single rec-F MULTI cluster so the inner fixer can author both diffs atomically. Prior `must-precede` shape failed three dispatch attempts on `f-stop-2 co-resolve-siblings-territory` — see forensic report at `.pi/tmp/spec-fix-failure-forensics/2026-05-24T12-46-52_a24539/`.).
...
**Advisory notes (non-binding).** `hard-ceilings.md:75` and PIC-1 (d) are believed already aligned and best left untouched if a consistent fix is achievable without them. **Cluster note:** T11d's CIO-4 rewrite, `max_rounds: 0` dispatch elision, and forced-respond non-compliance routing are co-resolved with this finding under a single MULTI dispatch (see Relationships); the cluster's fixer is authorised to author both members' territories in one diff and SHOULD do so atomically.

## Solution constraints

_None as hard constraints._

## Relationships

- T11d "Define CIO-4 branch predicate, `max_rounds: 0` dispatch trigger, and forced-respond non-compliance routing" — co-resolve (T11a's prose rewrite references T11d's exempt-routed-terminator vocabulary; T11d's CIO-4 rewrite swaps the cap-counting model that this finding's five sites currently state. Each member's edits invalidate the other's territory until both land; the pair MUST be dispatched as a single rec-F MULTI cluster so the inner fixer can author both diffs atomically. Prior `must-follow` shape cascade-parked with T11d after T11d's three dispatch attempts failed on `f-stop-2 co-resolve-siblings-territory` — see forensic report at `.pi/tmp/spec-fix-failure-forensics/2026-05-24T12-46-52_a24539/`.).
