...
Declare a new sibling sub-paragraph as the canonical home for the convention. Extend the *Naming convention* paragraph in `docs/spec_topics/frontmatter.md` with one trailing sentence that forward-links to a new sibling sub-paragraph (e.g. "For the binder-model concept's root-word convention across frontmatter, settings, diagnostic, and prose surfaces, see the *Binder-model root-word convention* sub-paragraph below."). Immediately after the *Naming convention* paragraph, insert a new bold-labelled sub-paragraph preceded by the anchor `<a id="binder-model-root-word-convention"></a>` and headed `**Binder-model root-word convention.**`, whose body is the one-sentence convention statement pinning the `bind_` (frontmatter) vs `binder` (settings, diagnostic, prose) root-word convention for the binder-related family. Add a `**binder model**` glossary entry to `docs/spec_topics/glossary.md`, alphabetised between the existing `**binder**` and `**callable set**` entries, whose body is a **back-reference** of the form `See the *Binder-model root-word convention* sub-paragraph in [frontmatter](./frontmatter.md#binder-model-root-word-convention) for the per-surface root-word mapping (`bind_*` frontmatter vs `binder*` / `binder-*` settings, diagnostic, prose).`, NOT a parallel statement of the convention.

## Solution constraints

- Do not rename `bind_model`, `bind_context`, or `bind_echo` to `binder_model` / `binder_context` / `binder_echo`.
- Do NOT restate the per-surface mapping (the four spellings, the `bind_` vs `binder` root-word delta, the relationship to sibling `bind_` fields) inside the glossary entry, inside the *Naming convention* paragraph's forward-link sentence, or anywhere other than the new *Binder-model root-word convention* sub-paragraph. The glossary entry is a back-reference only; the *Naming convention* paragraph's added sentence is a forward-link pointer only; the prose-level statement of the convention lives in the new sub-paragraph and only there.
