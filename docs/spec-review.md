...
# Discovery source failure-mode policy is unspecified

**Source:** docs/reviews/spec-review/spec-20260504-144255.md
**Original heading:** Discovery source failure modes partly unspecified
**Kind:** error-model, completeness

## Finding

`spec_topics/discovery.md` enumerates five discovery sources (global directory, project directory, packages, settings array, CLI flag) and the priority order between them, but it does not say what happens when any individual source is broken. None of the following cases has a defined behaviour:

- The global directory `~/.pi/agent/looms/` does not exist (typical fresh install).
- A discovery root exists but is not readable (permission denied, EACCES on Unix; ACL on Windows).
- A path supplied via `--loom <path>` does not exist or points to a non-`.loom` file.
- A `looms` entry in `settings.json` references a missing path or a path that is not a directory or `.loom` file.
- A `pi.looms` entry in a package's `package.json` references a path outside the package, or a non-existent path inside it.
- A discoverable `.loom` file is itself unreadable (permission, broken symlink, transient I/O error).

`spec_topics/diagnostics.md` defines the `loom/load/*` code namespace and gives "unreadable file" as one example, but it does not say which discovery sources should silently skip vs warn vs fatally error, nor whether a broken individual source aborts the whole scan or is isolated. Two reasonable implementers will diverge: one will treat every missing directory as a warning and pollute the diagnostics channel on every fresh install; another will swallow all I/O errors silently and leave authors unable to debug a misconfigured `--loom` flag.

The asymmetry matters because the sources have different authorial intent. Global and project directories are *conventional locations* — their absence is the normal case. Settings entries, `pi.looms` entries, and `--loom` flags are *explicit references* — their absence is a configuration bug the author wants surfaced.

## Spec Documents

- `spec_topics/discovery.md` — full file, new "Failure modes" subsection (edited)
- `spec_topics/diagnostics.md` — `loom/load/*` namespace examples (edited)
- `plan_topics/v14-tool-calls.md` — V14k–V14o test bullets (read-only; tests follow spec)
...
- M — MVP — (modified) — adds the missing-global-directory test case
- V14k — Discovery: global `~/.pi/agent/looms/` — (modified)
- V14l — Discovery: project `.pi/looms/` — (modified)
- V14m — Discovery: package `looms/` and `pi.looms` — (modified)
- V14n — Discovery: settings `looms` array — (modified)
...
Without per-source failure-mode rules, two implementers will produce observably different extensions: one warns on every missing directory (noise), another silently skips broken `--loom` paths (debuggability hole). End users will see inconsistent diagnostics for the same misconfigurations across versions, and authors of `pi.looms`-shipping packages have no contract for what happens to consumers when a path is wrong.
...
Add a "Failure modes" subsection to `spec_topics/discovery.md` after the source-priority list, with this table:
...
Plus three sentences of supporting text:

1. **Discoverable `.loom` files that are themselves unreadable** are reported as `loom/load/unreadable` *warnings* regardless of source, and the loom is not registered. The scan continues; one bad file does not poison the rest.
2. **All warnings are emitted via the standard diagnostics channel** with codes `loom/load/missing-source`, `loom/load/unreadable-source`, `loom/load/wrong-type-source`, `loom/load/unreadable` — each carrying the source name (e.g., "settings entry index 2", "--loom flag #1", "package `foo`") in `message` so the author can locate the configuration.
3. **Errors are fatal for the offending entry only**, not for the whole discovery pass: a bad `--loom` flag prevents *that* loom from registering and surfaces a `loom/load/missing-source` error, but other `--loom` flags and other sources are still processed.

The asymmetry rule — silent for conventional locations, error for explicit references — is the load-bearing decision; the table operationalises it. The new diagnostic codes slot into the existing `loom/load/*` namespace in `diagnostics.md`; add them to the example list there.

Edge cases the implementer must watch:
- On Windows, "missing" and "permission denied" can both surface as `ENOENT` from `fs.readdir` depending on parent ACLs; treat anything that is neither `ENOENT` on the leaf nor a successful read as the unreadable case.
- A symlink loop under a discovery root is an unreadable-source warning, not an error, even when discovery would otherwise be silent — silent-on-missing is about the *root* not existing, not about traversal failures inside a root that does exist.
- A `--loom` flag pointing at a directory is allowed (mirrors settings entries); the wrong-type rule applies only when the path exists but is neither a `.loom` file nor a directory containing them.

## Related Findings

- "`pi.looms` package.json key is not Pi-recognized" — same-cluster (defines the `pi.looms` shape this finding's `pi.looms` row presumes)
- "`--loom` CLI flag is not a Pi built-in; repeatable string flag type undocumented" — same-cluster (defines the CLI flag whose failure mode this finding tabulates)
- "`settings.json` `looms` array shape unspecified" — same-cluster (defines the settings shape whose failure mode this finding tabulates)
- "Same-priority `.loom` filename collisions undefined" — same-cluster (other gap in the discovery error model; resolved independently)
- "Slash-name validity from filename unspecified" — same-cluster (another `loom/load/*` diagnostic to define alongside these)
...
## spec_topics/schema-subset.md

---

# Depth ≤5: counting algorithm and enforcement point unspecified

**Source:** docs/reviews/spec-review/spec-20260504-144255.md
**Original heading:** Depth ≤5: counting algorithm and enforcement point unspecified
**Kind:** completeness, testability, assumptions

## Finding

`spec_topics/schema-subset.md` declares a normative ceiling — "≤ 5 levels of nesting at runtime (the JSON document depth, not the schema graph)" — but never defines what counts as a level or names the component that enforces it. Two implementers will disagree on the count for the same payload:

- Does `array<T>` consume a level (i.e. is `[{}]` depth 2 or depth 1)?
- Does an `anyOf` arm consume a level, or only the materialised variant?
- Is an empty object `{}` depth 0 or depth 1?
- Are scalars depth 0 or depth 1?

The enforcement point is also unstated. AJV has no native maxDepth keyword (a JSON Schema 2020-12 omission), so depth is not policed by the lowered document; some component must walk the value alongside AJV. Nothing in `schema-subset.md`, `query.md`, `tool-calls.md`, or `frontmatter.md` says which component, at which boundaries, or what `QueryError` variant the violation produces. The plan inherits the gap: V4i and V11i both reference the cap but cite no algorithm, so their tests ("depth-5 accepted, depth-6 rejected") cannot be written without first picking a counting rule.

## Spec Documents

- `spec_topics/schema-subset.md` — Depth bullet (edited)
- `spec_topics/query.md` — Failure modes / `ValidationError` (read-only — recipient of the error)
- `spec_topics/schemas.md` — recursion paragraph cross-referencing depth (read-only)
- `spec_topics/implementation-notes.md` — Runtime / AJV configuration (read-only — confirms AJV has no maxDepth keyword)
- `spec_topics/frontmatter.md` — `params` AJV validation site (read-only — additional enforcement boundary)
- `spec_topics/tool-calls.md` — tool-arg AJV validation site (read-only — additional enforcement boundary)
...
- V4a — AJV pipeline scaffold — (modified — the depth-walk hook lives in the validator service introduced here)
- V4i — Recursive schema references — (modified — its "depth-4 accepted, depth-6 rejected" tests need the precise counting rule)
- V11i — Runtime depth cap of 5 — (modified — primary leaf; current acceptance criteria silently assume the algorithm and the enforcement site that this finding pins down)

## Consequence

**Severity:** correctness

Two reasonable implementers will produce different answers for the same nested document — one counting `array<T>` as a level, one not — yielding inconsistent acceptance/rejection behaviour and inconsistent error paths. Without a designated enforcement point, the cap risks being silently dropped from one validation boundary (e.g. tool-arg validation) while applied to another (typed-query response), so authors cannot rely on it. V11i's tests cannot be written until the rule is fixed.
...
Add a "Depth enforcement" subsection to `spec_topics/schema-subset.md` immediately after the Depth bullet, specifying:

**Counting algorithm.** Depth is a property of the runtime JSON value, not of the schema graph:

- A scalar (`string`, `number`, `integer`, `boolean`, `null`) has depth `1`.
- An empty object `{}` or empty array `[]` has depth `1`.
- A non-empty object or array has depth `1 + max(depth(child))` over its members / elements.
- `anyOf` arms are not levels: only the materialised value is measured (one arm exists at runtime; the others do not).
- The cap is `depth ≤ 5`. Examples: `{"a": 1}` is depth 2; `{"a": [{"b": 1}]}` is depth 4; `{"a": {"b": {"c": {"d": {"e": 1}}}}}` is depth 6 and rejected.

**Enforcement point.** The depth check lives in the validator service (the same component that holds the AJV cache). It runs at every site where AJV validates a Loom-declared schema against runtime JSON:

1. Typed-query response validation (after the model's final assistant text is JSON-parsed).
2. Tool-call argument validation, both model-driven (post-decode of the model's `arguments`) and code-driven (pre-encode of `<name>(...)` arguments).
3. `params` validation at loom invocation (whether bound via the binder or supplied by `invoke`).

The walk runs **before** AJV (cheap fast-fail; avoids feeding pathologically deep payloads into the validator). Implementation: a recursive descent over the parsed JSON value with a depth counter; the first node whose depth would exceed 5 short-circuits and produces the failure.

**Error shape.** A depth violation surfaces as a standard validation failure: `Err(QueryError { kind: "validation", validation_errors: [ValidationFailure { path: "<JSON-Pointer to first too-deep node>", message: "JSON document depth exceeds 5", schema_keyword: "maxDepth" }], raw_response: <original text>, attempts: <as for any validation failure> })`. Because it is a `validation` error, it participates in coercion follow-ups exactly like any other AJV failure; the depth walk re-runs on each follow-up's response.

**Edge cases the implementer must watch:**

- The depth walk must run on the *post-decode* JSON value, not on the raw assistant text. A response that fails to parse as JSON surfaces as a JSON-parse validation failure (existing path), not a depth failure.
- Depth applies to the *whole* validated value, including any nested `array<T>` / nested object hoisted via inline schemas — the lowering pass's `__inline_<hash>` indirection does not affect runtime depth.
- For `params` whose declared types are primitive-only or `array<T>` over primitives (the V3 surface), depth is bounded structurally at 2 and the walk is effectively a no-op; do not special-case it — keep the walk uniform across all AJV validation sites so future widening of `params` types inherits the cap automatically.
- A schema graph cycle (`schema Tree { children: array<Tree> }`) is permitted; the cap fires only on the data instance.

## Related Findings

- "Cancellation checkpoints miss binder, AJV validation, and schema-lowering" — same-cluster (both touch the validator service; cancellation work and depth-walk land in the same component but resolve independently)
...
# Binder LLM call has no cancellation checkpoint

**Source:** docs/reviews/spec-review/spec-20260504-144255.md
**Original heading:** Cancellation checkpoints miss binder, AJV validation, and schema-lowering
**Kind:** completeness, implementability

## Finding

`spec_topics/cancellation.md` enumerates exactly three checkpoint families: every loop iteration boundary, before every `@`-query, and before every tool / `invoke` call. The slash-command argument binder — which `spec_topics/binder.md` defines as a one-shot LLM call to a tier-2 model that runs *before* any of the loom's own queries — is not in that list, and `binder.md` itself never mentions cancellation, `AbortSignal`, or any abort path. Neither page states whether `ctx.signal` is forwarded to the binder's underlying provider call.

The "typical sub-second" cost figure in `binder.md` understates the worst case: a cold provider, a slow tier-2 model, or a transport hiccup followed by the spec-mandated single retry can keep the binder in flight for many seconds. During that window the user has pressed `/foo …`, sees nothing, and (per the current spec) cannot cancel — pressing Esc has no defined effect because the binder is not a checkpoint and is not a tool / query / invoke / loop. The five-row failure-mode table in `binder.md` has no row for the cancelled case.

The original framing also names "AJV validation pass" and "schema-lowering at file-load time" as missing checkpoints. Those are millisecond-scale synchronous operations on non-interactive paths (lowering happens at load, AJV runs over already-received bytes); they are correctly not checkpoints. The spec should still say so explicitly so implementers do not bolt on speculative checks, but they are not the substance of the gap.

## Spec Documents

- `spec_topics/cancellation.md` — Granularity paragraph (edited)
- `spec_topics/cancellation.md` — Surfacing list (edited)
- `spec_topics/binder.md` — Failure modes table (edited)
- `spec_topics/binder.md` — Cost and latency paragraph (edited)
- `spec_topics/slash-invocation.md` — `cancelled` row in the per-`kind` system-note table (read-only — confirms surface text style)
- `spec_topics/implementation-notes.md` — Runtime / AJV configuration (read-only — confirms AJV path is in-process)
- `spec_topics/schemas.md` — Lowering algorithm (read-only — confirms lowering happens at load)
...
- V16e — `binder_model` resolution chain — (read-only — confirms the binder uses the same provider plumbing that already accepts a signal)
- V16f — `bind_context: none` — (modified — pre-flight signal check + signal forwarded to the provider call)
- V16g — `bind_context: session` — (modified — same as V16f for the session-context path)
- V16n — Binder transport failure single retry — (modified — abort observed during retry surfaces as cancelled, not as a second transport failure)
- V16o — Binder malformed envelope handling — (modified — abort during retry surfaces as cancelled)
- V18b — `AbortSignal` before every `@` query — (read-only — establishes the pre-flight pattern the binder leaves should mirror)

A new V16 leaf (e.g. `V16q — Binder cancellation`) is the natural home for the pre-flight + in-flight abort behaviour and the new system-note row; it would depend on V16f / V16n. Adding it is an editorial choice for the plan owner — the modifications above can also live inside the existing leaves.

## Consequence

**Severity:** correctness

Two implementers reading the current spec will diverge: one will forward `ctx.signal` to the binder's provider call (because every other LLM call in the system does), the other will treat the binder as uncancellable infrastructure (because `cancellation.md` does not list it and `binder.md` does not mention signals). The user-visible result of the second implementation is a slash command that ignores Esc until the binder finishes, which is the exact "looks frozen" failure the original finding describes. The failure-mode table also has no row for the cancelled case, so even an implementer who wires the signal through has no specified system-note text.
...
Edit `spec_topics/cancellation.md` Granularity paragraph to read: "The interpreter checks the signal at every loop iteration boundary, before every `@`...`` query, before every tool / `invoke` call, **and before and during the slash-command argument binder's LLM call**." Add a sentence: "Synchronous in-process work — schema lowering at load time, AJV validation of received bytes, default-merging — is not a checkpoint; it runs to completion. These steps are millisecond-scale and off the interactive hot path."

Edit `spec_topics/binder.md`:

1. Add to the binder section (near the failure-modes paragraph): "The binder participates in cancellation. The runtime checks `ctx.signal` immediately before issuing the binder call and forwards the signal to the binder model's provider invocation; both the initial attempt and the single transport-failure retry honour the signal. A cancelled binder produces the system note below and the loom does not run."
2. Add a sixth row to the failure-modes table:

   | Cause | System note |
   |---|---|
   | `ctx.signal` aborted before or during the binder call | `loom /<name>: argument binding cancelled` |

3. Note that the binder-bypass path (single no-default `string` param) is naturally cancellable at the next regular checkpoint inside the loom body, since no LLM call is made; the cancelled-binder system note does not apply to bypass-eligible looms.

Edge cases the implementer must watch:

- Abort observed *after* the binder returned `ok` but *before* AJV validation runs: AJV is fast and uncancellable; let it complete, then surface the cancellation at the next checkpoint inside the loom body (consistent with the "no retroactive `Ok` → `Err`" rule that the sibling race-conditions finding asks for).
- Abort observed during the single transport-failure retry: do not retry; surface the cancelled note immediately.
- Abort observed before the binder is even dispatched (signal already aborted at slash-command entry): surface the cancelled note without issuing the LLM call.
- The cancelled-binder note replaces execution (like `needs_info` / `ambiguous`); it is not the same surface as the loom's own top-level `cancelled` system note in `slash-invocation.md`, which fires when the loom body is cancelled mid-execution.

## Related Findings

- "Cancellation 'smallest unit' definition is ambiguous" — same-cluster (both touch the checkpoint list in `cancellation.md`; same edit pass)
- "Cancellation race conditions unspecified" — co-resolve (the "no retroactive `Ok` → `Err`" rule it requests is the principle the binder edge cases above rely on; both should land in the same revision of `cancellation.md`)
...
# Diagnostic codes are declared as namespaces but never assigned to specific errors

**Source:** docs/reviews/spec-review/spec-20260504-144255.md
**Original heading:** No diagnostic codes assigned to named parse errors
**Kind:** testability, traceability

## Finding

`spec_topics/diagnostics.md` defines four code namespaces (`loom/parse/*`, `loom/type/*`, `loom/load/*`, `loom/runtime/*`) and shows the example string `"loom/parse/binding-case-mismatch"` — but no concrete code is assigned to any specific error anywhere in the spec. A `grep` across `spec.md` and all of `spec_topics/` returns those four namespace bullets and the one example string, and nothing else. Meanwhile the spec names dozens of specific parse errors and warnings — `break` outside a loop, backslash followed by any other character, mixed-type `+` operands, ambiguous-discriminator, invocation cycle, mismatched-arm `match`, `?` outside a `Result` function, redundant wire-name rename, `bind_context: session` on subagent, `timeout:` field, and so on (see `spec_topics/bindings.md`, `control-flow.md`, `expressions.md`, `lexical.md`, `query.md`, `schemas.md`, `imports.md`, `invocation.md`, `frontmatter.md`, `v8-control-flow.md` test bullets, etc.).

Two consequences. First, tests can only assert that a parse failed, not that the *intended* error fired. A regression that emits the wrong code for the right input — or that conflates two distinct rules into one error — is undetectable from outside the parser. Second, downstream tooling (LSP quick-fixes, Pi system-note formatters, author-facing docs, error-message localisation) has no stable handle to key off; everything ends up matching on free-form English message text, which is brittle and re-translation-hostile.

The plan already shows the gap leaking through. `plan_topics/m-mvp.md` invents `loom/parse/unsupported-feature` and `loom/parse/unterminated-template` ad hoc; `v3-frontmatter.md` invents `loom/load/unknown-frontmatter-field`; `v14-tool-calls.md` invents `loom/load/unknown-tool`; `v15-invoke.md` invents `loom/load/prompt-mode-callable` and `loom/load/invocation-cycle`. These were minted by plan authors at the leaves where they were needed, with no central registry to check against. Two leaves landing in parallel will easily mint two different codes for the same situation, or the same code for two different situations.

## Spec Documents

- `spec_topics/diagnostics.md` — entire file (edited): adds a normative code-registry appendix and a rule that every author-visible diagnostic site must cite a registered code.
- `spec_topics/lexical.md` — string-literal, number-literal, identifier, comment paragraphs (edited): each named parse error annotated with its registered code.
- `spec_topics/bindings.md` — immutability, assignment-as-expression, `++`/`--` paragraphs (edited).
- `spec_topics/expressions.md` — truthiness, mixed-`+`, comparison-chaining, object-construction, array-construction paragraphs (edited).
- `spec_topics/control-flow.md` — `for`/`break`/`continue` rules (edited).
- `spec_topics/query.md` — escapes, discarded-result rule (edited).
- `spec_topics/errors-and-results.md` — `match` arm-type, `?` placement, panic sources (edited).
- `spec_topics/return.md` — bare-return rule (edited).
- `spec_topics/schemas.md` — unknown-variant, ambiguous/missing discriminator, duplicate-discriminator, nested-discriminator rules (edited).
- `spec_topics/frontmatter.md` — `system:` on prompt-mode, `tools:` resolution, `params:` defaults, `binder_*`/`bind_*`, `retry`, unknown-field warning (edited).
- `spec_topics/imports.md` — top-level-only, name-collision, cycle-detection rules (edited).
- `spec_topics/invocation.md` — argument-binding, cycle-detection rules (edited).
- `spec_topics/tool-calls.md` — argument-shape rule (edited).
- `spec_topics/binder.md` — bypass + `bind_echo: true` warning (edited).
- `spec_topics/cancellation.md` — `timeout:` rejection (edited).
- `spec_topics/runtime-value-model.md` — runtime-panic catalogue (edited, for `loom/runtime/*` codes).
- `spec_topics/type-system.md` — type-mismatch, unknown-identifier, schema-constraint sites (edited).
- `spec.md` — TOC / appendix anchor (edited): link the new code-registry appendix.

## Plan Impact

**Phases:** Horizontal, MVP, Vertical V1, Vertical V2, Vertical V3, Vertical V4, Vertical V5, Vertical V6, Vertical V7, Vertical V8, Vertical V9, Vertical V10, Vertical V11, Vertical V12, Vertical V13, Vertical V14, Vertical V15, Vertical V16, Vertical V17, Vertical V18

**Leaves (implementation order):**

- H3 — Diagnostics primitive and multi-error accumulator — (modified): the namespace constants gain an exhaustive registry of concrete codes (or a typed enum) and a lint/architectural test forbidding `DiagnosticsAccumulator` calls with codes outside the registry.
- M — Minimal end-to-end loom — (modified): the two ad-hoc codes (`loom/parse/unsupported-feature`, `loom/parse/unterminated-template`) become registry references rather than string literals minted in the leaf.
- V1a — Numeric literals — (modified): hex/octal/binary/underscore-separator tests assert specific codes.
- V1b — String literals and escapes — (modified): illegal-escape, literal-newline, unterminated-string tests assert specific codes.
- V1c — Line comments — (modified): block-comment rejection asserts a specific code.
- V1d — Identifier case rule and reserved keywords — (modified): case-mismatch and reserved-keyword-in-identifier-position tests assert specific codes (the spec's existing example `loom/parse/binding-case-mismatch` lands here).
- V1e — Statement separators — (modified): bracket-balance and semicolon-rejection tests assert specific codes.
- V2a — `let` bindings — (modified): immutable-rebinding test asserts specific code.
- V2b — `let mut` and assignment — (modified): assignment-as-expression test asserts specific code.
- V2c — Operator precedence and truthiness — (modified): non-boolean-condition, comparison-chaining, mixed-`+` tests assert specific codes.
- V2d — Member and index access — (modified): assignment-to-member/index test asserts specific code.
- V2h — Array construction — (modified): mixed-element-type-without-sink test asserts specific code.
- V3a — Frontmatter parser — (modified): unknown-field warning, deferred-field warning, `mode: subagent` deferred-error tests assert specific codes (`loom/load/unknown-frontmatter-field` lands here).
- V4b — Object schemas — (modified): unknown-field, missing-field, extra-field tests assert specific codes.
- V4c — Type aliases and unions — (modified): aliasing tests assert specific codes.
- V5d — Template escapes — (modified): unknown-escape test asserts specific code.
- V5f — Bare expression-statement query — (modified): documented-diagnostic test asserts specific code.
- V6c — Schema annotation on `@`-query — (modified): annotation-mismatch tests assert specific codes.
- V6h — `?` placement — (modified): `?`-in-non-Result-function test asserts specific code.
- V7a — `match` arm syntax — (modified): arm-type-mismatch test asserts specific code.
- V7e — Constructor patterns — (modified): unknown-constructor test asserts specific code.
- V7g — Deferred pattern features — (modified): guarded-arm, rest-pattern tests assert specific codes (one per deferred feature, not a single "deferred").
- V8a — `if`/`else` — (modified): single-line-`if` rejection asserts specific code.
- V8b — `for`/`in` — (modified): non-array-iterand, mutating-iteration-variable tests assert specific codes.
- V8d — `break` — (modified): break-outside-loop, `break expr` deferred tests assert specific codes.
- V8e — `continue` — (modified): continue-outside-loop test asserts specific code.
- V8f — `return` — (modified): bare-`return`-in-non-void, unreachable-code warning tests assert specific codes.
- V9a — Function definitions — (modified): nested-`fn` test asserts specific code.
- V9b — Tail-expression — (modified): tail-type-mismatch test asserts specific code.
- V9d — `?`-induced return inference — (modified): conflicting-declaration test asserts specific code.
- V10a — Enum declarations — (modified): duplicate-explicit-value, non-string-RHS tests assert specific codes.
- V10c — Variant access — (modified): unknown-variant test asserts specific code.
- V11b — Multiple-discriminator-candidates — (modified): asserts specific code.
- V11c — No-discriminator-candidate — (modified): asserts specific code.
- V11f — Nested-discriminator rejection — (modified): asserts specific code.
- V12b — `system:` on prompt-mode — (modified): asserts specific code.
- V12f — `bind_context: session` on subagent warning — (modified): asserts specific warning code.
- V13a — Wire-name rename syntax — (modified): redundant-rename warning, same-line-doc-comment-error tests assert specific codes.
- V14a — Tool-name resolution — (modified): unknown-tool test asserts specific code (`loom/load/unknown-tool` lands here).
- V14p — Source-priority shadowing — (modified): cross-source-collision warning asserts specific code.
- V15g — Cross-mode invocation rejection — (modified): asserts specific code (`loom/load/prompt-mode-callable`).
- V15l — Invocation cycle — (modified): asserts specific code (`loom/load/invocation-cycle`).
- V16k — Bypass + `bind_echo: true` warning — (modified): asserts specific warning code.
- V17a — `.warp` top-level restriction — (modified): top-level-statement, `let`, query rejection tests assert specific codes.
- V17h — `.warp` import cycle — (modified): asserts specific code.
- V17j — Import name collision — (modified): asserts specific code.
- V18j — Multi-error rollup — (modified): the 5-parse-errors-+-1-type-error test asserts that the six emitted diagnostics have six distinct, registered codes.
- V18o — Per-call timeout marker — (modified): `timeout:` rejection asserts specific code.

## Consequence

**Severity:** correctness

Without a normative code registry the spec's diagnostics surface ships in a state where (a) two implementers, or two leaves landing in parallel, will mint different codes for the same condition, and (b) tests that care about diagnostic identity have nothing to assert against beyond the namespace prefix and English message text. Downstream consumers (the system-note formatter from V18i, future LSP integrations, future doc-site cross-links from message → spec rule) have no stable contract to bind to.
...
Add a normative **Diagnostic Code Registry** appendix to `spec_topics/diagnostics.md`. The appendix is a table with columns `Code | Severity | Phase | Trigger | Spec rule (link) | Hint template (optional)`. Every named diagnostic in the spec — every parse error, parse warning, type error, load error/warning, and runtime panic — gets exactly one row. Each non-appendix mention of a diagnostic in the spec topics is rewritten to cite its code (e.g. *"`break` outside a loop is a parse error (`loom/parse/break-outside-loop`)"*).

Add three normative rules to `spec_topics/diagnostics.md` itself:

1. *Every author-visible diagnostic emitted by the runtime MUST carry a code from the registry. Emitting an unregistered code is a defect.*
2. *The registry is closed: adding a new code is a spec change, not an implementation change.*
3. *Codes are stable identifiers; renaming a code is a breaking change to the public diagnostics contract.*

Naming convention for new codes: `<namespace>/<kebab-case-rule-name>`, derived from the spec rule's identifier or short name. Examples already implied by plan or spec: `loom/parse/binding-case-mismatch`, `loom/parse/unsupported-feature`, `loom/parse/unterminated-template`, `loom/parse/break-outside-loop`, `loom/parse/illegal-escape`, `loom/parse/non-boolean-condition`, `loom/parse/comparison-chaining`, `loom/parse/mixed-plus-operands`, `loom/parse/ambiguous-discriminator`, `loom/parse/missing-discriminator`, `loom/parse/duplicate-discriminator-value`, `loom/parse/nested-discriminator`, `loom/parse/unknown-variant`, `loom/parse/system-on-prompt-mode`, `loom/parse/redundant-wire-name`, `loom/parse/discarded-query-result`, `loom/parse/match-arm-type-mismatch`, `loom/parse/question-outside-result-fn`, `loom/parse/bare-return-in-non-void`, `loom/parse/nested-fn`, `loom/load/unknown-frontmatter-field`, `loom/load/unknown-tool`, `loom/load/prompt-mode-callable`, `loom/load/invocation-cycle`, `loom/load/import-cycle`, `loom/load/import-name-collision`, `loom/load/duplicate-slash-name`, `loom/runtime/match-error`, `loom/runtime/index-out-of-bounds`, `loom/runtime/null-access`, `loom/runtime/missing-key`. The recommendation is to enumerate them, not invent a generation rule — the registry is the source of truth.

In the plan side, H3 grows a typed enum (or `as const` lookup) generated from the registry, and an architectural test that fails on any `accumulator.report(...)` call whose `code` argument is not a member of that enum. Each subsequent leaf updates its test bullets to assert the specific code, not just "parse error". This is mechanical and fits the existing per-leaf TDD ritual.

Edge cases the implementer must watch:

- The eight runtime-panic sources from `runtime-value-model.md` cluster — registry must enumerate each (`MatchError`, OOB on `arr[i]`, `null.field`, `null[i]`, missing object key, division-by-zero if any, integer overflow if any, explicit `panic`) rather than collapsing to one code.
- Some sites currently described as a single "parse error" actually cover several distinct conditions (e.g. `expressions.md` truthiness — non-boolean in `if`, in `while`, in ternary, in `&&`/`||`). Decide per-site whether they share a code or split; default is split unless the rule is identical and the formatter only varies the location.
- The hint text in the registry is normative when present, advisory when omitted; existing spec-required hints (`obj.keys()` for non-array `for`, `if (x != "")` for non-boolean truthiness, etc.) should be lifted into the hint column verbatim so per-leaf tests can assert them.
- Co-resolution with the panic-message-content finding: hint and message templates can share the same row, and the panic-source enumeration in the registry IS the answer to that finding.

## Related Findings

- "Panic message content per source unspecified" — co-resolve (the registry's `loom/runtime/*` rows are the home for normative panic-message templates).
- "Logging, metrics, and cost/token accounting missing entirely" — same-cluster (sits in the same `diagnostics.md` topic; the observability section can key telemetry off the same code registry, but the two findings resolve independently).
- "`LoadExtensionsResult.errors` is not a runtime-callable diagnostics channel" — same-cluster (also touches the diagnostics surface but is about delivery transport, not identity).
- "Per-`kind` system-note table covers only 5 of 8 `QueryError` variants" — same-cluster (a parallel completeness gap on the `QueryError`/system-note surface; both findings demand exhaustive enumeration of an open-coded set).
- "Missing normative grammar appendix" — same-cluster (analogous fix shape — promote scattered normative content into a closed appendix; both should be done together to keep `spec.md` appendix structure coherent).
- "No requirement identifiers anywhere in the spec; no acceptance criteria" — same-cluster (broader version of the same identifier-stability problem; resolving diagnostic codes does not solve it but uses the same pattern of closed identifier registry).
...
**Source:** docs/reviews/spec-review/spec-20260504-144255.md
**Original heading:** `QueryError` union has three conflicting authoritative definitions
**Kind:** consistency, cross-spec-consistency-broad, implementability, naming

## Finding

Three spec pages each present a `QueryError` definition or enumeration, and they disagree:

- `spec_topics/query.md` (lines 144–151) — the canonical declaration: 8 variants
  (`ValidationError | TransportError | ToolFailureError | ToolCallError |
  ContextOverflowError | CancelledError | InvokeFailure | InvokeCalleeError`).
- `spec_topics/descriptions.md` (lines 29–35) — embedded inside an example for
  `///` placement: 7 variants, **omits `ToolCallError`**. The block is
  schema-shaped and prefixed with the doc-comment `/// Top-level error returned
  by every query`, so a reader cannot tell it is illustrative rather than
  normative.
- `spec_topics/slash-invocation.md` (lines 20–26) — the per-`kind` system-note
  table for prompt-mode top-level `Err`: only 5 rows (`validation`, `transport`,
  `tool_failure`, `context_overflow`, `cancelled`). Three real kinds —
  `tool_call`, `invoke_failure`, `invoke_callee_error` — are unaccounted for, so
  an implementer of the formatter has no spec text to reach for when one of
  them surfaces.

Compounding this, `ToolCallError`'s discriminator is `kind: "tool_call"`. Every
other variant uses a failure-noun discriminator (`"validation"`, `"transport"`,
`"tool_failure"`, `"context_overflow"`, `"cancelled"`, `"invoke_failure"`,
`"invoke_callee_error"`); `"tool_call"` is an action noun and reads as the name
of a successful operation. The inconsistency is visible at every `match` site
and undermines the spec's own claim that `kind` discriminators name failure
categories.

## Spec Documents

- `spec_topics/query.md` — "Failure modes" (edited, authoritative union)
- `spec_topics/tool-calls.md` — "Failures" (edited, `ToolCallError.kind` rename and cross-link)
- `spec_topics/descriptions.md` — `///` placement example (edited, sync or de-normativise the example)
- `spec_topics/slash-invocation.md` — top-level `Err` per-`kind` table (edited, add missing rows or scope the table)
- `spec_topics/invocation.md` — `InvokeFailure` / `InvokeCalleeError` definitions (read-only, cross-reference target)
- `spec_topics/errors-and-results.md` — match grammar example using `QueryError` (read-only, sanity check)

## Plan Impact

**Phases:** Vertical V5, Vertical V6, Vertical V14, Vertical V18

**Leaves (implementation order):**

- V5g — `QueryError` union — initial variants — (modified — list of "later kinds" in the **Adds** bullet must rename `tool_call` → `tool_call_error`)
- V6i — AJV validation of typed query results — (modified — example `Err(QueryError {kind:"validation", ...})` stays; but the leaf's spec link target moves if the union is consolidated)
- V14f — `ToolCallError` variant: `validation` cause — (modified — discriminator and any `cause` references)
- V14g — `ToolCallError` variant: `execution` cause — (modified)
- V14h — `ToolCallError` variant: `cancelled` cause — (modified)
- V14i — `ToolCallError` variant: `unknown_tool` cause — (modified)
- V18i — Per-`kind` formatting for prompt-mode top-level `Err` — (modified — table grows from 5 to 8 rows; tests must enumerate every kind)

## Consequence

**Severity:** correctness

Two implementers reading different pages will produce different `QueryError`
unions: one will omit `ToolCallError` (descriptions.md), one will omit the
invoke kinds from the prompt-mode formatter (slash-invocation.md), and a third
will get the canonical 8-variant set (query.md). The `kind: "tool_call"`
discriminator additionally breaks the failure-noun pattern that `match` arms
across the spec rely on for readability — code-review findings will keep
flagging it. The V18i system-note formatter has no spec text covering three
real kinds and will silently drop them or print whatever the implementer
invents.
...
Make `spec_topics/query.md` the sole authoritative declaration of `QueryError`
and reconcile the other two sites against it; rename the off-pattern
discriminator. Concretely:

1. **Rename `ToolCallError.kind`** from `"tool_call"` to `"tool_call_error"`
   in `spec_topics/tool-calls.md` (line 29) and update every reference:
   `query.md` parenthetical (line 154), the variant comparison paragraph
   (line 156), and the `slash-invocation.md` table (new row, see step 3).
   Update plan leaves V5g, V14f–V14i to use the new name. Rationale for the
   suffix `_error` rather than e.g. `direct_tool` or `code_tool`: it changes
   only the discriminator, not the schema name, so existing prose and the
   `ToolCallError` / `ToolFailureError` distinction survive untouched. The
   companion finding "*`ToolCallError` / `ToolFailureError` names do not
   signal their contexts*" can independently revisit the schema names; this
   fix does not commit to either renaming.

2. **De-normativise `descriptions.md`'s `QueryError` block.** Replace the
   8-line schema declaration with a 2-line stub that still exhibits `///`
   on a `schema =` union but does not pretend to enumerate variants. For
   example:
   ...
   The `…` (or an explicit `// see query.md for the full union`) prevents
   future drift. Add a sentence under the example: *"Truncated; the
   authoritative union lives in [Query](./query.md)."*

3. **Extend the `slash-invocation.md` per-`kind` table** to cover all eight
   kinds. Add three rows after the existing five:
   ...
   Exact wording is the implementer's call; what the spec must commit to is
   that every `kind` has a defined formatting. Mention in the surrounding
   prose that for `invoke_callee_error` the chain-attribution suffix already
   handles the deeper `inner` recursion (see existing paragraph immediately
   after the table).

Edge cases the implementer must watch:

- The discriminator rename is a wire-format change. Because V1 has not shipped,
  no compatibility shim is needed, but every test fixture under V14f–V14i and
  any snapshot of `QueryError` JSON must be updated in the same commit.
- The `slash-invocation.md` table edit is downstream of the rename — write the
  rename first, then add the new rows using the new discriminator string.
- The companion finding "*`QueryError` variants split across three files with
  no consolidated reference*" proposes hoisting all variant schemas into
  `errors-and-results.md`. If that hoist lands, query.md's role here becomes a
  cross-reference rather than the schema home; the rename and the
  de-normativisation in this finding still apply unchanged.

## Related Findings

- "`QueryError` variants split across three files with no consolidated reference" — same-cluster (about `QueryError` placement; resolves independently but a co-resolution moves the authoritative union into `errors-and-results.md` instead of leaving it in `query.md`)
- "`ToolCallError` / `ToolFailureError` names do not signal their contexts" — decision-dependency (a schema-name rename there reframes the discriminator decision here; this finding deliberately renames only the discriminator to stay compatible with either outcome)
- "`ValidationFailure` / `ValidationError` — \"failure\" and \"error\" at wrong nesting levels" — same-cluster (touches the same `QueryError`-adjacent naming surface)
...
# File encoding, newline normalisation, and path resolution unspecified

**Source:** docs/reviews/spec-review/spec-20260504-144255.md
**Original heading:** File encoding, newline normalisation, and path resolution unspecified
**Kind:** assumptions, completeness

## Finding

Three foundational input-handling rules are absent from the spec, and each one is the kind of detail two implementers will silently disagree on:

1. **Source-file encoding.** Neither `lexical.md` nor any other page states what encoding `.loom` and `.warp` files use. UTF-8 is implied by the `\u{XXXX}` escape and by the existence of Unicode code-point handling, but the behaviour on a BOM-prefixed file, on Latin-1 or UTF-16 input, on invalid UTF-8 byte sequences, and on lone surrogates is undefined.

2. **Newline normalisation.** Statement-termination, multi-line templates, dedent (`query.md` §dedent), and `///` doc-comment joining (`descriptions.md`) all reason in terms of "newlines" but never say *which* byte sequence counts. A `.loom` checked in on Windows with `\r\n` line endings will produce different `length`s, different dedent prefixes, and different rendered query text than the same file on Linux unless normalisation is mandated. The dedent algorithm in particular is sensitive — a stray `\r` at the end of every line changes the common-prefix calculation.

3. **Path syntax in source literals.** `import "./shared/personas.warp"`, `invoke("./plan.loom", ...)`, and `tools: ["./summarise.loom"]` all use forward-slash literals throughout the spec, but no rule says forward-slash is mandatory, no rule says backslash is rejected (or silently translated), and no rule covers case sensitivity on case-insensitive filesystems (macOS default, Windows). Two looms in the same directory differing only by case (`Plan.loom` vs `plan.loom`) load on Linux and collide on Windows; today the spec gives an implementer no answer about which behaviour is conformant.

The discovery, import, and invocation pages all assume these rules without writing them down, so the missing answers leak across the whole front end of the loader.

## Spec Documents

- `spec_topics/lexical.md` — new "Source files" preamble paragraph (edited)
- `spec_topics/imports.md` — Path resolution paragraph (edited)
- `spec_topics/invocation.md` — Resolution paragraph (edited)
- `spec_topics/frontmatter.md` — `tools:` `.loom` path bullet (edited)
- `spec_topics/discovery.md` — case-collision rule (edited)
- `spec_topics/query.md` — dedent / newline-trim cross-reference to normalisation rule (read-only)
- `spec_topics/descriptions.md` — `///` joining cross-reference (read-only)

## Plan Impact

**Phases:** Horizontal, MVP, Vertical V1, Vertical V5, Vertical V14, Vertical V15, Vertical V17

**Leaves (implementation order):**

- H2 — `FakeFileSystem` / `FileSystem` interface — (modified) — must specify UTF-8 decode contract and CRLF→LF normalisation point (decode-time vs lex-time)
- M — Minimal end-to-end loom — (modified) — discovery + lex must apply the normalisation rules so Windows-checked-in `hello.loom` produces identical behaviour
- V1b — String literals and escapes — (modified) — "literal newline in regular string is a parse error" needs to be defined post-normalisation (a `\r\n` source line is one newline, not two)
- V1e — Statement separators and newline continuation — (modified) — separator detection runs against normalised `\n`
- V5c — Multi-line templates: newline-trim and dedent — (modified) — dedent test vectors must be normative against normalised input; add a CRLF-input fixture proving identical output to the LF-input fixture
- V14k — Discovery: global `~/.pi/agent/looms/` — (modified) — case-collision policy on case-insensitive filesystems
- V14l — Discovery: project `.pi/looms/` — (modified) — same
- V14m — Discovery: package `looms/` and `pi.looms` — (modified) — same
- V14o — Discovery: `--loom` CLI flag — (modified) — path syntax / separator handling on Windows-supplied flag values
- V15a — `invoke("./path.loom", ...)` parsing and resolution — (modified) — forward-slash-only literal rule; backslash → parse error
- V15e — `.loom` paths in `tools:` — (modified) — same
- V17c — `import { X } from "./y.warp"` — (modified) — same
- V17j — `invoke` from `.warp` resolves relative to `.warp` file — (modified) — same

## Consequence

**Severity:** correctness

A loom authored on Windows and committed with CRLF endings will dedent and tokenise differently on Linux — the same `.loom` file produces different prompts on different machines, breaking reproducibility and making CI failures impossible to reproduce locally. A loom directory containing both `Plan.loom` and `plan.loom` loads cleanly on Linux and silently shadows on macOS. A non-UTF-8 file (e.g. UTF-16 from a Notepad save) will either crash with an opaque internal error or produce mojibake in queries, both of which are observer-visible. None of these are exotic — they are first-day issues for any cross-platform user.
...
Add a "Source Files" paragraph to `spec_topics/lexical.md` (place it as the first section, before "Identifiers") with three rules:

1. **Encoding.** Source files are UTF-8. A leading UTF-8 BOM (`EF BB BF`) is consumed and ignored; any other BOM, or any byte sequence that is not valid UTF-8 (including lone surrogates), is a load-time error `loom/load/invalid-encoding` reporting the file path and byte offset of the first invalid byte. No transcoding from other encodings is performed.

2. **Newline normalisation.** Before lexing, the runtime normalises line endings: `\r\n` → `\n` and bare `\r` → `\n`. Every spec rule that mentions "newline" — statement separation, "literal newline in regular string is a parse error", `@`...`` template newline-trim and dedent, `///` doc-comment joining — operates on the normalised stream. Test vectors for V5c (dedent) must include a CRLF-input fixture asserting byte-identical output to the LF-input fixture.

3. **Path literals.** All path literals in source (`import "..."`, `invoke("...", ...)`, `tools: ["..."]`) use forward-slash separators only. A backslash in a path literal is a parse error `loom/parse/invalid-path-separator`. Discovery roots and `--loom <path>` CLI values are OS-native paths (no syntactic restriction), but their resolved targets are compared in normalised forward-slash form for collision detection.

Add a corresponding rule to `spec_topics/discovery.md`: when two discovered files in the same source resolve to paths that differ only in case (e.g. `Plan.loom` and `plan.loom`), emit a load-time warning `loom/load/case-collision` naming both paths; the lexicographically-first path (case-sensitive byte comparison) wins. This makes behaviour identical on case-sensitive and case-insensitive filesystems: both report the warning, both register the same winner.

Cross-reference the new lexical-structure paragraph from `imports.md`, `invocation.md`, and the `tools:` bullet in `frontmatter.md` so the path-syntax rule is discoverable without grep.

Edge cases the implementer must handle:

- BOM consumption happens *before* offset accounting for diagnostics (line 1, column 1 starts after the BOM).
- Newline normalisation happens *before* span recording, so diagnostic line/column numbers refer to logical lines, not raw byte offsets.
- A path literal containing `\\` is unambiguously a parse error — there is no "did the author mean an escape" interpretation, since path literals are ordinary strings and `\\` already lowers to a single backslash via the standard string-escape table.
- The case-collision rule fires per discovery source, not across sources (cross-source priority already handles inter-source shadowing).

## Related Findings

- "Discovery directory tree example contradicts documented path" — same-cluster (both touch discovery; resolve independently)
- "Same-priority `.loom` filename collisions undefined" — same-cluster (both add missing collision rules to `discovery.md`; co-edit the same paragraph)
- "Slash-name validity from filename unspecified" — same-cluster (filename → identifier sanitisation belongs in the same `discovery.md` revision pass)
- "Discovery source failure modes partly unspecified" — same-cluster (per-source error-mode table belongs in the same `discovery.md` edit)
- "Dedent algorithm — no normative test vectors" — co-resolve (the CRLF normative test vector this finding requires is the same fixture set the dedent finding asks for; resolve together)

---

# `looms.binderModel` is an extension-owned settings key, not a Pi-recognised setting

**Source:** docs/reviews/spec-review/spec-20260504-144255.md
**Original heading:** `looms.binderModel` described as a "Pi-level setting" when it is extension-owned
**Kind:** codebase-grounding-broad, doc-alignment-broad, assumptions

## Finding

`spec_topics/binder.md`, `spec_topics/frontmatter.md`, and `spec_topics/implementation-notes.md` describe the binder-model fallback as "the Pi-level `looms.binderModel` setting" / "Pi setting `looms.binderModel`". This wording implies Pi itself recognises and surfaces the key — through `/settings`, `pi --help`, schema validation of `settings.json`, or an extension-facing accessor like `ctx.settings.get(...)`.

None of that is true. Pi's settings catalogue (`docs/settings.md` in `@mariozechner/pi-coding-agent`) enumerates a closed set of recognised keys (`defaultProvider`, `compaction.*`, `retry.*`, `extensions`, `skills`, `prompts`, `themes`, `packages`, etc.); `looms.binderModel` is not among them and neither is any `looms.*` namespace. The `ExtensionContext` interface documented in `docs/extensions.md` exposes no settings accessor — there is no `ctx.settings`, `ctx.config`, or equivalent. Consequently the loom extension must read `~/.pi/agent/settings.json` and `.pi/settings.json` itself via `PiFileSystem`, parse them, look up `looms.binderModel`, and apply Pi's project-overrides-global precedence (with deep-merge semantics for nested objects, per `docs/settings.md`).

This same situation already applies to the `looms` discovery array (see `spec_topics/discovery.md`), but the spec only acknowledges the extension-owned read in passing there and not at all for `binderModel`. The fix is twofold: (1) reword every "Pi setting" / "Pi-level setting" mention of `looms.binderModel` to credit the loom extension as owner; (2) specify the read mechanism — which files, in which order, with which merge semantics — once, in a place both `binder.md` and `discovery.md` can cite.

## Spec Documents

- `spec_topics/binder.md` — "Binder model" paragraph (edited)
- `spec_topics/frontmatter.md` — `binder_model:` row in the example block and the `binder_model` / `bind_context` / `bind_echo` description bullet (edited)
- `spec_topics/implementation-notes.md` — slash-command invocation bullet describing `binder_model` resolution (edited)
- `spec_topics/discovery.md` — Settings source description (edited; gains the canonical "how settings are read" subsection that the other pages cite)
- `docs/settings.md` (in `@mariozechner/pi-coding-agent`) — read-only authority for the merge semantics being mirrored
- `docs/extensions.md` (in `@mariozechner/pi-coding-agent`) — read-only confirmation that no `ctx.settings` accessor exists

## Plan Impact

**Phases:** Vertical V14, Vertical V16
...
Without the correction, an implementer reading `binder.md` is licensed to assume Pi exposes `looms.binderModel` through some SDK accessor and will spend cycles searching for it; once they discover it does not exist, they will improvise a read path. Two such improvisations are likely to diverge on (a) which files are consulted, (b) merge semantics for the nested `looms.*` object (top-level replace vs deep merge), and (c) what happens when the JSON is malformed. V16e's acceptance test "each resolution step" is unverifiable as written because the middle step is undefined.
...
Reword every reference to `looms.binderModel` to identify it as a loom-extension key in `settings.json`, and add one canonical "Settings reading" subsection that all loom-side settings keys share.

Concrete edits:

1. **`spec_topics/discovery.md`** — add a new `## Settings file reads` subsection (or a similarly named anchor) that states once, normatively:
   - The loom extension reads `~/.pi/agent/settings.json` (global) and `.pi/settings.json` (project) directly via the injected `FileSystem` seam — Pi does not expose these to extensions.
   - Both files are optional; missing or unreadable files are treated as `{}` with a single load-time diagnostic per file (not a fatal error).
   - JSON parse failure on either file produces a load-time diagnostic and that file is treated as `{}`; the other file is still consulted.
   - Project values override global values with **deep merge for nested objects, replace for arrays and scalars**, mirroring Pi's own merge rule documented in `@mariozechner/pi-coding-agent/docs/settings.md`.
   - The loom extension reads, at minimum, `looms` (array, per the existing discovery spec) and `looms.binderModel` (string model id).
   - Settings are read once at extension load and re-read on file-watcher events for those two paths.

2. **`spec_topics/binder.md` line 7** — replace "the Pi-level `looms.binderModel` setting" with "the loom-extension setting `looms.binderModel` in `settings.json` (read per [Settings file reads](./discovery.md#settings-file-reads); not a Pi-recognised setting)".

3. **`spec_topics/implementation-notes.md` line 23** — replace "Pi setting `looms.binderModel`" with "loom-extension setting `looms.binderModel` (see [Settings file reads](./discovery.md#settings-file-reads))".

4. **`spec_topics/frontmatter.md` line 11** — change the inline comment from `# model used to bind slash-command args to params (default: Pi setting)` to `# model used to bind slash-command args to params (default: looms.binderModel in settings.json)`.

5. **`plan_topics/v14-tool-calls.md` (V14n)** — broaden the leaf's scope from "looms array" to "settings file reads (looms array, plus the read mechanism reused by V16e for binder model)". Add a test that a malformed JSON file produces a diagnostic and is treated as `{}`, and that project values deep-merge over global values.

6. **`plan_topics/v16-binder.md` (V16e)** — change the Adds line from "Pi setting `looms.binderModel`" to "settings.json `looms.binderModel` (read via the V14n mechanism)" and add V14n to `Deps`.

Edge cases the implementer must watch:

- Tilde expansion in the global path (`~/.pi/agent/settings.json`) must use the same expansion rule as Pi (typically `$HOME` / `%USERPROFILE%`); document the source-of-truth function or replicate Pi's behaviour exactly.
- The `looms.binderModel` value is a free-form string; no validation against the model registry happens at settings-read time (validation is V16e's job, and is the subject of the related "Binder model resolution unspecified when no model is configured" finding).
- File-watcher invalidation of cached settings should be debounced; a partial write (editor-in-progress) should not crash the extension.

## Related Findings

- "Binder model resolution unspecified when no model is configured" — same-cluster (same field; this finding fixes the read mechanism, that one fixes what to do with a model that lacks structured-output capability)
- "`settings.json` `looms` array shape unspecified" — co-resolve (the proposed `## Settings file reads` subsection is the natural home for both the array shape and the read mechanism)
- "`pi.looms` package.json key is not Pi-recognized" — same-cluster (parallel codebase-grounding error: another loom-extension-owned key incorrectly framed as Pi-native)
- "`--loom` CLI flag is not a Pi built-in; repeatable string flag type undocumented" — same-cluster (third instance of the same misframing — extension-registered surface presented as Pi-native)

---

# `package.json` peer ranges are unbounded and runtime deps `ajv`, `ajv-formats`, `chokidar` are undeclared

**Source:** docs/reviews/spec-review/spec-20260504-144255.md
**Original heading:** `peerDependencies` use `*` ranges; `ajv`, `ajv-formats`, `chokidar` undeclared
**Kind:** codebase-grounding-broad

## Finding

The repository's `package.json` declares every Pi peer dependency as `"*"` (`@mariozechner/pi-coding-agent`, `@mariozechner/pi-agent-core`, `@mariozechner/pi-ai`, `@mariozechner/pi-tui`, `typebox`). `*` accepts any future version — including a hypothetical `1.0.0` with a fully reorganised SDK surface — which directly contradicts the contract framing in `spec_topics/pi-integration-contract.md` ("a small, named surface from `@mariozechner/pi-coding-agent`. Each item below is the V1 contract"). The currently installed Pi is `0.72.1`; the four `@mariozechner/*` packages move in lockstep with that release line, so the V1 contract is concretely a `0.72.x` (or `^0.72.1`) contract, but nothing in the repo or the spec records that.

Three runtime dependencies the spec mandates are absent from `dependencies`/`peerDependencies` entirely:

- **`ajv`** — `spec_topics/implementation-notes.md` requires "AJV v8 with `strict: false` and `allErrors: true`". Pi does not re-export AJV (its own `dependencies` block does not include it), so loom must own this dependency.
- **`ajv-formats`** — same section requires `ajv-formats` registered.
- **`chokidar`** — `spec_topics/pi-integration-contract.md` §3 names chokidar as the watcher implementation, and `plan_topics/v18-cancellation.md` V18f calls for `chokidar` directly. Pi does not ship it.

Separately, the `"typebox": "*"` entry is bare-name `typebox`, not `@sinclair/typebox`. That happens to align with Pi's own choice (Pi pins `"typebox": "^1.1.24"`), so the package name is correct, but the `*` range still applies.

The H1 leaf already promises `depcheck` clean as a ship gate — that gate fails today against the spec, because depcheck reports `ajv`, `ajv-formats`, and `chokidar` as missing the moment the runtime imports them.

## Spec Documents

- `spec_topics/pi-integration-contract.md` — preamble paragraph naming the SDK surface (edited)
- `spec_topics/implementation-notes.md` — Runtime, AJV configuration bullet (edited)
- `package.json` — `peerDependencies`, new `dependencies` (edited)
- `plan_topics/h1-scaffold.md` — Ships-when (`depcheck` gate) (read-only)
- `plan_topics/v18-cancellation.md` — V18f (chokidar) (read-only)

## Plan Impact

**Phases:** Horizontal, Vertical V4, Vertical V18

**Leaves (implementation order):**

- H1 — Repository scaffold and test framework — (modified)
- H4 — Pi extension shell — (blocked)
- V4a — AJV pipeline scaffold — (blocked)
- V18f — File watcher (chokidar) over discovery roots — (blocked)
- V18g — AJV cache invalidation on file change — (blocked)

## Consequence

**Severity:** correctness

Two implementers running `npm install` weeks apart against `*` peer ranges will end up with different transitive Pi versions; once Pi ships a non-backward-compatible release, builds will silently start failing in ways that look like loom bugs. The undeclared `ajv`/`ajv-formats`/`chokidar` make V4a and V18f literally unresolvable: the import will only succeed if some other package happens to hoist them into `node_modules`, which is a pure accident of npm topology and reproduces inconsistently across `pnpm`/`yarn`/`npm`/lockfile regenerations.
...
Edit `package.json` and the two spec topics together so the version contract is pinned, recorded, and depcheckable.

**`package.json` — peer pins.** Replace each `"*"` with a caret range against the currently installed Pi minor:
...
The four `@mariozechner/*` versions advance together; pin them to the same minor and bump in lockstep. `typebox` mirrors Pi's own pin (`^1.1.24`).

**`package.json` — runtime deps.** Add a `dependencies` block declaring what loom owns directly:
...
These belong in `dependencies` (not `peerDependencies`) because they are loom-internal — Pi does not interact with the AJV instance or the watcher. `chokidar` v4 (not v3) for Node 18+ ESM compatibility and to drop the `fsevents` optional native dep on macOS.

**`spec_topics/pi-integration-contract.md` — version anchor.** Add one sentence at the end of the preamble: "The V1 contract is anchored to `@mariozechner/pi-coding-agent ^0.72.1` (and the matching `pi-agent-core`/`pi-ai`/`pi-tui` minor); a Pi minor bump requires re-validating this contract before the loom `peerDependencies` range is widened."

**`spec_topics/implementation-notes.md` — pin the AJV major.** The existing bullet already says "AJV v8" — extend it: "AJV v8 (`ajv ^8`, `ajv-formats ^3`) declared in loom's own `dependencies`; not provided by Pi."

**`plan_topics/h1-scaffold.md`** stays unchanged — the `depcheck` ship-when already enforces this; it just needs the actual `package.json` to be correct so it can pass.

Edge cases the implementer must watch:

- When pi-loom is installed into a host project alongside Pi, npm's peer-dep auto-install (npm 7+) should pick up the same `@mariozechner/*` minor the host already resolved. If the host pins an older minor, npm will warn — that warning is the desired signal.
- `chokidar` is gated `optional` in `pi-integration-contract.md` §3 ("Optionally registers a file watcher"). Even so, declare it as a regular `dependency`, not `optionalDependencies` — the latter is for native-build-may-fail packages, not for runtime-feature opt-outs. The watcher being optional means the runtime tolerates `chokidar` failing to start, not that it tolerates the package being absent.
- `ajv-formats` v3 requires `ajv` v8.x; do not let the lockfile drift onto `ajv-formats` v2 (which targets `ajv` v7).

## Related Findings

- "`pi.looms` package.json key is not Pi-recognized" — same-cluster (also a `package.json` correctness issue; resolved independently)
- "AJV schema cache risks singleton pattern prohibited by CLAUDE.md" — same-cluster (touches AJV configuration but resolves independently)
- "Per-query AJV cache key is inconsistent with schema-subset lowering" — same-cluster (AJV-adjacent; independent fix)
