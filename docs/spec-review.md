...
# T20 - Frontmatter `model` — "loom's lifetime" conflicts with per-invocation inheritance
...
In `frontmatter.md`, the absent-`model:` behaviour is stated two ways an implementer can read as contradictory. The behaviour table's `model` row (default cell) says the loom "inherits the caller's model and pins it for the loom's lifetime"; the `model` prose bullet says "Once chosen, the model is fixed for the loom's lifetime; it does not re-resolve per query." Because the glossary's *loom (file unit)* is a persistent artefact spanning many invocations, "the loom's lifetime" admits two readings: a loaded-once pin (first invocation snapshots `ctx.model` for all later invocations) versus a per-invocation pin (each invocation re-inherits the live session model). When the Pi session model changes between two invocations of an absent-`model:` loom, the readings produce observably different model choices, and the divergence propagates into the subagent-mode `createAgentSession({ model })` requirement.

## Solution approach

Clarify both sites in `frontmatter.md` — the behaviour table `model` row's default cell and the `model` prose bullet — to pin the per-invocation reading: the inherited model is fixed only for the duration of one invocation, and a subsequent invocation of the same loom re-inherits `ctx.model` afresh, so a session-level model change between invocations is observed. This reading aligns with the already-pinned "no load-time registry call" and "at invocation time" phrasing.

## Solution constraints

- Out of scope: the subagent-mode `ctx.model === undefined` disposition, owned by T19.
