...
# T18b — Add per-mode operator-side null parentheticals to slash-invocation.md
...
The **prompt mode** and **subagent mode** bullets under *Once a loom is invoked* in `docs/spec_topics/slash-invocation.md` already state that the loom's `Ok` return value is not surfaced to the user (prompt mode) and only reaches the caller (subagent mode), but neither bullet records that no `loom-system-note` is emitted on the `Ok` outcome either. Sibling T18a installs the central success-side null-policy paragraph in the PIC **Runtime event channel** section authoritatively, but a reader of the slash-invocation per-mode bullets must cross-file-grep to confirm the absence of a terminal operator-side note is deliberate rather than an under-specified surface.

## Solution approach

Insert a short parenthetical into each of the **prompt mode** and **subagent mode** bullets under *Once a loom is invoked* in `docs/spec_topics/slash-invocation.md`. The parenthetical sits inside the existing sentence that already discusses the `Ok` outcome — for prompt mode the existing "The loom's final `Ok` return value is not surfaced to the user" sentence; for subagent mode the existing "When the loom finishes, only its return value reaches the caller" sentence — and states that no `loom-system-note` is emitted on the `Ok` outcome, with a bare forward-link to the PIC **Runtime event channel — Success-side null-policy** paragraph (already installed by T18a) for the normative source. Both parentheticals are short and parallel in shape. Composition (parenthetical wording, link text, parenthetical-vs-dash punctuation) is the implementer's choice within those bounds.

## Solution constraints

- The edit is a **parenthetical insertion into the existing sentence**, not a new sentence. Do not author a new free-standing sentence in either bullet.
- Do not modify the rest of the pre-existing per-mode framing in either bullet (the prompt-mode current-conversation-driving description and `Ok`-return-value-not-surfaced-to-user clause; the subagent-mode fresh-isolated-conversation description and return-value-only-reaches-caller clause); the parenthetical attaches to the existing `Ok`-related sentence without changing its other claims.
- Each parenthetical is restricted to (i) the null statement (`no loom-system-note is emitted on the Ok outcome`, or equivalent) and (ii) a forward-link to T18a's PIC paragraph. **Do NOT author per-mode operator-surface enumerations** — the success-visible surfaces and the failure-side surfaces are already enumerated in T18a's PIC paragraph; restating them per-mode in slash-invocation.md duplicates content. The 2026-05-21 parking record shows the original "sentence + surface enumeration" Solution approach opened a blocker on the subagent-as-top-level dispatch-topology case (citing the prompt-mode-only `Top-level Err in prompt mode` template in the subagent-mode bullet); the parenthetical-only shape avoids that attractor by carrying no surface-enumeration content.
- Do NOT name the `Top-level Err in prompt mode` template in the subagent-mode parenthetical. The failure-side surface for subagent-mode top-level dispatch is out of scope.
- The central success-side null-policy paragraph (T18a), the `spec.md` aggregator forward-link (T18c), and the V18q test clause (T18d) are owned elsewhere; all three have resolved.
