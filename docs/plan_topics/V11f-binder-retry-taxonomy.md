# `V11f` — Binder per-class retry budget and failure taxonomy
...
**Deps.** `V11f-T`, `V11e`, `V9j`, `V16a`, `V5e`, `H4b`

**Ships when.** `npm test` asserts the per-class retry caps (≤3 calls), the six verbatim templates, the depth-walk fast-fail `<ajv-summary>` rendering, and the most-recent-failure surfaced note.
