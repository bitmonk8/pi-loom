...
**Adds.** The two ambient-wrapping host seams kept together so the `PIC-12`/`PIC-20` `// allow-ambient:` allow-list contract is not split: `Clock` (`now`/`wallNow`/`setTimeout`/`clearTimeout` via the `WallClock` adapter) and `IdSource` (`newInvocationId()` UUID). The `FileSystem` seam is owned by [`V8b`](./V8b-filesystem-seam.md); the `FileWatcher`/`TokenEstimator` seams by [`V8e`](./V8e-watch-token-seams.md).
