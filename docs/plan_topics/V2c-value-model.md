...
**Adds.** The JS representation of loom values (native map, enum tag, internal `Result` tag), structural `==`/`!=` equality, and the inbound/outbound wire-name translation boundary (post-AJV loom-side rebuild with enum-tag reattach via the `V5f`-produced per-schema sidecar; outbound wire-name JSON).
