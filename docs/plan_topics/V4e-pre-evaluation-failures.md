...
- `ERR-7`: a synthetic watcher-time reload failure injected via `ReloadFailureInjector.injectReloadFailure` at the **watcher-time reload failure-injection seam** — owned by `V9b` (the `loom/runtime/registry-swap-failed` registry-swap arm and the `.loom`/`.warp` re-parse arm) and `V10c` (the settings-re-merge sub-arm) — exercising both arms, the re-parse/re-merge diagnostic arm and the `loom/runtime/registry-swap-failed` registry-swap arm, routes pre-eval to `loom-system-note` with `triggerTurn:false`, without standing up a live `V10c`/`V9b` watcher.
- `ERR-16`: the slash-load `params` arm of ceiling #4, cross-routed via CIO-1 / ceiling #3 no-retry, routes pre-eval.

**Deps.** `V4e-T`, `V9a`, `V6a`, `V11f`, `V10a`, `V16a`, `V9b`, `V10c` (the latter two own the **watcher-time reload failure-injection seam** (`ReloadFailureInjector.injectReloadFailure`) — registry-swap and `.loom`/`.warp` re-parse in `V9b`, settings re-merge in `V10c` — against which `ERR-7`'s synthetic failure is injected)
