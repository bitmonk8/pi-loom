...
- **Specific exception types only.** No `catch (e)`, `catch (e: unknown)`, `catch (e: any)`, or `catch (e: Error)` — bind to a specific subtype or let the exception propagate. The rethrow-on-mismatch pattern (`catch (e) { if (!(e instanceof X)) throw e; … }`) is also forbidden. An ESLint rule (`no-broad-catch`) wired in the scaffold leaf enforces this. A `catch (e: unknown)` clause is permitted at a Pi SDK boundary site only if the same line carries a `// allow-broad-catch: <REQ-ID> — <spec-page>` comment and the call site is enumerated in the lint allow-list; the V1.0 closing gate asserts every allow-list entry has a matching REQ-ID present in [`coverage-matrix.md`](coverage-matrix.md).
