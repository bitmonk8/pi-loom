...
**Spec.** `<page-1.md>`, `<page-2.md>` — the `spec_topics/*.md` files this leaf implements. An implementer MAY restrict their reading to these pages. The field MUST be closed under normative cross-link: closure runs from each listed page outbound and applies **transitively to a fixed point** — when a listed page cites a normative rule on another page, that other page is also listed, and the same obligation re-applies to *its* normative cross-links, and so on until no new page is reachable. A cross-link is excluded only when it is *narrative*, i.e. its target page's row in the prefix table carries the byte-exact `(no IDs — narrative)` cell per [`governance.md` GOV-3](../spec_topics/governance.md); closure follows normative cross-links and stops at narrative ones. See [`conventions.md`](./conventions.md) *Leaf format* for the authoritative statement of this rule. _(Use **Convention.** instead of **Spec.** for infrastructure leaves that operationalise [`conventions.md`](./conventions.md) rather than a spec page; cite the section by anchor.)_

**Adds.** _One sentence describing what the leaf introduces — the new module, the new parser surface, the new diagnostic, etc. `Adds.` is descriptive by default; a named mechanism binds only if it carries a cited REQ-ID or is a cross-leaf seam that some other leaf binds against (a leaf listing this leaf in its **Deps.** names the seam in its own **Adds.** or **Tests.** — *names the seam* meaning an unambiguous prose reference that resolves to one mechanism and cites this leaf's owning leaf-ID, not necessarily an exact identifier-token match) — see [`conventions.md`](./conventions.md) *Leaf format*._
