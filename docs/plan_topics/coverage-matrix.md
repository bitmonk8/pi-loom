...
| `pi-integration-contract/conversation-drive.md` — *No additional access channels* denial-surface MUST (un-anchored; GOV-22 residue) | `V14d` |
...
| `pi-integration-contract/registration-steps.md` — the registry-swap atomicity obligation `loom/runtime/registry-swap-failed` (step 5 build-aside-then-publish: on any rebuild-step throw the staging set is discarded, the prior `LoomRegistry` / validator-cache / registration-cache snapshot stays live, and a single `loom/runtime/registry-swap-failed` diagnostic fires); the step-5 structural-change watcher-note pair at `#structural-changes-no-unregister` (`MUST NOT` emit the `loom-system-note` when `added.length + removed.length === 0`; `MUST` emit it on a same-window rename of path P observed as `removed` of P then `added` of P, where the disjoint arrays sum to 2); and the step-2 `pi.getCommands()` snapshot read-only-by-convention `MUST` (the loom 1.0 collision check is a single forward pass that never mutates the returned array). Out of scope here: the step-3 drain-state-gated-dispatch `MUST` (closed under the `drain-state-contract.md` row at `V9m`) and the step-2 build-time SDK surface-inventory `MUST` for `SlashCommandInfo` / `SlashCommandSource` (closed under the `V18a` / `V18b` capability-inventory rows) (un-anchored; GOV-22 residue) | `V9b` |
