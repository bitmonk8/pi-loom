...
- Per-boundary routing *decision* — which of the five site-classes maps to which destination surface class: typed-query response → `ValidationError`, model-driven tool args → model feedback, code-driven tool args → `CodeToolError`, `params` and `invoke<T>` return → `InvokeInfraError`, slash-load `params` → ceiling-#3 cross-route — asserted in isolation against the harness. The actual wrapping of a depth-6 breach into each carrier is asserted at the site owner: `ValidationError` at `V13c`, `CodeToolError` at `V14e`, `InvokeInfraError` at `V15j`, the slash-load cross-route at `V4e`. The model-driven and slash-load `params` rows produce no loom-code `Err` (model feedback / load-time respectively) and are decision-only here.
- The loom-owned walk's depth computation, asserted in isolation against the harness: a depth-5 value passes the walk while a depth-6 value trips the ceiling, scalar and empty values count as depth 1, a non-empty value counts as `1 + max child`, and `anyOf` arms are not counted as levels.
