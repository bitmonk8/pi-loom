...
**Adds.** The lowering pass over the accepted JSON-Schema subset ([`V5d`](./V5d-reject-gate.md) owns the reject gate): the `__inline_<slug>` hoist, auto `$defs`/`$ref`, wire-name and named-enum-position sidecars, per-query `$defs` pruning, and the canonical-hash → 16-hex-slug recipe with the slug-collision byte-verify posture.
