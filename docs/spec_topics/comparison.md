...
| Callable set | Tools (model only) | Tools (model only) | Unified callable set, addressable from both the model (in queries) and code (`<name>(...)`); accepts Pi tools and `.loom` callables (subagent-mode loom files referenced by path in `tools:`) |
