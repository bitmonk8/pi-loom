...
- Production wiring constructs one `Checkpoint` per `loomAbort` (i.e., per loom invocation). For the `loop-iter` kind, `before(...)` releases the event loop for one macrotask turn before resolving — production wiring schedules that resolution through the injected [`Clock`](#clock--fakeclock-interface) seam's `setTimeout(fn, 0)` (the PIC-12 timer surface), not a bare global `setTimeout`, which would trip PIC-12's ambient-primitive ban — so that a Pi-dispatched abort (the macrotask that flips `loomAbort.signal.aborted`) can run before the interpreter reads the signal at the next loop-iteration checkpoint; a compute-bound loom body with no genuine `await` between iterations therefore stays cancellable rather than draining only the microtask queue. The single macrotask turn suffices only under the loom-side presupposition that Pi flips `loomAbort.signal.aborted` within a single macrotask, recorded as presupposition (vi) of [Pi-side slash-handler promise lifecycle (consumption posture)](./host-prerequisites.md#pi-slash-handler-promise-lifecycle-presupposition). For every other checkpoint kind, `before(...)` resolves on the microtask queue (an already-resolved promise), because those checkpoints precede real async I/O that already yields the event loop. For `invoke`, parent and child each own their own `Checkpoint` instance constructed from the same factory, mirroring the per-invocation `loomAbort` rule from [Cancellation](../cancellation.md); a parent's `before(...)` is not observed by the child and vice versa.
...
- The runtime's per-query compiled-validator cache is keyed by the schema slug of the lowered per-query schema document (per [Schema Subset — Canonical schema hash](../schema-subset.md#canonical-schema-hash)). On a cache hit the runtime MUST verify byte-equality of the new document's canonical-form bytes against the cached document's bytes before serving the cached `CompiledValidator`; on a byte-mismatch — a schema-slug collision per [Schema Subset — Schema-slug collision posture](../schema-subset.md#schema-slug-collision-posture) — it recompiles the new document and emits `loom/runtime/validator-cache-collision` instead of serving the wrong cached validator. The cache stores the canonical-form bytes alongside the compiled validator so the check is a byte comparison, not a re-serialisation.
...
Production wiring uses a `WallClock` adapter that delegates `now()` to `performance.now()`, `wallNow()` to `Date.now()`, and the timer methods to the global `setTimeout` / `clearTimeout`. Tests use a `FakeClock` whose `advance(ms: number)` synchronously fires every timer whose deadline has elapsed in deadline order (a 0-ms timer — such as the `loop-iter` checkpoint's macrotask yield scheduled by [PIC-10](#pic-10)'s production-wiring rule — fires under `advance(0)`, so a conformance test asserts the `loop-iter` yield property by driving `clock.advance(0)` between the abort-flip and the next signal-check); equal-deadline timers fire in registration order, `clearTimeout` is a no-op for already-fired handles, `now()` returns the fake's accumulated time and is *not* implicitly advanced, and `wallNow()` returns a constructor-injected epoch value that is likewise *not* implicitly advanced. `Clock.now()` is monotonic (forbids `Date.now()`-style NTP drift); the timing-primitive ban below already exempts the `WallClock` adapter, so its `wallNow()` delegation to `Date.now()` is permitted. One `Clock` instance is constructed per runtime instance — never as a module-level global — and lives for the lifetime of that runtime; parallel runtimes get independent clocks. The runtime MUST NOT call `Date.now`, `performance.now`, `Date.prototype.getTime`, or the global `setTimeout` / `clearTimeout` outside the `WallClock` adapter. A conforming implementation MUST carry a build-time check that rejects any **direct lexical reference** to those timing primitives outside the `WallClock` adapter; *how* the check is realised (a grep-test, a lint rule, or equivalent) is an implementation choice, but the presence of such a check and the references it rejects are the conformance criterion. The criterion covers direct lexical references only — indirect forms (aliased, destructured, computed, helper-wrapped, or re-exported reads) are out of scope for the spec criterion and are conceded to plan-side review.
...
interface FileSystem {
  ...
  readBytes(path: string): Promise<Uint8Array>;     // raw, pre-decode byte sequence; rejects with the same Node-style `.code` shape as `readText`
...
- <a id="pic-13-readbytes"></a>`readBytes` is the load-bearing surface for `.loom` / `.warp` source reads: source-file content is obtained as the raw, pre-decode byte sequence through this member so the runtime can UTF-8-validate it (per [Lexical — Encoding](../lexical.md)) before decoding — `readText` is not used for source files, because UTF-8 validation and first-invalid-byte offset recovery both require the undecoded bytes. It carries the same Node-style `.code` rejection mapping as `readText` (`ENOENT` for missing paths, `EACCES` / `EPERM` for permission failures; other I/O failures surface their underlying `.code` unchanged).
- `readdir` and `lstat` are the load-bearing surface for the *clean leaf-`ENOENT`* ancestor walk defined in [Directory Convention](../discovery.md) (the **Failure modes** paragraph and the bullet that defines *clean leaf-`ENOENT`*) and for the package-discovery walk in [Directory Convention — Package discovery](../discovery/package-and-settings.md#package-discovery). Rejected `.code` values map onto the discovery rules verbatim: an `ENOENT` chain that bottoms out cleanly is *missing*; an `EACCES` / `EPERM` / `ENOTDIR` (or any other code) anywhere on the chain is an unreadable-source failure. **Entry-name encoding guarantee:** the names `readdir` returns are the raw on-disk filename bytes interpreted as UTF-8 and MUST NOT be subjected to Unicode normalisation — an entry whose on-disk bytes spell `café` in NFD is returned in NFD, not folded to NFC (mirroring the encoding posture of [`readBytes`](#pic-13-readbytes) and [Lexical — Encoding](../lexical.md)). This guarantee binds every conforming seam implementation — both the production `PiFileSystem` adapter and the `FakeFileSystem` — and is what the byte-for-byte final-segment match in [Imports — IMP-1](../imports.md#imp-1) relies on to keep NFC and NFD spellings distinct.
- `realpath` is the load-bearing surface for the `realpath`-then-discovery-root-containment check in [Invocation — Resolution](../invocation.md) — both the load-time check and the invocation-time re-check — and for the single named function loom 1.0 factors that check behind per the [symlink-resolution-hardening seam](../invocation.md#loom-1-0-seam-symlink-resolution-hardening). The production `PiFileSystem` implementation delegates to Node's `fs.promises.realpath`; the `FakeFileSystem` implementation resolves the path against its constructor-injected in-memory symlink table, applying the same `.code` rejections (`ELOOP` on a cycle in that table, `ENOENT` on a missing component, `EACCES` / `EPERM` on an injected permission failure) so a symlink-farm fixture exercises the same rejection surface through the fake that production reaches through Node. **Case-canonicalisation guarantee:** on a case-insensitive filesystem, two input paths that differ only in component or leaf case but resolve to one directory entry MUST yield byte-identical `realpath` output. This guarantee binds every conforming seam implementation — both the production `PiFileSystem` adapter (bare delegation to `fs.promises.realpath` is insufficient because Node's `fs.promises.realpath` and `fs.promises.realpath.native` differ on leaf-case output, so the adapter must canonicalise leaf case to satisfy it) and the `FakeFileSystem` (whose in-memory symlink table must resolve case-variant inputs to one canonical path). **Transitive-symlink-resolution guarantee:** `realpath` MUST follow *every* symlink on the path — each intermediate path component as well as the final leaf, not only the leaf segment — so the returned canonical path contains no unresolved symlink anywhere along its length. This guarantee binds every conforming seam implementation — both the production `PiFileSystem` adapter (Node's `fs.promises.realpath` resolves the full component chain) and the `FakeFileSystem` (whose in-memory symlink table must resolve every symlinked component, not just the leaf). **Byte-stable canonical-output guarantee:** two input paths that name the same on-disk target MUST yield byte-identical `realpath` output (the case-canonicalisation guarantee above is the case-insensitive-filesystem instance of this more general property). This guarantee binds every conforming seam implementation — both the production `PiFileSystem` adapter and the `FakeFileSystem` — and is what the byte-for-byte discovery-root containment comparison in [Invocation — Resolution](../invocation.md) — the load-time check and the INV-1 invocation-time re-check — relies on to decide symlink escape.
...
<a id="idsource-interface"></a>

<a id="pic-20"></a> **PIC-20. `IdSource` seam.** Per-invocation identifier minting is provided by an `IdSource` service injected at construction time, modelled on the other DI seams in this section. The seam exists so that the `invocationId` values minted at [`ActiveInvocationRegistry` insertion](./active-invocation-registry.md#active-invocation-registry) — which surface on the insertion-ordered `<list>` rendering of `loom/runtime/reload-teardown-timeout` and on the `RuntimeEvent.invocation_id` wire field — are deterministically controllable by a test fake rather than drawn from a runtime-global UUID source, which the no-globals DI rule the registry section relies on for its `shutdownReason` channel forbids. The runtime MUST mint each `invocationId` — and each code-side `loom-direct:` `toolCallId` UUID body minted, fresh per code-side tool call, at the [Tool execution from loom code](./host-interfaces-core.md#tool-execution-from-loom-code) dispatch site — through this seam, and MUST NOT call `crypto.randomUUID()` (or any other ambient UUID source) outside the production adapter below. The interface member below is a non-normative reference; per [GOV-18 arm (a)](../governance/corpus-direction-and-scope.md#gov-18-arm-a) the exact member signature of this internal DI seam is not itself binding, while the usage constraint stated above remains normative.

```ts
interface IdSource {
  newInvocationId(): string;   // canonical lowercase 8-4-4-4-12 hex UUID
  newToolCallId(): string;     // canonical lowercase 8-4-4-4-12 hex UUID body for the code-side `loom-direct:` toolCallId
}
```

- Production wiring uses a `CryptoIdSource` adapter whose `newInvocationId()` and `newToolCallId()` each delegate to `crypto.randomUUID()` and return its result unchanged — the canonical lowercase 8-4-4-4-12 hex form required by the [§7 `<invocation-id>` / `<uuid>` placeholder convention](../diagnostics/placeholder-rendering-b.md#7-identifier--descriptor--and-closed-enum-placeholders).
- Tests use a `FakeIdSource` whose constructor takes the sequence of ids it should hand out; `newInvocationId()` and `newToolCallId()` each return the next configured id on each call, so a conformance test for the `loom/runtime/reload-teardown-timeout` `<list>` ordering drives the minted `invocationId` values at known boundaries instead of matching against nondeterministic UUIDs.
- One `IdSource` instance is constructed per runtime instance — never as a module-level global — and lives for the lifetime of that runtime; parallel runtimes get independent sources.
