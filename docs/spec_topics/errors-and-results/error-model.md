...
The trichotomy applies only once evaluation has begun. The complete loom 1.0.0 set of failures that occur *before* evaluation begins is the eight below; each surfaces per [Diagnostics](../diagnostics.md) on the `loom-system-note` channel, does not fire a new turn (`triggerTurn: false`) and produces no final value, and is not subject to cancellation. The note itself enters subsequent provider calls as a `user`-role transcript entry per Pi's `convertToLlm` transform — see [Pi Integration Contract — Delivery surface](../pi-integration-contract.md):
