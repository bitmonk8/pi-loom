...
<a id="bypass-cases"></a>
Two cases skip the binder call entirely; in both, no envelope schema is constructed at load time. The bypass decision is made at theta-load time from the static schema; there is no per-invocation branching.
...
1. **No-params bypass.** When `params:` is absent or `params: {}`, the theta takes no parameters and the binder does not run. Slash-argument overflow against a no-params theta is governed by [Slash-Command Invocation — No-params overflow](../slash-invocation.md#slsh-1); the binder's only contribution is to not run. `bind_echo`, `bind_context`, and `bind_model` on a no-params theta have nothing to bind — an explicit `bind_echo: true` is a load warning (`theta/load/bind-echo-without-params`) and produces no echo regardless; `bind_context` and `bind_model` are silently ignored (`bind_model` may be inherited from the project-wide `theta.binderModel` setting; `bind_context` has no project-wide carrier and defaults to `none`).
...
All other shapes — multiple fields, non-string types, defaults present, optional or nullable types — go through the binder. The no-params bypass check runs **before** the single-string bypass check, so a `params: {}` theta does not accidentally match the single-string branch.

## Binder envelope

The binder is asked to return one of three structured outputs (the schema is constructed dynamically by the runtime from the theta's `params:`):

- `{ kind: "ok", args: <typed params object> }` — successful extraction. The runtime fills any defaulted fields not present in `args`, then AJV-validates the merged result against the params schema (safety net for hallucinated field shapes), and starts the theta.
- `{ kind: "needs_info", message: string }` — the binder could not extract one or more required fields. The `message` is shown to the user as a system note; the theta does not run.
- `{ kind: "ambiguous", message: string, candidates: array<string> | null }` — multiple plausible bindings exist and the binder cannot pick one. The `message` is shown to the user as a system note; the theta does not run.

The envelope is runtime-internal; it is never a Theta-visible type and never appears in theta code. Authors only see the *consequences* of binding (theta runs, or system note appears).

> **theta 1.0 seam — binder refinement loop.** <a id="theta-1-0-seam-binder-refinement-loop"></a> The structural distinction between the two failure arms exists for the deferred binder refinement loop (cf. [Future Considerations — Binder refinement loop](../future-considerations/surface-extensions.md#binder-refinement-loop)), where only `needs_info` reopens for a clarifying turn while `ambiguous` still terminates. Three theta 1.0 carriers preserve the post-theta 1.0 migration; each is pinned individually below as its own normative obligation. The split is forward-compatible without the cost of either collapsing the arms now or rendering candidates the theta 1.0 templates do not require.

<a id="bndr-1"></a> **BNDR-1.** The [Binder envelope schema](#binder-envelope-schema-constructed-dynamically-from-params) MUST keep the three-arm `ok | needs_info | ambiguous` discriminator. Collapsing to two arms (e.g. folding `ambiguous` into `needs_info` because theta 1.0 surfaces both as terminating system notes) is a breaking change to the deferred loop.

<a id="bndr-2"></a> **BNDR-2.** The `ambiguous.candidates` field MUST remain in the envelope schema (`array<string> | null`; the binder may emit it; AJV accepts `null`). Dropping it from the schema is breaking. The theta 1.0 runtime MUST NOT surface it on the user-facing system note — the `ambiguous` row of the [failure-mode templates](./determinism-cancellation-failure.md#failure-mode-templates-normative) renders only `<model's message>`, per [System-note rendering](./defaulting-system-note-echo.md#system-note-rendering) rule 5.

<a id="bndr-3"></a> **BNDR-3.** The [failure-mode templates](./determinism-cancellation-failure.md#failure-mode-templates-normative) table MUST keep distinct `theta /<name>: argument binding needs more info — <model's message>` and `theta /<name>: ambiguous arguments — <model's message>` row prefixes. Collapsing the two failure-arm prefixes into a single shared phrase is breaking.

### Binder envelope schema (constructed dynamically from `params:`)

The runtime emits one envelope schema per theta at load time and reuses it for every binder call. The envelope is a discriminated union over `kind` and conforms to the [Schema Subset](../schema-subset.md); the runtime constructs it directly rather than via the lowering pass, but the shape is exactly what the lowering pass would produce for `schema BinderEnvelope = Ok | NeedsInfo | Ambiguous`.
...
1. **Theta identity line.** A line of the form `Theta: /<name>` MUST appear, where `<name>` is the bare slash command name (no leading `/`, matching the byte sequence hashed by [Determinism](./determinism-cancellation-failure.md#determinism) below). Exactly one such line per prompt.
2. **Description line.** When the theta's frontmatter `description:` is non-empty, a line of the form `Description: <description>` MUST appear. When `description:` is absent or empty, the line MUST be omitted entirely (no `Description:` token with an empty value).
...
6. **Session-context block.** When `bind_context: session` and the [Session-context truncation](./binder-model-and-context.md#session-context-truncation-bind_context-session) walk produced ≥1 included turn, the prompt MUST contain a Session-context block framed exactly as follows: an **opening line** that begins with the literal token `Recent session context` and ends with `:`, terminated by one `\n`; then the **body** — the included transcript rendered per [Compact-transcript format](./binder-model-and-context.md#compact-transcript-format-normative); then a **terminating blank line**. The body ends with the trailing `\n` of its last turn block, and the terminating blank line is exactly one further `\n` after that, so the block ends with the byte sequence `\n\n`. The terminating blank line is part of the Session-context block and MUST be emitted regardless of the block's position among the system-prompt blocks (first, middle, or last) and regardless of whether any further block follows — it is not a separator borrowed from a sibling block, and it is distinct from the inter-turn blank line that the [turn-delimiter rule](./binder-model-and-context.md#compact-transcript-format-normative) places between successive turn blocks inside the body. When the walk produced zero included turns (single oversized newest turn, empty session) or `bind_context: none`, the entire block — opening line, body, and terminating blank line — MUST be omitted (no header with empty body).
...
*Type display.* The per-field type rendering in item 4 MUST use the field's declared Theta type written in the surface syntax of [Type System](../type-system.md), not the JSON Schema lowering. Reference renderings (normative; conforming implementations MUST reproduce these exactly):

| Declared Theta type | Renders as |
| --- | --- |
| `string` | `string` |
| `integer` | `integer` |
| `boolean` | `boolean` |
...
| Field declaration (Theta source) | Per-field line |
| --- | --- |
