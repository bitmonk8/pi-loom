...
<a id="loom-flag-namespace"></a>The `--loom` CLI flag name is likewise coined by this extension; the loom extension registers it via `pi.registerFlag('loom', …)` (see [Registration steps](../pi-integration-contract/registration-steps.md) step 1). The flag-name overlap and shared flag-value-slot behaviours this coining relies on — that `pi.registerFlag('loom', …)` does not throw on a flag-name overlap and that loom's own `pi.getFlag('loom')` read still observes loom's registration — are recorded as host presuppositions at [Host prerequisites — `pi.registerFlag('loom', …)` flag-name overlap and shared flag-value-slot behaviour](../pi-integration-contract/host-prerequisites.md#loom-flag-namespace-presupposition). There is no `loom/load/...` code for flag-name collision in loom 1.0.
