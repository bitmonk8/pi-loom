...
2. **Hoists anonymous inline object schemas** (`{ field: T }` appearing in any type position) into `$defs` under a synthesised name `__inline_<slug>`, where `<slug>` is the schema slug of the *lowered* schema fragment, produced by the canonical schema hash recipe in [Canonical schema hash](#canonical-schema-hash) below. Two inline schemas resolve to one `$defs` entry only when their lowered JSON Schema fragments are byte-identical — the lowering pass establishes that identity by the byte-equality check the [Schema-slug collision posture](#schema-slug-collision-posture) below mandates on every `__inline_<slug>` slug match, so source-level cosmetic differences that lower to byte-identical fragments still collapse to one entry, while a slug match whose fragments are *not* byte-identical is a schema-slug collision that raises the load-time error `loom/load/schema-slug-collision` instead of silently merging the two schemas.
3. **Emits per type form:**
   ...
   - `Result<T, E>` is **not** a lowerable type form: it carries no emission rule and is rejected at parse time as `loom/parse/result-in-schema-position` ([Grammar Appendix — Type grammar](./grammar.md#type-grammar)) before the lowering pass runs, so no `Result` ever reaches this step.
   - **Array element order.** The lowered fragment carries array values in four positions, all emitted in loom-source declaration order so that source fragments which lower identically also serialise to identical bytes: object `required` lists wire names in declaring-field order (matching the `properties` order of the same Object form); `enum` lists wire values in source enumeration order; the `{ "type": [...] }` primitive-union form lists arms in source order, with `"null"` last whenever the union admits it; and `anyOf` lists variants in source order. The canonical schema hash depends on this ordering — see [Canonical schema hash](#canonical-schema-hash) step 2.
...
2. **Canonical form.** Serialise the fragment to a deterministic UTF-8 JSON byte sequence:
   ...
   - numeric literals embedded in the lowered fragment — the values appearing in `const` and `enum` positions — are serialised by the `integer` / `number` rendering algorithm pinned in [Binder — Echo policy](./binder/defaulting-system-note-echo.md#echo-policy) *Format rules* (its [`integer`](./binder/defaulting-system-note-echo.md#bndr-4) and [`number`](./binder/defaulting-system-note-echo.md#bndr-5) value-rendering rules together with the [normative reference-renderings table](./binder/defaulting-system-note-echo.md#bndr-6), which fix the otherwise-ambiguous cases — e.g. `1e21` serialises as `1000000000000000000000`, `42.0` as `42`, `-0` as `0`). Only that rendering algorithm is borrowed: the binder's 120-code-point line cap and `(default)` suffix are echo-formatter concerns and do not apply to the canonical-hash recipe;
   <!-- `default` and the integer-valued structural keywords (`minLength`, `maxItems`, …) are deliberately absent here: `default` is a runtime-filled surface-syntax feature the binder never lowers, and the structural keywords are rejected at parse time per the top-of-file enumeration, so no such keyword reaches the lowered fragment. -->
   - array elements are left in the order fixed by the [Lowering Algorithm](#lowering-algorithm) (its *Array element order* rule in step 3); the canonical form sorts object keys but never reorders array elements, just as key sorting is independent of emitted property order (see the closing note below);
...
<a id="schema-slug-collision-posture"></a>**Schema-slug collision posture.** Because the schema slug is a 64-bit truncation of SHA-256 (step 4), two distinct lowered fragments can in principle produce the same slug. Any slug-keyed cache or dedup table that uses a slug as a stand-in for fragment identity therefore verifies byte-equality of the candidate fragment's canonical-form bytes against the stored bytes on a slug match *before* treating the two as the same fragment; on a byte-mismatch the site surfaces a diagnostic and disambiguates — refusing to dedup, reuse, or serve the cached artefact — rather than silently aliasing two distinct fragments. Each such site stores the canonical-form bytes alongside the keyed artefact so the check is a byte comparison, not a re-serialisation. In loom 1.0 the three slug-keyed sites and their site-specific collision diagnostics are:

- the `__inline_<slug>` `$defs` dedup in [step 2 of the Lowering Algorithm](#lowering-algorithm) — a load-time site whose collision raises `loom/load/schema-slug-collision`;
- the per-query AJV compiled-validator cache (see [Pi Integration Contract — PIC-11](./pi-integration-contract/host-interfaces-services.md#pic-11)) — a runtime site whose collision raises `loom/runtime/validator-cache-collision`;
- the prompt-mode `pi.registerTool` registration cache (see [Pi Integration Contract — Tool-registration lifetime](./pi-integration-contract/tool-registration-lifetime.md#tool-registration-lifetime-and-visibility)) — a runtime site whose collision raises `loom/runtime/registration-cache-collision` and applies the counter-suffixed disambiguation pinned there.

The three diagnostics are kept distinct so a collision's originating cache is identifiable from the code alone.
