...
- **Draft**: JSON Schema Draft 2020-12. loom fixes this draft for the schemas it emits. *(Non-normative provenance.)* It was chosen at loom 1.0 authoring time as the draft both supported providers' strict modes accepted (Anthropic's strict `input_schema` required it; OpenAI Structured Outputs was compatible with it); the choice is loom-owned and is not a live claim about either provider's current requirements.
