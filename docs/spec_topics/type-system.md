...
Rules outside this list are deliberately **not** part of V1 compatibility: function-parameter contravariance, optional-field widening, excess-property tolerance on objects, and `number ⊑ integer` are all rejected. A future widening of the relation is non-breaking iff it only admits new pairs.
