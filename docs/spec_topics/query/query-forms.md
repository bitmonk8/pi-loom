...
- **Crossed (transparent):** parenthesisation `(...)`; the RHS of `let x: T = ...`; function / tool / `invoke` arguments matched to a typed parameter; the tail expression of an enclosing function or loom whose return type is declared; the operand of `return`; the branches of a ternary `cond ? a : b` *if and only if* the ternary itself has a sink; the elements of an array literal `[a, b]` *if and only if* the literal has a sink (binding annotation, parameter type, etc.); the operand of the postfix error-propagation `?` ([ERR-18](../errors-and-results/error-model.md#err-18)), whose unwrap of `Result<T, QueryError>` to `T` preserves the operand's type context, so the walk continues outward from `?` to any enclosing sink.
...
- `let x: ReviewScore = @\`...\`?` — the walk crosses the postfix `?` (transparent; [ERR-18](../errors-and-results/error-model.md#err-18) unwraps `Result<ReviewScore, QueryError>` to `ReviewScore`) and the `let` RHS, reaching the sink at the binding annotation. ✅
