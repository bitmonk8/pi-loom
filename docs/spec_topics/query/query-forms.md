...
- `let x: ReviewScore = @\`...\`?` — the walk crosses the postfix `?` (transparent; [ERR-18](../expressions.md#err-18) unwraps `Result<ReviewScore, QueryError>` to `ReviewScore`) and the `let` RHS, reaching the sink at the binding annotation. ✅
