...
rounds, or disable model-driven tool calls for a query. Configure the theta's
`tool_loop:` frontmatter block.

## Steps

1. Set `tool_loop.max_rounds` in frontmatter (default `25`; a non-negative
   integer, no upper bound). It applies to every query in the theta.
...
[`docs/examples/configure-tool-loop.theta`](../examples/configure-tool-loop.theta)
...
```
pi --theta docs/examples -p "/configure-tool-loop"
```
...
`Err(QueryError { kind: "tool_loop_exhausted", rounds: 4, ... })` and the theta
...
- Example `configure-tool-loop.theta` requested from `theta-docs-example-runner`.
