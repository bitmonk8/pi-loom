...
You want a parent loom to run a subagent-mode child in its own isolated
...
1. Write the child as a `mode: subagent` loom whose final value has the type you
...
2. From the parent, reach the child in one of two operationally equivalent ways:
   ...
   - call `invoke<Schema>("./child.loom", args)?` with an inline path and an
...
Child [`docs/examples/sentiment.loom`](../examples/sentiment.loom) returns a typed
`Sentiment`:
...
Parent [`docs/examples/typed-return.loom`](../examples/typed-return.loom) calls it
as a `.loom` callable and branches on the typed result:
...
Run it (the child is exercised through the parent — a `.warp`/child loom is not
...
The inline-path form `let s: Sentiment = invoke<Sentiment>("./sentiment.loom",
...
- `.loom`-callable registration and return-type flow — [Frontmatter](../reference/frontmatter.md).
...
- Spec: `docs/spec_topics/invocation.md` (typed return, final-value propagation,
  ...
  *Pi tool* vs *`.loom` callable*.
- Examples `typed-return.loom` + `sentiment.loom` requested from
