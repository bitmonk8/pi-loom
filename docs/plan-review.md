...
_126 findings retained, 3 false positives dropped, 0 persistent failures_
...
# Runtime event channel and always-log set are unimplemented by the plan

**Source:** docs/reviews/plan-review/plan-20260505-083349.md
**Original heading:** Runtime event channel / always-log set wholly absent from the plan
**Kind:** spec-coverage, validation

## Finding

The spec defines an operator-facing **runtime event channel** that is normative behaviour, not telemetry: a designated **always-log set** of `QueryError` failures (`transport`, `code_tool`, `model_tool`, `tool_loop_exhausted`, `invoke_failure`, every binder-failure row, and every panic) MUST emit a structured `RuntimeEvent` through the `loom-system-note` channel exactly once per occurrence, regardless of whether the author matched the `Err`, propagated it via `?`, or discarded it via `let _ =`. The full contract is in [`spec_topics/pi-integration-contract.md`](../../../spec_topics/pi-integration-contract.md) under "Runtime event channel" and is cross-referenced from [`spec_topics/query.md`](../../../spec_topics/query.md) (`let _` discard semantics), [`spec_topics/slash-invocation.md`](../../../spec_topics/slash-invocation.md) (`details.event` payload populated for every top-level `Err` row), and [`spec_topics/diagnostics.md`](../../../spec_topics/diagnostics.md) (the two disjoint `details` payload shapes).

A grep across `plan_topics/*.md` and `plan.md` for `RuntimeEvent`, "always-log", "runtime event channel", or `display: false` returns zero hits. No leaf in V18 (or anywhere else) covers: (a) `display: false` emission for handled / discarded / `?`-propagated failures in the always-log set; (b) the four explicitly excluded kinds (`validation`, `context_overflow`, `cancelled`, `invoke_callee_error`) producing zero events; (c) `?`-propagation chains emitting exactly once at the originating site (with the consumer-facing dedup key `(kind, query_site, message, occurrence-timestamp)`); (d) coercion follow-ups not emitting per-attempt events; (e) the `RuntimeEvent` payload-shape conformance (`kind`/`code`/`loom`/`query_site`/`message`/`attempts`/`tokens_used`); (f) panic-surface emission flowing through the `details: { diagnostics: [...] }` shape rather than `details: { event: ... }`; (g) the subagent-mode `display: false` rule on top-level cascades.

V18h adds the `loom-system-note` Pi message type and renderer but only documents the `display: true` user-facing path. V18i adds per-`kind` formatting for top-level `Err` in prompt mode but says nothing about the operator-facing channel. The coverage-matrix row "Pi Integration Contract" maps to `M, V12a, V14a–V14j, V18f, V18g, V18h` — none of which assert any always-log property. The V18o REQ-ID gate cannot catch the omission today because the REQ-ID assignment pass has not run; once it runs, every `pi-integration-contract.md` REQ-ID for this section will fail to map.

## Plan Documents

- `plan_topics/v18-cancellation.md` — V18h (edited), V18i (edited), `<new>` leaf inserted after V18i (edited)
- `plan_topics/coverage-matrix.md` — Pi Integration Contract row (edited)
- `plan_topics/v5-untyped-queries.md` — V5f Tests (edited; cross-link only)
- `plan_topics/v6-typed-queries.md` — V6a / V6b Tests (edited; cross-link only)
- `plan_topics/v14-tool-calls.md` — V14f–V14i Tests (edited; cross-link only)
- `plan_topics/v15-invoke.md` — V15l, V15m Tests (edited; cross-link only)
- `plan_topics/v16-binder.md` — V16e–V16p Tests (edited; cross-link only)
- `plan_topics/m-mvp.md` — system-note channel paragraph (read-only)
- `plan.md` — Vertical slice index (read-only)

## Spec Documents

- `spec_topics/pi-integration-contract.md` — "Runtime event channel" section (read-only)
- `spec_topics/query.md` — `let _` discard semantics (read-only)
- `spec_topics/slash-invocation.md` — `details.event` payload (read-only)
- `spec_topics/diagnostics.md` — disjoint `details` payload shapes (read-only)
- `spec_topics/future-considerations.md` — richer runtime-event telemetry (read-only)
...
- V18h — Custom Pi message type `loom-system-note` and renderer — (modified)
- V18i — Per-`kind` formatting for prompt-mode top-level `Err` — (modified)
- `<new>` — Runtime event channel and always-log emission — (added)
- V18k — Runtime panic: array index out of bounds — (modified)
- V18l — Runtime panic: indexed access on `null` / missing key — (modified)
- V18m — Panic routing: slash-command surface — (modified)
- V18n — Panic routing: `invoke` parent surface — (modified)

(Tests-bullet-only cross-link edits to V5f, V6a, V14f–V14i, V15l, V15m, V16e–V16p are editorial: each origin site adds a single Tests bullet asserting the event fires; the substantive verification lives in the new leaf.)

## Consequence

**Severity:** correctness

Without a leaf that owns the always-log channel, implementers will ship V18h's `loom-system-note` renderer with `display: true` only and never wire the operator-facing emission path. The result is silent loss of every `transport`, `code_tool`, `model_tool`, `tool_loop_exhausted`, `invoke_failure`, and binder-failure event that the author handled, discarded, or `?`-propagated — the channel that downstream replay tools, log scrapers, and `/tree` navigation depend on simply does not exist in V1. Two reasonable implementers will diverge on dedup, payload shape, and which sites emit; the eventual fix will be a breaking change to the `RuntimeEvent` schema after consumers have already started reading it.
...
- `plan_topics/v18-cancellation.md`: append a new leaf with the literal body:

  > `## V18p — Runtime event channel and always-log emission`
  >
...
- `plan_topics/v18-cancellation.md` V18h **Adds.**: append `The `details` field accepts either `{ diagnostics: Diagnostic[] }` or `{ event: RuntimeEvent }` per the disjoint-payload rule in the spec; the runtime-event population path is owned by V18p.`
- `plan_topics/v18-cancellation.md` V18i **Adds.**: append `The user-facing top-level `Err` note also populates `details: { event }` with the same `RuntimeEvent` payload V18p emits at the originating site; consumers deduplicate on `(kind, query_site, message, occurrence-timestamp)`.`
- `plan_topics/v18-cancellation.md` V18m and V18n **Tests.**: add a bullet asserting the panic emission arrives through `details: { diagnostics: [...] }` (per V18p), not through `details: { event: ... }`.
- `plan_topics/coverage-matrix.md`: add a new row `| [Pi Integration Contract — Runtime event channel](../spec_topics/pi-integration-contract.md) | V18p |` immediately below the existing Pi Integration Contract row.
- `plan_topics/v5-untyped-queries.md` V5f **Tests.**: append `cross-linked from V18p — `let _ = @`...`` of an always-log-set `Err` emits exactly one `display: false` runtime event at the discarding `let _` site.`
- `plan_topics/v6-typed-queries.md` V6a / V6b **Tests.**: append a bullet `cross-linked from V18p — `?`-propagation of an always-log-set `Err` emits exactly one event at the originating site, not at each rethrow.`
- Equivalent one-line Tests cross-links on V14f–V14i, V15l, V15m, V16e–V16p.
...
- "Discarded-query operator-facing observability and `let _ =` interaction — no plan leaf" — co-resolve (the `let _` runtime event is one of the always-log emission sites; both fixes land in the V18p leaf described above with a Tests cross-link from V5f)
- "`loom-system-note` delivery fallback chain unasserted" — same-cluster (touches the same channel; the fallback chain is orthogonal to the always-log emission contract)
...
# Panic routing has no defined path for unexpected interpreter exceptions

**Source:** docs/reviews/plan-review/plan-20260505-083349.md
**Original heading:** V18m / V18o: panic routing has no debug/verbose surface
**Kind:** risk

## Finding

`spec_topics/errors-and-results.md` declares the V1 panic-source list **closed**: six entries (`MatchError`, `index-out-of-bounds`, `null-member-access`, `null-index-access`, `missing-object-key`, `invoke-depth`), each with a normative message template registered in `diagnostics.md`. V18m and V18n test routing only for those six. The spec is silent on what happens when the *runtime itself* throws something outside the closed set — a `TypeError` from a host function, an internal invariant violation, an AJV-cache-corruption throw, an SDK reject the runtime did not anticipate. The only mention of "any unexpected exception thrown by the interpreter or the Pi SDK" is `pi-integration-contract.md` line 97, where it is one of several triggers for a subagent's `dispose()`-in-`finally` — not a routing path of its own.

The consequence is that the slash-command top-level wrap and the `invoke` parent surface have no defined behaviour for an unexpected throw. Two reasonable implementers will diverge: one lets the exception bubble out of the `pi.registerCommand` handler and tears down the user's turn (violating V18m's *"User session not torn down"* gate); a second wraps everything in a generic `try/catch` and silently swallows; a third coerces the throw into the nearest spec-listed source (e.g. fakes a `MatchError`-shaped note) and reports a misleading template. All three pass V18m's current Tests bullet, which only covers `MatchError`, OOB, and null-access.

The original framing also calls out the lack of an operator-facing crash record (stack trace, panic origin, debug log) for triage. The diagnostic carries the panic site via `file`/`range` already, so "no panic origin" is overstated; but for an *unexpected* exception there is no diagnostic site at all (no `loom/runtime/*` code applies), so the underlying triage gap is real for that branch specifically.

## Plan Documents

- `plan_topics/v18-cancellation.md` — V18m, V18n (edited)
- `plan_topics/coverage-matrix.md` — `Errors and Results — runtime panics` row (option-dependent — only edited if a new diagnostic code is introduced)
- `plan_topics/conventions.md` — Diagnostics-code discipline (read-only)

## Spec Documents

- `spec_topics/errors-and-results.md` — *Runtime panics* paragraph and the closed-list claim (option-dependent)
- `spec_topics/diagnostics.md` — `loom/runtime/*` registry (option-dependent — adds one row if a code is introduced)
- `spec_topics/pi-integration-contract.md` — *System notes* / *Subagent session lifecycle* (read-only — already names the unexpected-exception case in the disposal context)
- `spec_topics/slash-invocation.md` — top-level Err / panic surface (read-only)
- `spec_topics/future-considerations.md` — *Tooling deferrals* bucket (option-dependent — only edited if a crash-log file is deferred)
...
V18m's *"User session not torn down"* exit gate cannot fire for unexpected interpreter throws because no leaf exercises that branch and the spec gives no normative behaviour. Two implementers will produce divergent runtimes that both pass the current Tests bullets — one tears down the user turn, another silently swallows, a third mislabels — and the divergence is invisible to V18o's coverage gate because no REQ-ID owns the path.
...
- `plan_topics/v18-cancellation.md`, V18m, append to **Deps.**: `(no new dep — the same probe seam V18h uses).`
- `plan_topics/v18-cancellation.md`, V18n, append to **Tests.**: `An unexpected interpreter throw inside the invoked child (same probe) surfaces to the parent as \`Err(InvokeInfraError { kind: "invoke_failure", reason: "internal_error", message: <error.message>, ... })\`; exactly one \`loom/runtime/internal-error\` diagnostic is emitted at the child's site; the parent's \`?\` / \`match\` arm runs after the diagnostic.`
- `plan_topics/v18-cancellation.md`, V18m **Ships when.**: extend to read `Panics never tear down user session, including unexpected interpreter exceptions.`
- `plan_topics/coverage-matrix.md`: extend the `Errors and Results — runtime panics` row to include the new code, or add a sub-row for the new spec paragraph.
...
1. The new branch must not catch spec-listed panics. The catch site sits *outside* the existing panic-routing path; the closed-list panics still flow through their own routes and emit their own codes.
2. `InvokeInfraError.reason` gains a third arm (`"internal_error"`) alongside the existing `"panic"` and other reasons. The `kind` discriminator stays `"invoke_failure"`. Update the QueryError schema in `errors-and-results.md` accordingly.
3. The `error.stack` placed into `hint` is best-effort: stacks may be absent on some runtimes / minified builds. Render `"<no stack available>"` when `error.stack` is falsy rather than emitting an empty `hint`.
4. The synthetic-throw probe used by the new Tests must inject from inside a frame the runtime would normally trust (e.g. an AJV validator call site), not from a frame that already has its own catch — otherwise the test asserts the wrong thing.

## Related Findings

- "Closed diagnostic registry — many codes have no asserting plan leaf" — co-resolve (Option A adds one new code; that code's CI mapping should land in the same edit as the broader diagnostic-registry CI gate this finding proposes)
- "V2d cites wrong panic-routing leaf" — same-cluster (both touch V18m/V18n citation discipline; resolve independently)
...
# `ToolDefinition` `label` derivation, `Type.Unsafe<unknown>` `parameters` wrap, and in-`execute` AJV re-validation are unasserted

**Source:** docs/reviews/plan-review/plan-20260505-083349.md
**Original heading:** `ToolDefinition` `label` derivation and `Type.Unsafe<unknown>` wrap — no test leaf
**Kind:** spec-coverage

## Finding

`spec_topics/pi-integration-contract.md` is normative on three pieces of the per-loom `ToolDefinition` shape that `defineTool(...)` emits before the runtime hands it to `pi.registerTool` (prompt mode) or `createAgentSession`'s `customTools` (subagent mode):

1. **`label` derivation.** The loom file's basename, hyphens preserved, leading character upper-cased: `summarise.loom` → `"Summarise"`, `code-review.loom` → `"Code-Review"` (line 21). The synthesised typed-query one-shot tool's label is the literal string `"Loom typed-query response"` (same line, second sentence).
2. **`parameters` wrap.** A TypeBox `TSchema`, **not** a raw JSON Schema document. The lowered JSON Schema artefact must be wrapped via `Type.Unsafe<unknown>(loweredJsonSchema)` (line 23). Line 86 reiterates that loom-callee `customTools` entries use the same wrap.
3. **In-`execute` AJV re-validation.** Because the wrap is `Type.Unsafe<unknown>`, the `params` argument observed inside `execute` is effectively `unknown`; the adapter MUST AJV-validate `params` against the original lowered JSON Schema before forwarding (line 23, second half: "Pi's structural typing offers no guarantees here").

No plan leaf asserts any of these. V14e ("Pi tool wired into `@` queries as model-callable") covers per-mode wiring (`customTools` vs `pi.registerTool` + `setActiveTools`) and dedup count, but its Tests bullet says nothing about the shape of the constructed `ToolDefinition`. V6i ("AJV validation of typed query results") describes the synthesised `__loom_respond_<slug>` respond tool but never names its label or its `parameters` wrap. H4 establishes the registration cache and asserts that two `defineTool(...)` calls with the same lowered `parameters` hash dedupe, but takes the `defineTool(...)` shape as a given. The coverage-matrix row for `pi-integration-contract.md` lists `M, V12a, V14a–V14j, V18f, V18g, V18h` — none of which closes the three rules above.

The three rules are independent: an implementer who skips the basename label rule produces wrong UI labels, one who passes the raw JSON Schema directly to `pi.registerTool` produces a `ToolDefinition` Pi will misroute through its provider lowering, and one who omits the `execute`-side AJV re-validation will crash or silently forward malformed `params` whenever Pi's structural typing fails to catch a mis-typed call (which it never can, since the wrap is `unknown`).

## Plan Documents

- `plan_topics/v14-tool-calls.md` — V14e Tests (edited)
- `plan_topics/v6-typed-queries.md` — V6i Tests (edited)
- `plan_topics/h4-extension-shell.md` — Tests (read-only; the `defineTool(...)` dedup test is the natural cross-link target)
- `plan_topics/coverage-matrix.md` — `Pi Integration Contract` row (option-dependent; may need V6i added to the leaf list once it closes the synthesised-respond-tool shape rules)

## Spec Documents

- `spec_topics/pi-integration-contract.md` — Per-loom registration (lines 19–24) and customTools shape rule (line 86) (read-only)
- `spec_topics/implementation-notes.md` — Runtime, synthesised typed-query respond tool (read-only)
- `spec_topics/schema-subset.md` — Lowering Algorithm, the JSON Schema artefact that V14e/V6i wrap (read-only)
...
Two reasonable implementers will diverge on the label algorithm (e.g., title-case the whole basename, drop hyphens, or take the slash name post-`as` rename instead of the file basename), and a careful implementer will omit the `execute`-side AJV revalidation on the (incorrect) assumption that Pi's `ToolDefinition.parameters` typing already enforces it. The latter is a latent crash/UB site: malformed `params` from a misbehaving model or provider reaches the adapter as raw `unknown` and either throws on first field access or silently spawns the loom callee with garbage arguments.
...
Extend `plan_topics/v14-tool-calls.md` V14e's `Tests.` bullet (after the existing `prompt-mode active-set restoration fires on every exit path …` clause) with the loom-callee `ToolDefinition` shape assertions:

- `defineTool(...)` for a loom callee parsed from `summarise.loom` produces `label: "Summarise"`; for `code-review.loom`, `label: "Code-Review"`; the basename rule is hyphens-preserved, leading-character-upper-cased and is independent of any `as` rename applied to `tools:` entries.
- The constructed `ToolDefinition.parameters` is `Type.Unsafe<unknown>(loweredJsonSchema)` (asserted by walking the value: it is a TypeBox `TSchema` whose underlying JSON Schema strict-equals the lowering output, not the raw JSON Schema object).
- `ToolDefinition.execute` AJV-validates `params` against the original lowered JSON Schema before forwarding to the subagent spawn / `invoke<T>(...)` adapter; a synthesised malformed-`params` call (bypassing Pi's lowering, e.g. via a direct `customTools[i].execute(badParams, ctx)` invocation in a test) returns the spec's validation error path rather than throwing or silently forwarding.

Extend `plan_topics/v6-typed-queries.md` V6i's `Tests.` bullet with the synthesised respond-tool shape assertions:

- The synthesised `__loom_respond_<slug>` `ToolDefinition` has `label: "Loom typed-query response"` (literal string).
- Its `parameters` is `Type.Unsafe<unknown>(loweredJsonSchema)` over the lowered response schema (same wrap as loom callees).
- Its `execute` AJV-validates `params` against the original lowered response schema before unwrapping (this overlaps with the existing "invalid respond payload yields `validation` error" assertion; the new sub-bullet is that the validation site is `execute`, not a post-Pi structural-type check).

Cross-link the two: V6i's existing `Deps.` line (`V6c, V4`) gains `V14e` so the shape contract is established before V6i specialises it for the synthesised tool. Optionally add V6i to the `pi-integration-contract.md` row of `plan_topics/coverage-matrix.md` so the coverage gate sees a closing leaf for the per-loom registration rules.
...
- "V14c too large — three distinct concerns" — same-cluster (V14c also touches `defineTool`-style call sites but for code-side tool calls; independent of model-facing shape)
...
# Plan tests assert verbatim diagnostic messages, but no central normative source guarantees those messages exist

**Source:** docs/reviews/plan-review/plan-20260505-083349.md
**Original heading:** Plan tests cite "spec's exact wording" / "verbatim" without verifying spec owns each message string
**Kind:** assumptions

## Finding

A pattern recurs across at least nine leaves: a Tests bullet asserts "message text matches spec verbatim", "spec's exact-wording errors", "spec's exact message", "spec's exact examples", or "text matches spec verbatim". The leaves are M (`/<name> ignoring extra arguments` overflow note; `AbortError surfaces as a system note`), V1d (binding-case / schema-case mismatch), V2c (non-associativity), V2h (`array-element-type-mismatch`), V11b (`ambiguous-discriminator`), V11c (`missing-discriminator`), V11d (`by-on-object-schema`), V13h (`schema_repeat` follow-up turn text), V16c (envelope shape), V16i (`bind_echo` formatter examples), V16l/m/n/o/p (binder failure-mode system notes), V3c (no-params overflow note text). The implicit contract is that for every cited code or note, the spec carries a single canonical message string the test can assert string-equality against.

The spec does not in fact carry that contract uniformly. `spec_topics/diagnostics.md`'s registry tables for `loom/parse/*`, `loom/load/*`, and `loom/type/*` codes have columns *Code, Sev, Phase, Trigger, Spec rule, Hint* — no *Message* column. (The `loom/runtime/*` table at line 132 does have *Message template*; that subset is well-grounded.) Canonical messages for parse / load / type codes live, when they exist, scattered across spec topic pages in inline prose: `lexical.md` carries the binding-case and schema-case messages; `schemas.md` carries the discriminator messages; `grammar.md` carries `by-on-object-schema`; `slash-invocation.md` carries the no-params overflow note; `binder.md` lines 187–193 carry the binder failure-mode templates with explicit "Renderers MUST emit the surrounding template text verbatim". For other cited codes the spec carries no canonical message at all: `loom/parse/mixed-plus-operands`, `loom/parse/array-element-type-mismatch`, `loom/parse/reserved-keyword-as-identifier`, and `loom/parse/doc-comment-misplaced` have only a Trigger description and an optional Hint in the registry; no spec page renders the message text. V13h's `schema_repeat` follow-up turn is described methodologically ("re-states the expected schema without quoting a specific error" in `frontmatter.md` line 135) but no template string is given. V16i's "spec's exact examples" is contradicted by `binder.md`'s own framing of the example as illustrative-not-fixture (already noted by sibling finding `V16i "spec's exact examples" but spec disclaims them`).

The result is a class of tests that are unimplementable without per-leaf detective work — implementer A copies a sentence from `lexical.md` prose, implementer B copies a slightly different rendering, and the test assertion silently encodes a different normative phrasing than the next leaf. For codes with no canonical message in spec at all, the test simply cannot be written until the spec is amended; the leaf nevertheless ships under the current Ships-when wording, which means the assertion gets weakened in implementation without anyone noticing.

## Plan Documents

- `plan_topics/m-mvp.md` — Tests bullets ("text matches spec verbatim"; "AbortError surfaces as a system note") (edited)
- `plan_topics/v1-lexer.md` — V1d Tests ("spec's exact-wording errors") (edited)
- `plan_topics/v2-expressions.md` — V2c Tests ("non-associativity diagnostic verbatim from spec"); V2h Tests ("element-type-mismatch in literal rejected with spec's exact message"); V2f Tests ("the hint text matches spec") (edited)
- `plan_topics/v3-frontmatter.md` — V3c Tests ("text matches spec verbatim") (edited)
- `plan_topics/v11-discriminated-unions.md` — V11b, V11c, V11d Tests ("message text matches spec verbatim") (edited)
- `plan_topics/v13-wire-names.md` — V13h Tests ("Follow-up turn text matches spec") (edited)
- `plan_topics/v16-binder.md` — V16c, V16i, V16l, V16m, V16n, V16o, V16p Tests ("matches spec verbatim", "system-note text matches spec") (edited)
- `plan_topics/conventions.md` — add a project-wide rule that "verbatim" / "exact wording" assertions must cite a spec line range or quote the canonical string inline (edited)
- `plan_topics/coverage-matrix.md` — read-only (the V18o gate already cross-checks codes; messages are not in scope) (read-only)

## Spec Documents

- `spec_topics/diagnostics.md` — add a *Message* column to the `loom/parse/*` / `loom/load/*` / `loom/type/*` tables, populated for every code from the existing inline prose where present and freshly authored where not (edited)
- `spec_topics/expressions.md` — supply canonical messages for `loom/parse/mixed-plus-operands` and `loom/parse/array-element-type-mismatch` if they remain absent from the registry (option-dependent)
- `spec_topics/lexical.md` — supply a canonical message for `loom/parse/reserved-keyword-as-identifier` (option-dependent)
- `spec_topics/grammar.md` — supply a canonical message for `loom/parse/doc-comment-misplaced` (option-dependent)
- `spec_topics/query.md` / `spec_topics/frontmatter.md` — supply a canonical follow-up-turn template for `coercion.methodology: schema_repeat` (option-dependent; required for V13h)
- `spec_topics/binder.md` — read-only for the failure-modes table (already normative); option-dependent for the `bind_echo` formatter (sibling finding handles whether a fixture set or per-rule property assertions replace "spec's exact examples") (option-dependent)
- `spec_topics/schemas.md`, `spec_topics/slash-invocation.md` — read-only (already carry verbatim messages for the cited codes) (read-only)

## Affected Leaves

**Phases:** MVP, Vertical V1, Vertical V2, Vertical V3, Vertical V11, Vertical V13, Vertical V16
...
- V1d — Identifier case rule and reserved keywords — (modified)
- V2c — Arithmetic, comparison, logical, ternary, parens — (modified)
- V2f — Truthiness rule — (modified)
- V2h — Array stdlib and array literals — (modified)
- V3c — Bypass binder (no-params and single-string forms) — (modified)
- V11b — Ambiguous-candidate diagnostic — (modified)
- V11c — Missing-discriminator diagnostic — (modified)
- V11d — Explicit `by <field>` form — (modified)
- V13h — Coercion methodology: `schema_repeat` — (modified; blocked on spec amendment unless test is weakened)
- V16c — Binder envelope schema construction — (modified)
- V16i — `bind_echo` formatter — (modified; co-resolves with sibling V16i finding)
...
Two reasonable implementers will produce different message strings for the same code. The first leaf to test a code freezes whatever phrasing its author copied from spec prose; later leaves either copy the same string (correct by coincidence) or assert a paraphrase that drifts. For codes with no canonical message in spec (`mixed-plus-operands`, `array-element-type-mismatch`, `reserved-keyword-as-identifier`, `doc-comment-misplaced`, `schema_repeat` follow-up), the leaf cannot be implemented as written and the assertion will be silently weakened or deleted, which the V18o coverage gate does not catch (the gate matches on REQ-IDs / codes, not on message-string fidelity).
...
Make `spec_topics/diagnostics.md` the single source of truth for every author-visible message string. Concretely:

1. In `spec_topics/diagnostics.md`, add a *Message* column to the `loom/lex/*` / `loom/parse/*` table (heading currently at line ≈72) and to the `loom/load/*` table (line ≈156). The column carries the rendered message with `<…>`-style placeholders for interpolated content, exactly mirroring the *Message template* column already present in the `loom/runtime/*` table (line ≈174). For each code, populate from the inline prose in the spec rule's home page when one exists; freshly author a one-line template for codes that currently have none. Add a sentence under "Code registry rules (normative)" stating: *"The Message column is normative; renderers MUST emit it character-for-character with placeholders interpolated. Wording changes are spec-versioned breaking changes."*

2. In `plan_topics/conventions.md`, append a bullet under the per-leaf cross-cutting rules: *"Tests asserting a diagnostic's rendered message MUST cite the diagnostic code; the message string is sourced from the *Message* column of the diagnostics registry. Phrasings like 'matches spec verbatim' without a code anchor are insufficient."*

3. In each affected leaf, replace every *"matches spec verbatim"* / *"spec's exact-wording"* / *"spec's exact message"* phrasing in the **Tests.** field with *"emits `<code>` with the registry's *Message* template"*. Specifically:
   - `plan_topics/m-mvp.md` — Tests, the `/hello extra text` bullet: replace *"text matches spec verbatim"* with *"emits a `loom-system-note` whose `content` matches the `slash-invocation.md` 'No-params overflow' template (`loom /<name>: ignoring extra arguments — this loom takes no parameters`)"*. The "AbortError surfaces as a system note" bullet is handled by sibling finding `M's "AbortError" system-note path not defined in spec` — co-resolve.
   - `plan_topics/v1-lexer.md` — V1d Tests: replace *"the spec's exact-wording errors"* with *"emits `loom/parse/binding-case-mismatch` and `loom/parse/schema-case-mismatch` whose messages match the diagnostics registry templates"*.
   - `plan_topics/v2-expressions.md` — V2c Tests: replace *"non-associativity diagnostic verbatim from spec"* with *"`a < b < c` and `a == b == c` each emit `loom/parse/comparison-chaining` with the registry template"*. V2h Tests: replace *"spec's exact message"* with *"`loom/parse/array-element-type-mismatch` with the registry template, naming the offending element"*. V2f Tests: replace *"the hint text matches spec"* with *"the hint text matches the diagnostics registry's *Hint* column"*.
   - `plan_topics/v3-frontmatter.md` — V3c Tests, the trailing-text bullet: replace *"(text matches spec verbatim)"* with *"(content matches the `slash-invocation.md` 'No-params overflow' template)"*.
   - `plan_topics/v11-discriminated-unions.md` — V11b, V11c, V11d Tests: replace each *"message text matches spec verbatim"* and *"diagnostic message matches the spec verbatim"* with *"message matches the diagnostics registry template for `<code>`"* (codes: `loom/parse/ambiguous-discriminator`, `loom/parse/missing-discriminator`, `loom/parse/by-on-object-schema`).
   - `plan_topics/v13-wire-names.md` — V13h Tests: replace *"Follow-up turn text matches spec"* with *"Follow-up turn text matches the `schema_repeat` template in `query.md` (Schema-validation coercion)"*. **Blocked** until that template is added to `query.md` or `frontmatter.md`; until then, the leaf cannot be tested as written.
   - `plan_topics/v16-binder.md` — V16c Tests: replace *"Envelope shape matches spec verbatim"* with *"Envelope shape matches `binder.md` Binder envelope schema (lines ≈45–95)"*. V16l, V16m, V16n, V16o, V16p Tests: replace each *"system-note text matches spec"* with *"system-note `content` matches the `binder.md` Failure-modes table row for `<row label>`"* (rows already explicitly normative per `binder.md` line ≈182). V16i is co-resolved with the sibling finding — the *"spec's exact examples"* phrasing is replaced per that finding's recommendation, not this one.

The fix is mechanical once step 1 lands — every leaf points at a single column instead of guessing. Edge case: codes whose message currently has no canonical text in spec (enumerated in step 1) MUST be authored as part of the spec edit, not deferred — leaving them blank reproduces the original problem inside the registry.

## Related Findings

- "\"Spec's exact-wording errors\" without inline quote" — co-resolve (same edit fixes both: V1d's bullet is rewritten per the recommendation above)
- "V16i \"spec's exact examples\" but spec disclaims them" — same-cluster (V16i is the one leaf where the recommendation defers to the sibling finding's per-rule property-assertion approach rather than a single message string)
- "V15n invocation-cycle message format not pinned to spec template" — same-cluster (parallel symptom: a leaf asserting a path notation but not the leading template text; co-resolves under the convention added in step 2)
- "V18f structural-change note text unspecified" — same-cluster (a code whose canonical message is missing from spec entirely; would be authored as part of step 1's "freshly authored where not" sweep)
- "V18i per-kind formatter: catch-all row, `last_tool_name=null`, chain recursion unasserted" — same-cluster (per-kind formatter rows in `slash-invocation.md` are already normative; the related finding extends assertion coverage rather than adding new messages)
- "Closed diagnostic registry — many codes have no asserting plan leaf" — decision-dependency (that finding adds leaves for missing codes; those new leaves must use the convention added in step 2 from inception)
...
# `ModelToolError` has no closing leaf — V5g forward-references a leaf that does not exist

**Source:** docs/reviews/plan-review/plan-20260505-083349.md
**Original heading:** `ModelToolError` forward-referenced in plan but no leaf implements it
**Kind:** spec-consistency, spec-naming

## Finding

`plan_topics/v5-untyped-queries.md` V5g declares the `QueryError` union with `transport`, `context_overflow`, `cancelled` variants only and lists where every other variant lands: "(`validation` lands V6i; `code_tool` V14f-i; `model_tool` V14; `invoke_failure`/`invoke_callee_error` V15l-m.)". Every other forward reference resolves: V6i exists and adds `validation`; V14f–V14i exist and add the four `CodeToolError` causes; V15l/V15m exist and add `InvokeInfraError`/`InvokeCalleeError`. The `model_tool` reference does not resolve. `grep -n 'ModelToolError\|model_tool' plan_topics/v14-tool-calls.md` returns no matches; no V14 leaf — and no leaf anywhere in the plan — adds `ModelToolError` to the runtime union, specifies its detection inside the query tool-loop, populates its `tool_call_id` / `raw_response` fields, or supplies test vectors.

The coverage matrix corroborates the gap. `plan_topics/coverage-matrix.md` row "[Query — failure modes (`QueryError`)]" lists `V5g, V6i, V6k, V6j, V14f–V14i, V15l, V15m`. The seven QueryError variants the spec defines are `ValidationError` (V6i), `TransportError` (V5g), `CancelledError` (V5g), `ContextOverflowError` (V5g), `ToolLoopExhaustedError` (V6k), `CodeToolError` (V14f–V14i), `InvokeInfraError` (V15l), `InvokeCalleeError` (V15m), and `ModelToolError` — eight named, no closing leaf for the ninth. `spec_topics/errors-and-results.md` lines 89–94 declare `ModelToolError` as a first-class arm of the `QueryError` discriminated union; `spec_topics/errors-and-results.md` lines 138–144 declare its full shape (`message`, `tool_name`, `tool_call_id`, `raw_response`); `spec_topics/query.md` lines 192, 211–213, 229, 234 specify when it fires (model-invoked tools inside a query's tool-call loop, including coercion follow-ups) and require it to propagate as the proximate cause of any non-validation failure during a follow-up turn.

The variant is also load-bearing for two adjacent areas: the spec's "always-log" set in `spec_topics/pi-integration-contract.md` lines 121–125 names `model_tool` alongside `transport`, `code_tool`, and `tool_loop_exhausted`, so any runtime-event leaf has nothing to attach to until `ModelToolError` exists; and `spec_topics/slash-invocation.md` line 32 includes a `model_tool` row in the slash-invocation system-note table that the binder leaves cannot exhaustively render until the variant is in the union.

## Plan Documents

- `plan_topics/v5-untyped-queries.md` — V5g (edited)
- `plan_topics/v14-tool-calls.md` — new leaf inserted between V14e and V14f (edited)
- `plan_topics/coverage-matrix.md` — row "Query — failure modes (`QueryError`)" (edited)
- `plan_topics/v6-typed-queries.md` — V6i (read-only; the new leaf cross-links to it for free-phase coverage)
- `plan_topics/v15-invoke.md` — V15l, V15m (read-only; structural template for a single-leaf variant addition)

## Spec Documents

- `spec_topics/errors-and-results.md` — `ModelToolError` declaration (read-only)
- `spec_topics/query.md` — Tool-loop failures, Non-validation failures during coercion (read-only)
- `spec_topics/pi-integration-contract.md` — Always-log set (read-only)
...
- V5g — `QueryError` union — initial variants — (modified)
- `<new>` (placed between V14e and V14f) — `ModelToolError` variant: model-loop tool-call failure — (added)

## Consequence

**Severity:** blocking

V5g's Deps parenthetical is unfulfillable, so the V5g implementer either invents a `ModelToolError` shape on the spot (forking from spec) or ships a `QueryError` union that silently omits a spec-mandated variant. Downstream coverage breaks in cascade: the coverage matrix row passes vacuously because all the IDs it lists do exist; the V18o REQ-ID gate cannot detect the gap because no leaf claims the relevant spec text; the runtime-event always-log leaf (a separate finding in this review) has nothing to attach `model_tool` emission to; the binder slash-invocation system-note table cannot render its `model_tool` row exhaustively. Any loom that issues a query whose model invokes a failing tool returns an error of an undefined kind.
...
Insert one new leaf into `plan_topics/v14-tool-calls.md` between V14e and V14f. Use placeholder ID `V14<new>` (the implementer or the next renumbering pass picks the real ID and shifts V14f–V14q if needed). The leaf body:

```
## V14<new> — `ModelToolError` variant: model-loop tool-call failure
...
- In `plan_topics/v5-untyped-queries.md` V5g `Adds.` bullet, replace the literal substring `model_tool` V14;` with `model_tool` V14<new>;` (preserving surrounding text).
- In `plan_topics/coverage-matrix.md`, edit the row whose left column is `[Query — failure modes (`QueryError`)]`: replace the right column `V5g, V6i, V6k, V6j, V14f–V14i, V15l, V15m` with `V5g, V6i, V6k, V6j, V14<new>, V14f–V14i, V15l, V15m` (leaf appears in implementation order between V6j and the V14f-range).

PascalCase normalisation of the variant name across plan prose (e.g. V5g, V6i, V14f–V14i still write `model_tool` and `code_tool` rather than the spec's `ModelToolError`/`CodeToolError`) is the subject of a separate finding in this review and should be handled there as a uniform sweep, not piecemeal here. The new leaf above already uses PascalCase in its title and `Adds.` so it is consistent with that future sweep.

Edge cases the implementer must watch:

- The detection rule must distinguish a `cancelled` cause from `CancelledError`: when the abort fires mid model-tool call, the proximate cause is the tool failure, so the spec resolves it as `ModelToolError` with a cancellation message — not as `CancelledError`. The test vector for cancellation-during-tool must assert the variant is `ModelToolError`.
- The coercion-follow-up test depends on V13g, which is itself not yet implemented when V14 lands. The Deps line names V13g so the leaf cannot ship before coercion is in place; that ordering is correct (the leaf cannot prove its full spec coverage without the follow-up machinery), but the implementer should not weaken the test set to remove the dependency.
...
# H1 ships engineering hygiene with no declared traceability source

**Source:** docs/reviews/plan-review/plan-20260505-083349.md
**Original heading:** H1 scaffolds engineering hygiene without spec basis
**Kind:** spec-fidelity

## Finding

`plan_topics/h1-scaffold.md` adds Prettier, a GitHub Actions workflow, `depcheck`, ESLint rules (`no-floating-promises`, `no-globals`, `no-broad-catch`), a per-directory `__present` re-export test, and a `no-static-state.test.ts` greppable assertion. None of these items appear anywhere under `spec_topics/`. They derive instead from the cross-cutting rules in `plan_topics/conventions.md` ("No globals, statics, singletons. … Architectural test in H1 enforces"; "Specific exception types only. … ESLint rule wired in H1") and from the project-level guidance in `CLAUDE.md`.

The leaf format mandated by `conventions.md` requires a `**Spec.**` field as the first bullet, with leaves whose source is purely narrative being the only documented exception. H1 omits the field outright. This leaves a future reader (and the V18o coverage-matrix gate's reviewer) unable to tell at a glance whether the items are unsourced (a defect), convention-derived (legitimate), or simply forgotten.

A secondary effect: the `Ships when` gate includes `depcheck` clean. `depcheck` is not mentioned in the spec, in conventions, or in any other leaf, and no leaf installs or configures it. The gate is therefore a load-bearing dependency on tooling whose presence and configuration are not themselves planned.

## Plan Documents

- `plan_topics/h1-scaffold.md` — the entire leaf (edited)
- `plan_topics/conventions.md` — Leaf format section; Cross-cutting rules section (edited)
- `plan_topics/coverage-matrix.md` — preamble paragraph about the V18o gate's directionality (read-only)
- `plan_topics/v18-cancellation.md` — V18o (read-only; confirms the gate is spec→matrix, so unsourced H1 items do not trip it)
...
The plan still ships if H1 is implemented as written; the V18o gate fires on spec REQ-IDs that lack a matrix mapping, not on plan items that lack a spec citation, so unsourced hygiene items do not break CI. The cost is interpretive: a reader auditing leaf-to-spec traceability cannot distinguish "convention-derived, legitimately fieldless" from "spec citation forgotten." The same shape recurs in H2, H3, and H4, all of which omit `**Spec.**` and all of which are flagged by sibling findings — so the fix wants to be uniform.
...
1. In `plan_topics/conventions.md`, amend the **Leaf format** section to declare an alternative first bullet for horizontal-phase leaves. Replace the current "**Spec.** Page(s) under [`../spec_topics/`](../spec_topics/) the leaf implements." line with a two-form rule: "**Spec.** Page(s) under [`../spec_topics/`](../spec_topics/) the leaf implements, *or* **Convention.** Section(s) of [`conventions.md`](conventions.md) the leaf operationalises (used by horizontal phases H1–H4, which derive from project-level conventions rather than spec rules)."

2. In `plan_topics/h1-scaffold.md`, prepend a new first bullet:
...
3. In the same file, strike `depcheck clean` from the **Ships when** gate. The leaf does not install or configure `depcheck`, no other leaf does either, and dependency-hygiene tooling is not a spec or convention requirement. The remaining gate (`npm run typecheck && npm run lint && npm test` green) is sufficient to demonstrate the toolchain runs.

   If `depcheck` is wanted, it belongs in a separate convention bullet inside `conventions.md` and a configuration step inside H1's body — not as a silent Ships-when dependency.

Edge cases the implementer must watch:

- The same `**Convention.**` substitution must be applied to H2, H3, and H4 leaves for consistency; sibling findings ("H2 missing mandatory Spec field", "H3 missing mandatory Spec field", "H4 missing mandatory Spec field") cover those leaves and should be resolved with the same edit shape.
- The V18o gate text in `plan_topics/v18-cancellation.md` already specifies the spec→matrix direction; no edit is needed there. Confirm before closing.
- The `**Convention.**` bullet for H1 should NOT cite a REQ-ID (REQ-IDs come from spec pages only, per the REQ-ID discipline rule in `conventions.md`).

## Related Findings

- "H1 missing mandatory Spec field" — co-resolve (same edit closes both: adding the `**Convention.**` field to H1 simultaneously satisfies the missing-Spec-field complaint)
- "H2 missing mandatory Spec field" — same-cluster (same fix shape applies; H2's DI seams are convention-derived)
- "H4 missing mandatory Spec field" — same-cluster (same fix shape; H4's extension shell does have spec basis in `pi-integration.md`, so it gets `**Spec.**` not `**Convention.**`)
- "`depcheck` gate not set up or self-tested" — co-resolve (the recommendation here strikes `depcheck` from the gate, which is one of that finding's two viable fixes)
- "GitHub Actions workflow added but never validated" — same-cluster (also touches H1 hygiene scaffolding; resolves independently)
- "`no-static-state.test.ts` allow-list undefined" — same-cluster (touches the same H1 test; resolves independently)
- "REQ-ID system referenced everywhere but no leaf creates it" — decision-dependency (clarifying that horizontal phases are convention-derived constrains where the REQ-ID-creation leaf must live — it cannot be H1)
- "Exception-handling convention weaker than CLAUDE.md" — same-cluster (touches `conventions.md` Cross-cutting rules, the same section this fix amends)
