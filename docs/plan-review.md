...
_Triage tally: 2 blocker, 9 high, 20 medium retained; 9 low discarded; 9 low findings merged into 1 medium finding; 25 NIT dropped; 0 false dropped._
...
# T31 — Extension-bootstrap SDK-failure rule and `loom/load/extension-bootstrap-failed` have no closing leaf

**Original heading:** `loom/load/extension-bootstrap-failed` and the entire Extension-bootstrap SDK-failures rule are uncovered
**Original section:** Cross-cutting / global
**Kind:** spec-coverage
**Importance:** blocker
**Score:** 200
...
The spec page `spec_topics/pi-integration-contract/extension-bootstrap-and-per-loom.md` defines the **Extension-bootstrap SDK failures** rule: a per-call-type granularity contract for a throw or rejection from any of the registration calls in registration steps 1–5, surfaced through the registered diagnostic code `loom/load/extension-bootstrap-failed` (severity `E`, phase `load`, declared in `spec_topics/diagnostics/code-registry-load.md`). The rule pins five distinct surfaces, each with its own observable behaviour:

1. `pi.registerMessageRenderer` failure — the renderer registration is dropped, the factory still completes, and this extension instance's system notes permanently degrade to the `ctx.ui.notify` arm of the System-notes fallback chain.
2. `pi.registerCommand` failure (factory-time *or* `session_start`-time, per-loom) — only the failing loom is dropped; siblings still register; one diagnostic is emitted per failing loom (`details.loom` carries the slash-name).
3. `pi.registerFlag` failure — fatal to the whole extension; steps 2–5 do not execute; a single diagnostic is emitted.
4. `pi.on(...)` subscription failure — fatal to the whole extension; the remaining steps do not execute; a single diagnostic is emitted (`details.event` names the subscribed Pi event).
5. `session_start`-time `pi.getCommands()` read failure — the pending-registration list for that `session_start` pass is dropped (no `pi.registerCommand` calls issue); the handler swallows the throw and MUST NOT set drain state; one diagnostic is emitted.

No plan leaf owns this contract and no leaf asserts `loom/load/extension-bootstrap-failed`. `H4a` establishes the never-throw factory boundary and the per-call `try`/`catch` wrapping, but asserts neither the diagnostic code nor any of the five per-surface granularity behaviours. `V9b` owns registration steps 1–5 and the `session_start` collision pass and asserts `loom/runtime/registry-swap-failed`, but not the bootstrap-failure code or its granularity. The `coverage-matrix.md` code-keyed obligation table has no row for `extension-bootstrap-and-per-loom.md` or for the code, and `H5b`'s transitive-completeness Deps name no producing leaf for it.

Because `loom/load/extension-bootstrap-failed` is a registered diagnostic code with no closing leaf, the gap is self-witnessing: the `H5a` closing gate, on the live-corpus footing that binds at the loom 1.0 release gate, fails on a registered code that no green test asserts against the diagnostics registry.

## Plan Documents

- `docs/plan_topics/coverage-matrix.md` — Code-keyed obligation areas (no numbered REQ-IDs) table (edited)
- `docs/plan_topics/H5b-warn-only-canary.md` — Deps (transitive-completeness set) (edited)
- `docs/plan_topics/V9b-registration-drain-state.md` — Adds / Tests (option-dependent — edited only if the contract extends V9b)
- `docs/plan_topics/H4a-factory-shell-and-harness.md` — Adds (never-throw factory boundary) (read-only)
- `docs/plan_topics/H5a-closing-gate-automation.md` — closing-gate mechanism (read-only — the gate that self-witnesses the gap)
- new leaf file under `docs/plan_topics/` (option-dependent — created only under the new-leaf approach)
- `docs/plan.md` — V9 slice list (option-dependent — links the new leaf if one is created)

## Spec Documents

- `docs/spec_topics/pi-integration-contract/extension-bootstrap-and-per-loom.md` — §Extension-bootstrap SDK failures (read-only — obligation source)
- `docs/spec_topics/diagnostics/code-registry-load.md` — `loom/load/extension-bootstrap-failed` registry row (read-only — code definition)

## Affected Leaves

**Phases:** Horizontal, Vertical V9

**Leaves (implementation order):**

- `H4a` — Extension factory shell and end-to-end harness — (read-only context: establishes the never-throw factory boundary the contract builds on)
- `V9b` — Registration steps and drain-state contract — (modified — candidate host if the contract is folded in rather than given a new leaf)
- `<new>` — Extension-bootstrap SDK-failure granularity closing leaf (V9 slice) — (added — created under the new-leaf approach)
- `H5b` — Warn-only live-corpus canary — (modified — transitive-completeness Deps gains the closing leaf)

## Consequence

**Severity:** blocking

The five per-call-type bootstrap-failure MUSTs would ship unimplemented, and the registered code `loom/load/extension-bootstrap-failed` would have no closing leaf — so the `H5a` closing gate fails on its live-corpus footing and `H6a` (the loom 1.0 release-gate activation) cannot complete. The gap blocks the release gate from activating cleanly rather than silently shipping.

## Issue introduction

**Verdict:** present-since-inception
**Introducing commits:** `c6a664e` (`pi-loom plan: build/update plan for spec.md + review`, 2026-06-10) — the commit that authored the current plan corpus.
**History:** The spec rule and its registered code predate the plan. The failure-contract rule entered the spec at `2d2fc8e` (`pi-loom spec: resolve "Pi SDK capability calls have no failure contract"`, 2026-05-06) and was refined to the present five-surface per-call-type granularity by `79ea4e8` (`resolve "throw from pi.on subscription has no granularity rule"`, 2026-06-06) and `bf85092` (`resolve "non-Error-throw coercion + session_start pi.getCommands() failure contract"`, 2026-06-06). When the plan corpus was authored at `c6a664e` the rule and code already existed, but no closing leaf, no `coverage-matrix.md` row, and no `H5b` transitive-completeness Deps entry was created for them. `git log -S"extension-bootstrap-failed"` over `docs/plan*` returns no commits — the token has never appeared anywhere in the plan corpus — so the gap was never introduced by a later edit; it is an omission present since the plan corpus's inception.
...
- T21 "Per-loom registration `ToolDefinition` field-derivation MUSTs unnamed by any leaf" — same-cluster (same spec page `extension-bootstrap-and-per-loom.md`, same coverage-matrix-row gap; a single new V9-registration leaf could host both, but each closes independently)
- T03 "Two PIC teardown pages absent from coverage-matrix code-keyed table" — same-cluster (same class of missing code-keyed coverage-matrix row; resolves independently)
- T20 "Systemic leaf over-bundling across the leaf corpus" — decision-dependency (choosing to fold this contract into V9b vs a new leaf depends on resolving V9b's over-bundling)
