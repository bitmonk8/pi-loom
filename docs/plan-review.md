...
_Triage tally: 1 blocker, 5 high, 31 medium retained; 34 low discarded; 4 low/duplicate findings merged into 4 cluster findings; 16 NIT dropped; 0 false dropped._
...
# T36 — Session double's model / tool / binder-response scripting surface is undefined

**Original heading:** Harness model/tool/binder-response scripting surface is undefined
**Original section:** docs/plan_topics/H4a-factory-shell-and-harness.md
**Kind:** assumptions, implementability
**Importance:** high
**Score:** 100
...
`H4a` establishes the reusable end-to-end harness and its in-process Pi session double, plus a **session-double fidelity contract**. That contract enumerates only four *observable-output* axes the double must reproduce: streamed-token order relative to `ctx.waitForIdle()`, single prompt-mode turn append, the `pi.on` cancel-forward subscription, and cancellation propagation. None of the four define the *input* side — the surface a test uses to program what the double does: what assistant turns (and streamed fragments) the model emits, what a `tool_use` resolves to (including `isError` results), what a binder/provider call returns or how it fails, and where a retry, a `tool_loop.max_rounds` round-exhaustion, or a cancellation is injected.

Every harness-driven assertion in the plan presupposes this scripting surface. `M-T`/`M` assert "exactly one appended turn" / `SLSH-2`. `H7a` drives a binder-retry-class breach co-occurring with a `tool_loop.max_rounds` round boundary and asserts `CIO-5` arbitration order. The per-leaf tests for `V9c`, `V11f`, `V13c`, `V13d`, and `V17a` all depend on the same programmability.

Because the surface is undefined, each leaf's implementer would invent a divergent per-leaf scripting API for the double. The shared `H7a` integration gate and the per-leaf gates would then be written against incompatible scripting APIs, and the conversation-output assertions become effectively unwritable against one harness.

## Plan Documents

- `docs/plan_topics/H4a-factory-shell-and-harness.md` — `Adds.` (fidelity contract) and the self-check `Tests` bullet (edited)
- `docs/plan_topics/H7a-integration-acceptance.md` — `Tests` / `Ships when` (read-only)
- `docs/plan_topics/M-minimal-slash-command.md` — `Tests` (read-only)
- `docs/plan_topics/M-T-minimal-slash-command.md` — `Tests` (read-only)
- `docs/plan_topics/V9c-conversation-drive.md` / `V11f-binder-retry-taxonomy.md` / `V12a-slash-dispatch.md` / `V13c-query-tool-loop.md` / `V13d-query-failure-repair.md` / `V17a-cancellation-core.md` — `Tests` and their `-T` partners (read-only)
- `docs/plan_topics/conventions.md` — end-to-end harness / harness self-checks (read-only)

## Spec Documents

None. The scripting surface is a plan-level test-harness contract; no spec REQ-ID or host rule is touched.

## Affected Leaves

**Phases:** Horizontal, MVP, Vertical slices (V9, V11, V12, V13, V17)

**Leaves (implementation order):**

- `H4a` — Extension factory shell and end-to-end harness — (modified)
- `H7a` — Terminal integration-acceptance run (cross-slice end-to-end gate) — (blocked)
- `M-T` — Minimal end-to-end `.loom` slash command (tests) — (blocked)
- `M` — Minimal end-to-end `.loom` slash command — (blocked)
- `V9c` — Prompt-mode conversation drive and active-set gating — (blocked)
- `V11f` — Binder cancellation, per-class retry budget, and failure taxonomy — (blocked)
- `V12a` — Slash dispatch, overflow, and streaming — (blocked)
- `V13c` — Query tool loop and typed two-phase — (blocked)
- `V13d` — Query failure and respond-repair — (blocked)
- `V17a` — Cancellation core — (blocked)

(The red acceptance assertions for the paired feature leaves live in their `-T` partners — `V9c-T`, `V11f-T`, `V12a-T`, `V13c-T`, `V13d-T`, `V17a-T` — which cannot be authored without the shared scripting surface.)

## Consequence

**Severity:** correctness

Without a single defined scripting surface, two reasonable implementers invent divergent per-leaf APIs for programming the double's model turns, tool results, binder failures, retries, round-exhaustion, and cancellation injection. The shared `H7a` integration gate (which must force a co-occurring ceiling #3 / ceiling #2 breach) and the per-leaf gates are then written against mutually incompatible scripting, and the conversation-output assertions cannot compose against one harness.

## Issue introduction

**Verdict:** present-since-inception
**Introducing commits:** `c6a664e` (pi-loom plan: build/update plan for spec.md + review)
**History:** `git log --follow` over `docs/plan_topics/H4a-factory-shell-and-harness.md` shows the leaf was created in `c6a664e` already referencing an "in-process Pi session double" the harness "drives a slash dispatch" against, with no definition of how a test programs that double's output. `git log -S 'session-double fidelity contract'` shows the fidelity contract first appeared in `eeb0014` and was refined in `07403da`, `85fc906`, and `328ba4d`; each of those enumerated or sharpened the four *observable-output* axes but none introduced the *input*/scripting surface. The gap is original to the harness/double concept.
...
In `docs/plan_topics/H4a-factory-shell-and-harness.md`, extend the **session-double fidelity contract** in the `Adds.` field and its self-check `Tests` bullet so the harness defines one shared response-programming surface alongside the existing four observable axes. The surface must give a test an observable way to:

- script the sequence of assistant turns the double emits and the streamed fragments per turn;
- script the `tool_use` result the double returns for a given tool call, including an `isError: true` tool-result and a mixed-success parallel batch (the `V13c` tool-calls vector);
- script the binder/provider-call response or failure the harness feeds the binder path, including a malformed-envelope failure and a transport-class failure that drive the `V11f` retry budget;
- force a `tool_loop.max_rounds` round-exhaustion (`V13c` ceiling #2);
- inject an abort/cancellation at a chosen point in the drive (pre-call, in-flight provider call, and during a budgeted retry — the `V11f` / `V17a` vectors).

State explicitly that this surface is the single shared contract all harness-driven leaves consume (`M`, `M-T`, `H7a`, `V9c`, `V11f`, `V12a`, `V13c`, `V13d`, `V17a`), so they script against one API rather than per-leaf inventions, and add to the self-check that the scripting surface drives the double deterministically. The literal injection points are the content the contract must name; leave the harness API's exact shape to the implementer.

## Relationships

- T31 "Real-host fidelity of the session double has no reproducible detection point" — same-cluster (same `H4a` double; concerns output fidelity vs real Pi, resolves independently of the input/scripting surface)
- T32 "H7a Tests bullets 1–2 leave 'expected appended turns' / 'expected diagnostics' without a defined referent" — must-precede (defines what the harness can observably assert; settle the scripting surface before H7a's golden referents)
- T24 "V4c-T/V4c assert no-rollback over subagent, query, tool-call, and invoke surfaces built in later slices" — must-precede (the seam-based exercise V4c-T routes through relies on this scripting surface)
- T30 "M's happy-path discovery read has no defined source under the ambient-access ban" — must-precede (M's in-memory fixture supply lands in the same H4a harness contract)
- T28 "V12a Ships-when gate omits the SLSH-2 failure-path assertions" — must-precede (V12a's failure-path assertions run through the H4a double)
- T29 "V18c-T runtime-evidence test exercises integrated features its Deps do not satisfy" — must-precede (Option B's feature-free harness double depends on this scripting surface)

---

# T37 — Broad-catch allow-comment predicate is unsatisfiable for two of its own mandated exempt sites

**Original heading:** Broad-catch allow-comment vs exempt sites without a REQ-ID
**Original section:** docs/plan_topics/conventions.md
**Kind:** consistency
**Importance:** blocker
**Score:** 200
...
The *Specific exception types only* cross-cutting rule in `conventions.md` permits a `catch (e: unknown)` only at an enumerated **exempt broad-catch site**, and only when the same line carries a `// allow-broad-catch: <REQ-ID> — <spec-page>` comment. The loom 1.0 closing gate then "asserts every allow-list entry has a matching REQ-ID present in `coverage-matrix.md`." The allow-comment grammar and the gate predicate therefore both demand a numbered `PREFIX-N` REQ-ID that resolves to a coverage-matrix row.

Two of the rule's own five enumerated exempt sites cannot satisfy that demand:

- **Site (1), the generic Pi SDK boundary site** — defined as any call into / `typeof` / `"in"` / property access against a `@earendil-works/pi-coding-agent` value whose runtime shape loom cannot statically guarantee. This is a structural exemption with no associated spec obligation, so no `PREFIX-N` REQ-ID exists for it. (`H4a`'s factory never-throw boundary is the canonical consumer.)
- **Site (3), the *Unknown-reason rule* `event.reason` property-access read** — closed by `V9h`, which owns the unknown-reason rule through `loom/host/session-shutdown-*` diagnostic-registry codes and un-anchored MUSTs, not a numbered REQ-ID. The `coverage-matrix.md` *Code-keyed obligation areas* table does not even list `unknown-reason-rule.md`.

The predicate is thus unsatisfiable for sites the same paragraph mandates as exempt: an implementer writing the mandated broad catch has no valid REQ-ID to cite, and the closing gate has no row to match against. The sibling *Sequential by default* rule already resolved exactly this shape — its allow-comment and gate predicate accept "a REQ-ID **or** a code-keyed obligation area" — but that widening was never mirrored onto the broad-catch rule, leaving the two parallel rules inconsistent.

## Plan Documents

- `docs/plan_topics/conventions.md` — *Specific exception types only* (edited)
- `docs/plan_topics/coverage-matrix.md` — *Code-keyed obligation areas (no numbered REQ-IDs)* (option-dependent)
- `docs/plan_topics/H2a-cross-cutting-gates.md` — `no-broad-catch` allow-comment fixture test (edited)
- `docs/plan_topics/H5a-closing-gate-automation.md` — closing-gate allow-list reconciliation (edited)
- `docs/plan_topics/H4a-factory-shell-and-harness.md` — Pi SDK boundary exempt-site consumer (read-only)
- `docs/plan_topics/V9h-degraded-unknown-reason.md` — `event.reason` exempt-site consumer (read-only)
...
**Phases:** Horizontal phases; Vertical slices (V9 — Extension host integration)
...
The mandated broad catches at the Pi SDK boundary (`H4a`) and the unknown-reason `event.reason` read (`V9h`) cannot carry a `// allow-broad-catch:` comment that satisfies the closing-gate predicate, so once those leaves land, the loom 1.0 release gate (`H6a`) cannot pass without inventing a non-existent REQ-ID. Independent implementers diverge on what token to cite (a fabricated REQ-ID, a diagnostic code, the spec-page anchor, or nothing), and the gate cannot fire correctly against any consistent choice.

## Issue introduction

**Verdict:** multi-commit-interaction
**Introducing commits:** 15f69aa — finish scaffold/template re-pivot (2026-05-26, Thomas Andersen); b8bc4ce — resolve "Broad-catch sites lack matching no-broad-catch exemption" (2026-06-04, Thomas Andersen); 75b6a9b — resolve "Sequential by default carve-out admits only a numbered REQ-ID" (2026-06-10, Thomas Andersen)
**History:** 15f69aa rewrote the *Specific exception types only* bullet into its REQ-ID-only form — dropping the earlier transitional spec-page-anchor allow-comment escape — so even the lone "Pi SDK boundary site" exemption (1) had no satisfiable REQ-ID from the start. b8bc4ce expanded the single exemption to five enumerated sites, adding the unknown-reason `event.reason` read (3), while leaving the REQ-ID-only allow-comment grammar and gate predicate untouched. 75b6a9b then widened the parallel *Sequential by default* predicate to accept a code-keyed obligation area but did not mirror that widening onto the broad-catch rule, making the inconsistency overt.
...
Widen the broad-catch allow-comment grammar and closing-gate predicate in the `conventions.md` *Specific exception types only* rule to mirror *Sequential by default*, rather than narrowing the exempt-site enumeration (narrowing would re-break the spec-fidelity fix in b8bc4ce, which deliberately admitted these spec-mandated broad-catch sites).

Concretely:

- In `conventions.md` *Specific exception types only*: change the gate sentence so an allow-list entry's cited token resolves to any of: a REQ-ID present in `coverage-matrix.md`, an enumerated *Code-keyed obligation areas (no numbered REQ-IDs)* entry there, or a `loom/...` diagnostics-registry code — paralleling the *Sequential by default* sentence. Update the allow-comment token placeholder from `// allow-broad-catch: <REQ-ID> — <spec-page>` to the widened form (e.g. `// allow-broad-catch: <REQ-ID-or-code-keyed-area-or-registry-code> — <spec-page>`).
- The generic Pi SDK boundary site (1) traces to no spec obligation, REQ-ID, code, or code-keyed area at all. Give site (1) an explicit anchor the widened predicate admits: either register a "Pi SDK boundary" entry in `coverage-matrix.md` *Code-keyed obligation areas*, or admit the exempt-site class itself as a valid allow-comment token for that one structural site. State which, so the gate has a concrete thing to match.
- If the predicate keys off the *Code-keyed obligation areas* table, add the `unknown-reason-rule.md` page (and the Pi SDK boundary anchor, if chosen) as rows there with `V9h` (resp. `H4a`) as the closing leaf.
- Reconcile the downstream gate machinery to the widened predicate: `H2a`'s *Specific exception types only* fixture test and `H5a`'s allow-list reconciliation must accept the widened token set rather than a bare REQ-ID.

Edge case: sites (2)/(4)/(5) already trace to PIC REQ-IDs (`V9a` PIC-3…6, `V9g` PIC-7, `V7a` PIC-21) and continue to satisfy the REQ-ID arm unchanged; only sites (1) and (3) need the widened arms.
