...
# T38 — Live-host smoke pass criterion assumes a non-deterministic LLM reproduces a transcript recorded against the in-process double

**Original heading:** Live-host smoke pass criterion assumes a real LLM reproduces a double-recorded golden transcript
**Original section:** Consolidated Plan Review — plan
**Kind:** assumptions
**Importance:** high
**Score:** 100
...
`H4a` defines the manual real-host smoke run — loom 1.0's only post-merge detection mechanism for the in-process session double diverging from real Pi — and pins its pass/fail criterion as: the live-host run **passes** iff its appended turns match `H7a`'s committed golden transcript *in order and count* **and** the run emits *exactly* the `loom-system-note` codes in `H7a`'s committed golden diagnostics list; any divergence in turn order, turn count, or emitted-code set is a **fail** (a "confirmed behavioural-divergence finding"). `H6a`'s release-gate acceptance bullet and `V18c`'s version-bump revert path both consume that same criterion.

But those goldens are recorded by `H7a` against the **deterministic in-process Pi session double** (`H7a` runs end-to-end "against the in-process Pi session double"). The smoke replaces that double with a **live, non-deterministic LLM** driving the same multi-feature fixture — typed query → tool loop → code-tool invoke → schema lowering/validation → binder → cancellation. The turn count and order (how many tool-loop rounds the model takes, whether the respond-repair path fires) and the emitted-code set (which `loom/...` diagnostics fire) all depend on live-model behaviour. Exact reproduction of a double-recorded transcript by a real model on a multi-feature pipeline is asserted as the pass criterion with no backing for why it is achievable, and the fixture is not specified to be model-output-invariant.

The consequence is a criterion that benign model variance fails by construction: the smoke's "fail" verdict is wired to a Pi-SDK-pin revert (`V18c`) and a merge block, so a non-defective live run is liable to trigger a spurious revert/block — or operators quietly reinterpret "pass" ad hoc, leaving loom 1.0's only real-host fidelity evidence undefined.

## Plan Documents

- `docs/plan_topics/H4a-factory-shell-and-harness.md` — Tests bullet, **Pass/fail criterion** sentence (edited)
- `docs/plan_topics/H6a-live-corpus-activation.md` — Release-gate acceptance Tests bullet and **Ships when** (edited)
- `docs/plan_topics/H7a-integration-acceptance.md` — fixture `.loom` / golden-transcript construction (read-only)
- `docs/plan_topics/V18c-version-bump-checklist.md` — runtime-evidence / revert-path reference to the smoke (read-only)
...
**Phases:** Horizontal, Vertical V18, Release gate

**Leaves (implementation order):**

- `H4a` — Extension factory shell and end-to-end harness — (modified)
- `H7a` — Terminal integration-acceptance run — (read-only context)
- `V18c` — Pi version-bump procedure and gates — (modified; reference inherits the narrowed criterion; edit only if the criterion is not single-sourced from `H4a`)
- `H6a` — Live-corpus closing-gate activation — (modified)
...
A live, non-deterministic LLM driving `H7a`'s multi-feature fixture will frequently not reproduce the exact turn order/count or emitted-code set that `H7a`'s goldens captured against the deterministic in-process double. Under the stated criterion, benign model variance is scored as a confirmed behavioural-divergence finding — a Pi-SDK-pin revert trigger or a merge block — so the gate fires spuriously, or operators reinterpret "pass" inconsistently and loom 1.0's only real-host fidelity evidence becomes ill-defined.
...
**History:** Commit e7f14dd added the manual real-host smoke run as the post-merge double-vs-real divergence detection mechanism and, in the same edit, introduced the `H4a` **Pass/fail criterion** sentence asserting the live-host run "passes iff" its appended turns match `H7a`'s committed golden transcript in order and count and it emits exactly the golden diagnostics codes. Those goldens were recorded against the deterministic in-process session double established earlier in `H4a`, so the assumption that a live, non-deterministic model reproduces an exact match entered the corpus together with the smoke in that single commit; there was no prior live-host criterion to inherit the defect from.
...
Decouple the live-host criterion from the exact golden transcript. Rewrite `H4a`'s **Pass/fail criterion** so the live-host smoke passes on model-output-invariant observables rather than on exact turn-order/count and exact code-set equality. Candidate invariants: the run completes without error; the binder pass produces structurally-valid output; cancellation propagation is observed; the emitted `loom-system-note` codes are a subset of a permitted set drawn from the slices in `H7a`'s **Deps**; the four fidelity-contract axes hold. The in-process `H7a` gate keeps its exact-match goldens unchanged (deterministic double — exact match is correct there).

In `H4a`, replace the "appended turns match … in order and count … and emits exactly …" predicate with the model-output-invariant predicate; keep the single-shared-fixture reuse with `H7a`. In `H6a`, update the release-gate acceptance bullet and **Ships when** to restate `H4a`'s narrowed criterion (or reference it by anchor). `H7a` and `V18c` stay as-is if both reference the criterion by `H4a` anchor.

Watch the edge case that the narrowed "fail" remains observable enough to drive the `V18c` revert / merge-block path — a genuine divergence must still produce a falsifiable "fail".

## Relationships

- T31 "Manual real-host fidelity gate leaves no falsifiable record" — same-cluster (same manual-smoke item; record-keeping concern, resolves independently).
- T25 "Golden transcript and diagnostics list have no initial-correctness establishment step" — same-cluster (touches the H7a goldens this finding reuses; resolves independently).
...
# T40 — V4d `QueryError` family consumed by V13a / V14a / V17a without a declared or transitive dependency

**Original heading:** V4d `QueryError` variants referenced by V13a/V14a/V17a without a declared or transitive dependency
**Original section:** Consolidated Plan Review — plan
**Kind:** ordering
**Importance:** high
**Score:** 100
...
`V4d` owns the `QueryError` nine-variant union, the `ValidationError` / `ValidationIssue` surface, and the `ToolLoopExhaustedError` shape (its `Spec.` is `errors-and-results/queryerror-variants.md` + `error-model.md`). Three vertical leaves directly reference symbols from that family:

- `V13a` short-circuits a degenerate template to `ValidationError{empty_template, attempts:0}` (Adds and third Tests bullet).
- `V14a` maps a host-side denial to `Err(QueryError{kind:"code_tool", cause:"execution"})` (denial-surface Tests bullet).
- `V17a` surfaces child cancellation as `Err(QueryError{kind:"cancelled"})` (downward-only-propagation Tests bullet).

None of these leaves — nor their paired `-T` tests-task leaves — names `V4d` in `Deps.`, and none reaches it transitively. Confirmed by walking the full dependency closure: `V13a` (`V13a-T, V11d, V2c`), `V14a` (`V14a-T, V15a, V9f, V8a`), and `V17a` (`V17a-T, V8a`) all resolve closures that exclude `V4d`. The omission has been present since the leaves were first authored; no revision ever added the edge.

Sibling leaves that consume the same `V4d` symbols *do* declare the edge — `V13d` (`Deps: V13c, V4d, V9j`) and `V12b` (`Deps: V12a, V4d, V15a`) — so the three omissions read as accidental gaps rather than a deliberate design choice.

## Plan Documents

- `docs/plan_topics/V13a-query-render.md` — `Deps.` field (edited)
- `docs/plan_topics/V13a-T-query-render.md` — `Deps.` field (edited)
- `docs/plan_topics/V14a-tool-calls.md` — `Deps.` field (edited)
- `docs/plan_topics/V14a-T-tool-calls.md` — `Deps.` field (edited)
...
- `docs/plan_topics/V4d-queryerror-variants.md` — dependency target, owns the referenced symbols (read-only)
- `docs/plan.md` — Vertical-slices V13/V14/V17 leaf listings (read-only)

## Spec Documents

None — `V4d` already owns the `QueryError`/`ValidationError`/`ToolLoopExhaustedError` surface in `errors-and-results/`; the fix is internal to plan `Deps.` fields.

## Affected Leaves

**Phases:** Vertical slices — V13 (Query), V14 (Tool calls), V17 (Cancellation)
...
Following "pick the next leaf whose `Deps` are satisfied," an executor can reach `V17a` (deps `V8a → H3a` only), `V13a`, or `V14a` before `V4d` exists. The `QueryError`/`ValidationError` references then fail to compile, and each paired `-T` leaf's `Ships when` ("the tests exist, compile, and fail red **for the intended reason**") cannot be satisfied — the red is an undeclared-symbol compile error, not the intended assertion failure. The implementer is pushed to hand-roll an ad-hoc local error shape that need not match `V4d`'s wire forms.

## Issue introduction

**Verdict:** present-since-inception
**Introducing commits:** `c6a664e` (2026-06-10, "pi-loom plan: build/update plan for spec.md + review")
**History:** The plan corpus is git-tracked. All three leaf files (and their `-T` partners) were authored in `c6a664e`; `git show c6a664e:` confirms their `Deps.` fields already omitted `V4d` at that commit (`V13a`: `V13a-T, V11d, V2c`; `V14a`: `V14a-T, V15a, V9f`; `V17a`: `V17a-T, V8a`). `git log -S 'V4d'` over the three files returns no commit, so `V4d` was never present in any of their `Deps` fields — the missing edge dates to initial authoring, not a later edit. (Later commits to `V17a`/`V14a` touched suppression-test and `V8a`-dep content, not the `V4d` edge.)
...
Add `V4d` to the `Deps.` field of all six leaves. Concrete edits:

- `V13a-query-render.md` → `**Deps.** \`V13a-T\`, \`V11d\`, \`V2c\`, \`V4d\``
- `V13a-T-query-render.md` → `**Deps.** \`V11d\`, \`V2c\`, \`V4d\``
- `V14a-tool-calls.md` → `**Deps.** \`V14a-T\`, \`V15a\`, \`V9f\`, \`V8a\`, \`V4d\``
- `V14a-T-tool-calls.md` → `**Deps.** \`V15a\`, \`V9f\`, \`V8a\`, \`V4d\``
- `V17a-cancellation-core.md` → `**Deps.** \`V17a-T\`, \`V8a\`, \`V4d\``
- `V17a-T-cancellation-core.md` → `**Deps.** \`V8a\`, \`V4d\``

`V4d`'s own deps are `V4d-T`, `V5d` — both already sequenced ahead of the V13/V14/V17 slices — so the new edges introduce no cycle and force no reordering.

Edge case: `V13c` is independently missing the same `V4d` edge (it consumes `ToolLoopExhaustedError`); that is covered by its own finding and should be edited there, not folded into this one.
