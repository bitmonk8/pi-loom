...
# T49 — project-config §Plan rules describes horizontal IDs as `H1`–`H4`, contradicting the live `H<n><letter>` surface

**Original heading:** §Plan rules — horizontal-phase IDs `H1`–`H4` contradict live `H1a`–`H7a` and conventions.md `H<n><letter>`
**Original section:** .pi/project-config.md
**Kind:** implementability
**Importance:** high
**Score:** 90
**MustFix:** false
...
`.pi/project-config.md` §Plan rules opens with "Plan-leaf IDs follow `H1`–`H4` (horizontal phases), `M` (MVP), and `V<N><letter>`". This is the ID-allocation reference an allocator or fixer consults when minting a new leaf ID, and its horizontal-phase clause disagrees with the rest of the corpus on two independent points.

First, the **form**: project-config gives the bare `H1`–`H4` (no letter suffix), but `docs/plan_topics/conventions.md` states leaf IDs follow `<group><letter>` and reserves "`H1`–`Hn` … (with their leaf and `-T` forms)", and every live horizontal leaf uses the `H<n><letter>` form — `H1a`, `H2a`, `H3a`, `H4a`, `H5a`, `H5b`, `H6a`, `H7a`. Second, the **range**: project-config caps the horizontal series at `H4`, but the live plan already runs through `H7a` (plus the `H5b`/`H6a` release-gate leaves).

An allocator that takes project-config at its word would either refuse to allocate past `H4` or mint a letterless ID such as `H8`, which violates the `H<n><letter>` scheme conventions.md gates leaf shape against. The authoritative statement of the horizontal-ID form lives in conventions.md; project-config's §Plan rules is a stale secondary surface that was never updated to track it.

## Plan Documents

- `.pi/project-config.md` — §Plan rules (edited)
- `docs/plan_topics/conventions.md` — Plan structure / leaf-ID form (read-only — authoritative source)
- `docs/plan.md` — Horizontal phases, Release gate (read-only — evidence of the live `H1a`–`H7a` surface)
...
None — the fix corrects descriptive allocation prose in project-config. No live leaf's identity, acceptance criteria, Deps, or sequencing changes; the live horizontal leaves (`H1a`, `H2a`, `H3a`, `H4a`, `H5a`, `H5b`, `H6a`, `H7a`) already conform to the corrected form.

## Consequence

**Severity:** correctness

An allocator or fixer consulting project-config §Plan rules to mint a new horizontal leaf would cap at `H4` (the live plan already reaches `H7a`) or allocate a letterless ID such as `H8`, producing an identifier that violates the `H<n><letter>` scheme conventions.md gates against. Two implementers reading the two surfaces would diverge on both the form and the upper bound of horizontal IDs.

## Issue introduction

**Verdict:** present-since-inception
**Introducing commit:** `854dead` "Migrate fix-spec/plan-shape-single-findings orchestration to pi-config" (2026-05-08) — created `.pi/project-config.md` with the §Plan rules `H1`–`H4` clause, which has remained byte-identical since (`git log -S '`H1`–`H4`' -- .pi/project-config.md` returns only this commit).
**History:** The `H1`–`H4` text was authored at the artefact's inception and never revised. The contradicting surfaces landed afterward: conventions.md adopted the `H<n><letter>` / `H1`–`Hn` form in `15f69aa` (2026-05-26), and the live plan grew to the `H1a`–`H7a` horizontal series in `c6a664e` (2026-06-10) and subsequent plan commits. project-config's §Plan rules was not kept in sync as those siblings evolved, so the stale clause hardened into a live contradiction. The durable fix is to re-state the project-config clause to match conventions.md, not to revert any single later commit.
...
Edit the first sentence of `.pi/project-config.md` §Plan rules so the horizontal-phase clause states the `H<n><letter>` form and an open-ended range, matching conventions.md. Replace the existing clause:

> Plan-leaf IDs follow `H1`–`H4` (horizontal phases), `M` (MVP), and `V<N><letter>` (vertical-slice leaves, e.g. `V4b`, `V18o`).

with text of the form:

> Plan-leaf IDs follow `H<n><letter>` (horizontal phases, e.g. `H1a`, `H5b`, `H7a`), `M` (MVP), and `V<N><letter>` (vertical-slice leaves, e.g. `V4b`, `V18o`).

conventions.md remains the authoritative home for the leaf-ID scheme; the project-config clause must agree with it rather than re-derive or re-restate the binding ritual. The surrounding project-config sentences ("use the next free letter in the target phase"; the `-T` pairing rule) already match the letter form and need no change. The fix is confined to `.pi/project-config.md`; do not touch `docs/plan_topics/conventions.md`, `docs/plan.md`, or the spec for this finding.
...
# T55 — V9j has no coverage-matrix row — its un-anchored binder/provider MUSTs are untraceable

**Original heading:** No coverage-matrix entry (untraceable leaf) — V9j
**Original section:** V9j — Binder inference and provider mapping
**Kind:** traceability
**Importance:** high
**Score:** 100
**MustFix:** true
...
`V9j` cites two non-narrative `pi-integration-contract/` spec pages as its **Spec** field — `binder-inference.md` and `provider-error-mapping.md` — both of which carry the registered `PIC` prefix (per [`req-id-prefix-table-active-a.md`](../spec_topics/governance/req-id-prefix-table-active-a.md)) yet anchor their obligations only at section level (`#binder-inference-call`, `#provider-error-mapping`, `#provider-seed-field-mapping`, …) with no `PIC-N` REQ-ID and no `loom/...` registry code. Those owned normative MUSTs (the `complete()` symbol/signature/structured-output re-validation MUST on `binder-inference.md`; the provider-error→`QueryError` mapping-table, `TransportError.retryable`, overflow-extraction, stop-reason, and provider seed-field-mapping MUSTs on `provider-error-mapping.md`) are un-anchored-obligation residue under [`governance.md` GOV-22](../spec_topics/governance/req-id-prefix-table-active-b.md#gov-22).

`coverage-matrix.md` does not list `V9j` anywhere — not as a numbered-REQ-ID closing leaf and not as a code-keyed obligation-area closing leaf. A grep across `coverage-matrix.md` returns no occurrence of `V9j`, `binder-inference`, `provider-error-mapping`, or `typed-query-unsupported-provider`.

The `H5a` closing gate's un-anchored-MUST recogniser scans every non-narrative `spec_topics/**` page and fails on any owned normative MUST that carries no `PREFIX-N` REQ-ID, no registry code, and no row in the *Code-keyed obligation areas (no numbered REQ-IDs)* table with a closing leaf. On the live-corpus footing that binds at the loom 1.0 release gate (`H6a`'s hard-fail flip, pre-flighted by `H5b`'s warn-only canary), the un-anchored MUSTs on these two pages have no closing-leaf row, so the gate has no way to certify that `V9j` closes them. (The `loom/load/typed-query-unsupported-provider` warning code that `V9j` also surfaces is a *registered* code, reconciled by `H5a`'s registry-code arm against `V9j`'s asserting test — it does not itself need a matrix row.)

## Plan Documents

- `docs/plan_topics/coverage-matrix.md` — *Code-keyed obligation areas (no numbered REQ-IDs)* table (edited)
- `docs/plan_topics/V9j-binder-inference-provider-mapping.md` — Spec / Tests fields (read-only)
- `docs/plan_topics/H5a-closing-gate-automation.md` — un-anchored-MUST gate arm (read-only)
- `docs/plan_topics/H5b-warn-only-canary.md` — warn-only live-corpus canary (read-only)
- `docs/plan_topics/H6a-live-corpus-activation.md` — release-gate hard-fail flip (read-only)
- `docs/plan_topics/conventions.md` — *REQ-ID discipline* (read-only)

## Spec Documents

- `docs/spec_topics/pi-integration-contract/binder-inference.md` — §Binder inference call (read-only)
- `docs/spec_topics/pi-integration-contract/provider-error-mapping.md` — §Provider error mapping, §Provider seed-field mapping (read-only)

## Affected Leaves

**Phases:** Horizontal (release gate), Vertical V9

**Leaves (implementation order):**

- H6a — Live-corpus release-gate activation — (blocked)
- V9j — Binder inference call and provider-error mapping — (blocked)

## Consequence

**Severity:** blocking

On the live-corpus footing, `H6a`'s hard-fail release gate reddens on the un-anchored MUSTs on `binder-inference.md` and `provider-error-mapping.md` because neither is enumerated in the *Code-keyed obligation areas* table with a closing leaf, so the loom 1.0 release gate cannot pass green. `V9j`'s closure of its own Spec pages is unverifiable from the matrix, and `H5b`'s canary surfaces the gap as a finding ahead of the flip.

## Issue introduction

**Verdict:** single-commit
**Introducing commits:** c6a664e — pi-loom plan: build/update plan for spec.md + review (2026-06-10, Thomas Andersen)
**History:** Commit c6a664e introduced the `V9j` leaf — citing the non-narrative `binder-inference.md` and `provider-error-mapping.md` pages and the `loom/load/typed-query-unsupported-provider` code — and in the same commit applied a 65-line update to `coverage-matrix.md`, but added no row mapping `V9j` to any REQ-ID or code-keyed obligation area. A pickaxe (`git log -S 'V9j'`) over `coverage-matrix.md` returns no commit, so `V9j` has never appeared in the matrix; the traceability gap has existed since the leaf was created.
...
Add two rows to the *Code-keyed obligation areas (no numbered REQ-IDs)* table in `docs/plan_topics/coverage-matrix.md`, each naming `V9j` as the closing leaf, in the same rule-driven style as the existing `V9b` / `V9c` / `V9e` / `V9h` PIC-area rows:
...
Do not add a separate matrix row for `loom/load/typed-query-unsupported-provider`: it is a registered diagnostics code already owned in `code-registry-load.md`, closed by `V9j`'s asserting test through `H5a`'s registry-code arm, not the un-anchored-MUST arm. Do not edit the spec pages — the obligations remain un-anchored GOV-22 residue dispositioned via the closing-leaf row, exactly as the comparable `V9b`/`V9h` PIC-page rows do; the spec is read-only for this fix.
