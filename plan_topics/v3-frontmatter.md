...
- **Adds.** Real YAML frontmatter; recognised fields: `description`, `argument-hint`, `mode`, `model`, `tools`, `system`, `bind_model`, `bind_context`, `bind_echo`, `coercion`, `params`. Unknown fields produce `loom/load/unknown-frontmatter-field` warning, with one carve-out: the key `timeout` is *not* routed through the generic warner — it is instead rejected by the V18o parse-phase path as `loom/parse/timeout-field-rejected` (severity `E`), so the more-specific deferred-feature error wins at the frontmatter site. Loom-specific fields other than `mode` and `description`/`argument-hint`/`params` are recognised but ignored with a "not yet implemented in this leaf" warning until their implementing leaf lands. Absent `params:` and `params: {}` are equivalent and mean "no-params loom"; `params: null` is `loom/load/params-null`. `argument-hint` is parsed and stored on the AST for binder-grounding consumption (V16f); declaring `argument-hint:` without `description:` emits the advisory `loom/load/argument-hint-not-displayed` so authors are not surprised by an empty-looking dropdown entry (the autocomplete UI shows only `description`).
- **Tests.** YAML parse errors point at correct column; unknown field warning shape; `timeout:` in frontmatter is *not* reported as `loom/load/unknown-frontmatter-field` (the V18o parse-phase code wins — cross-checked with V18o's frontmatter fixture); `mode: subagent` is the documented "not implemented yet" parse error referencing V12a; `params` field type-grammar fragment recognised (primitive types only here; array/named types in V3b); absent `params:` and `params: {}` produce the same internal no-params state; `params: null` is rejected with `loom/load/params-null` and the loom does not register; `argument-hint:` set without `description:` → `loom/load/argument-hint-not-displayed` advisory and the loom still loads; both fields present → no advisory.
- **Deps.** V2.
