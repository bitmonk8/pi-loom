# V13 — Wire names, descriptions, respond-repair
...
- **Adds.** After AJV validation against lowered schema, runtime walks JSON and rebuilds value with loom-side identifiers using each schema's translation map *and* at every position the lowered schema annotates as a named enum, reattaches the declaring-enum tag for that position so the value compares equal to a locally constructed variant of the same enum. Anonymous string-literal-union positions receive no tag. The walk recurses through arrays, nested object fields, and `Result.Ok` / `Result.Err` payloads; tags are attached at the same depth as the value the schema annotates and never propagate to enclosing arrays, objects, or `Result` wrappers. The same walk runs at every inbound boundary — typed-query response, typed registered-loom-callee return, `invoke<T>` return, and binder `args` — and is implemented once here.
- **Tests.** Model output `{"FirstName":"x"}` becomes loom value `{first_name:"x"}`; recursive structures translated; arrays of renamed objects translated; model-emitted `"low"` decoded at a named-enum (`Severity`) position arrives branded so `received == Severity.Low` is `true`; model-emitted `"low"` decoded at an anonymous `"low" | "medium" | "high"` position arrives unbranded so `Severity.Low == received` is `false` and `received == "low"` is `true`; cross-enum: a `Severity`-annotated position decoded as `"low"` is `!=` to `OtherEnum.Low` even when wire values match; recursion: branded values appear at the correct depth inside `array<Severity>`, inside a nested object field typed `Severity`, and inside `Result<Severity, …>.Ok` / `…Err` payloads, while the enclosing array / object / `Result` wrapper carries no tag; `JSON.stringify` of a rebuilt branded value yields the bare wire string (consistency with V10e's representation rule, exercised on an inbound-decoded value rather than a locally constructed one); each of the four boundaries exercises at least one of the above — one test under the V6i fixture, one under V14c (registered-loom-callee typed return), one under V15c, one under V16p — as thin wrappers asserting the decoded value equals a locally constructed `Severity.Low`, proving all four call sites route through this walk.
- **Deps.** V13a, V10e.
- **Ships when.** Wire-side JSON becomes loom-side values with declaring-enum tags reattached at named-enum positions.
...
## V13g — Respond-repair methodology: `validator_error`
...
## V13h — Respond-repair methodology: `schema_repeat`

- **Spec.** [Query](../spec_topics/query.md) (respond-repair).
- **Adds.** Follow-up turn re-states the schema instead of error.
- **Tests.** Follow-up turn text matches the `schema_repeat` template in [`query.md` Schema-validation respond-repair](../spec_topics/query.md#schema-validation-respond-repair); same termination/attempt-counting rules apply. **Blocked** on the spec authoring the canonical `schema_repeat` follow-up-turn template in `query.md` (or `frontmatter.md` `respond_repair.methodology`); until that template lands the assertion cannot be written without speculating about the wording.
- **Deps.** V13g.
- **Ships when.** Alternative methodology selectable.

## V13i — Respond-repair methodology: `none`
...
## V13j — Respond-repair preserves tool-call side effects

- **Spec.** [Query — Schema-validation respond-repair](../spec_topics/query.md), [Query — Non-validation failures during a respond-repair follow-up](../spec_topics/query.md).
- **Adds.** Respond-repair appends a *new* user turn rather than re-issuing the original (per spec's tool-side-effect concern). Non-`schema_validation` failures during a follow-up (transport, cancellation, tool-failure, tool-loop-exhausted, context-overflow, invoke-failure, invoke-callee-error, plus the defensive-case `cause: "empty_template"` short-circuit on a follow-up turn) propagate as the corresponding `QueryError` variant / cause and do **not** consume an `attempts` slot. Only `cause: "schema_validation"` failures consume slots. `context_overflow` short-circuits respond-repair permanently for the lifetime of that typed query.
- **Tests.** Conversation transcript shows malformed respond-tool call + follow-up user turn (not a re-run of the original user turn); a follow-up that fails with `transport` propagates `transport` (not `validation`) and the next attempt would still be available; a follow-up that overflows context returns `context_overflow` immediately and no further follow-ups are issued; a follow-up that hits `tool_loop_exhausted` propagates that variant; a follow-up that hits the empty-template short-circuit (defensive) propagates `validation` with `cause: "empty_template"` and does **not** consume an `attempts` slot.
- **Deps.** V13g, V6k.
