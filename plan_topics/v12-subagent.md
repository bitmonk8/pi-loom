...
- **Spec.** [Pi Integration Contract — subagent-mode drive](../spec_topics/pi-integration-contract.md), [Pi Integration Contract — Subagent session lifecycle](../spec_topics/pi-integration-contract.md), [Overview — Scope of a Loom File](../spec_topics/overview.md#scope-of-a-loom-file), [Implementation Notes — Runtime](../spec_topics/implementation-notes.md#runtime) (reference implementation of the typed-query mechanism for loom 1.0, reused inside the spawned session).
- **Adds.** Frontmatter `mode: subagent` accepted; runtime spawns in-process `AgentSession` (against `FakeAgentSession` in tests) with in-memory session manager and a loom-constructed `ResourceLoader` adapter whose `getSystemPrompt()` returns the resolved-and-interpolated frontmatter `system:` string and whose `getAppendSystemPrompt()` returns `[]` (per rule 4 of [Pi Integration Contract — Conversation drive — subagent mode](../spec_topics/pi-integration-contract.md)); the parent session's `DefaultResourceLoader` is **not** passed through unchanged. Replaces V3a's "not implemented yet" stub. Disposal runs in a `finally` block that wraps the entire interpreter execution against the spawned session, so `AgentSession.dispose()` fires on every exit path (normal return, `Err`, panic, cancellation, unexpected interpreter/SDK exception). `dispose()` failure is reported via `loom/runtime/subagent-dispose-failure` (see [Diagnostics](../spec_topics/diagnostics.md)) without masking the original error.
- **Tests.** Spawn happens at loom invocation, not at load; transcript not retained on `FakeFileSystem`; the `resourceLoader` passed to `createAgentSession` is a loom-constructed adapter (not the parent's `DefaultResourceLoader`) and `agentSession.systemPrompt` after spawn equals the resolved-and-interpolated loom `system:` string verbatim; mutating the host project's `AGENTS.md` file between loom load and spawn does not change `agentSession.systemPrompt` (the loom adapter does not read it); the adapter's `getAppendSystemPrompt()` returns `[]`; `createAgentSession` is called with **no** `signal` field on the options object (per [Pi Integration Contract — Conversation drive — subagent mode](../spec_topics/pi-integration-contract.md)); a one-shot `loomAbort.signal` listener is registered immediately after the awaited `createAgentSession(...)` resolves and detached in the same per-invocation `finally` that calls `AgentSession.dispose()`; `dispose()` invoked exactly once on normal return; `dispose()` invoked on `Err` return (query failure, tool failure, child invoke failure, validation failure); `dispose()` invoked on panic inside the subagent; `dispose()` invoked on parent-`AbortSignal`-fired-before-first-turn (spawn-then-immediate-cancel) — the test drives cancellation through the listener path: when the parent `AbortController` fires before the first turn, exactly one `session.abort()` call lands on the spawned handle and the per-invocation `finally` then runs `dispose()`; when `loomAbort.signal.aborted` is already true at the moment `createAgentSession(...)` resolves, `session.abort()` is invoked synchronously *before* the listener is registered (and the listener does not produce a second `session.abort()` call); `dispose()` invoked on unexpected interpreter exception; `dispose()` thrown error surfaces as `loom/runtime/subagent-dispose-failure` diagnostic without masking the original error; an `AgentSession.abort()` synchronous throw or a rejection of its returned `Promise<void>` routes to `loom/runtime/internal-error` without masking the original error the surrounding `finally` was protecting (per [Pi Integration Contract — Subagent session lifecycle](../spec_topics/pi-integration-contract.md)); nested subagent invocations dispose deepest-first; subagent-mode loom callees and the typed-query `__loom_respond_<slug>` one-shot tool reach the model exclusively via `customTools` on `createAgentSession` and are released by `dispose()` itself (no separate `pi.unregisterTool` step — Pi exposes none); zero `pi.registerTool` / `pi.setActiveTools` calls fire on the prompt-mode user session as a side effect of any subagent-mode invocation.
- **Deps.** V3a, V5e, H2, H4.
