...
// real whole-file parser (`parseThetaDocument`) and the runtime accessor.
//
// Every test reds today for the intended reason:
//   * the parse-phase tests red because the type-layer checkers are unfed a
//     per-expression static type in production, so `parseThetaDocument` emits no
...
// ===========================================================================
// theta/parse/non-boolean-condition — `cka-4` (owned V3a), integration witness.
// ===========================================================================
...
describe("V20c-T — non-boolean-condition fires in production", () => {
  it("rejects an integer `if` condition (reds today — `if 1` silently takes else)", () => {
    // theta/parse/non-boolean-condition — a non-`boolean` `if` condition.
    const codes = codesOf(["if 1 {", "  let a = 1", "}"].join("\n"));
    expect(codes).toContain("theta/parse/non-boolean-condition");
  });

  it("rejects an integer `while` condition", () => {
    // theta/parse/non-boolean-condition — a non-`boolean` `while` condition.
    const codes = codesOf(["while 1 {", "  let a = 1", "}"].join("\n"));
    expect(codes).toContain("theta/parse/non-boolean-condition");
  });

  it("rejects a non-`boolean` ternary condition", () => {
    // theta/parse/non-boolean-condition — a non-`boolean` ternary condition.
    const codes = codesOf("let x = 1 ? 2 : 3");
    expect(codes).toContain("theta/parse/non-boolean-condition");
  });

  it("rejects a non-`boolean` `&&` operand", () => {
    // theta/parse/non-boolean-condition — a non-`boolean` `&&` operand.
...
// ===========================================================================
// theta/parse/non-array-iterand — `cka-5` (owned V3c), integration witness.
// ===========================================================================

describe("V20c-T — non-array-iterand fires in production", () => {
  it("rejects `for x in <non-array>` (reds today — the iterand is unchecked)", () => {
    // theta/parse/non-array-iterand — the `for … in` iterand is not `array<T>`.
    const codes = codesOf(["for x in 5 {", "  let a = x", "}"].join("\n"));
    expect(codes).toContain("theta/parse/non-array-iterand");
  });
});

// ===========================================================================
// theta/parse/question-on-non-result — ERR-18 (owned V4a), integration witness.
// ===========================================================================

describe("V20c-T — question-on-non-result fires in production", () => {
  it("rejects `?` applied to a non-`Result` static type", () => {
    // theta/parse/question-on-non-result — `?` on an `integer` operand.
...
// ===========================================================================
// theta/parse/question-outside-result-fn — (owned V4a), integration witness.
// ===========================================================================
...
describe("V20c-T — question-outside-result-fn fires in production", () => {
  it("rejects `?` in a function whose return type is not Result<T, QueryError>", () => {
    // theta/parse/question-outside-result-fn — `?` in an `integer`-returning fn.
    const codes = codesOf(
      ["fn f(): integer {", "  let x = @`hi`?", "  1", "}"].join("\n"),
    );
    expect(codes).toContain("theta/parse/question-outside-result-fn");
  });
});
...
// ===========================================================================
// theta/parse/return-no-common-type — (owned V3d), integration witness.
// ===========================================================================

describe("V20c-T — return-no-common-type fires in production", () => {
  it("rejects a function whose return sites share no common type", () => {
    // theta/parse/return-no-common-type — `return 1` vs `return "a"`.
    const codes = codesOf(
      ["fn f() {", '  if true { return 1 }', '  return "a"', "}"].join("\n"),
    );
    expect(codes).toContain("theta/parse/return-no-common-type");
  });
});

// ===========================================================================
// theta/parse/integer-narrowing — (owned V2b), integration witness.
// ===========================================================================

describe("V20c-T — integer-narrowing fires in production", () => {
  it("rejects a narrowing `number → integer` coercion", () => {
    // theta/parse/integer-narrowing — `1.5` (number) assigned to `integer`.
    const codes = codesOf("let x: integer = 1.5");
    expect(codes).toContain("theta/parse/integer-narrowing");
  });
});

// ===========================================================================
// theta/parse/match-arm-type-mismatch — (owned V4a), integration witness.
// ===========================================================================

describe("V20c-T — match-arm-type-mismatch fires in production", () => {
  it("rejects `match` arms whose bodies share no common type", () => {
    // theta/parse/match-arm-type-mismatch — an `integer` arm vs a `string` arm.
    const codes = codesOf(
      ["let y = 1", "let z = match y {", "  0 => 1,", '  _ => "hi",', "}"].join(
        "\n",
      ),
    );
    expect(codes).toContain("theta/parse/match-arm-type-mismatch");
  });
});

// ===========================================================================
// theta/parse/non-indexable-receiver — (owned V3a), integration witness.
...
describe("V20c-T — non-indexable-receiver fires in production", () => {
  it("rejects indexing a `string` receiver (reds today — string indexing accepted)", () => {
    // theta/parse/non-indexable-receiver — `s[0]` on a `string`.
    const codes = codesOf(['let s = "hi"', "let c = s[0]"].join("\n"));
    expect(codes).toContain("theta/parse/non-indexable-receiver");
  });
});

// ===========================================================================
// theta/parse/non-string-object-index — (owned V3h), integration witness.
// ===========================================================================

describe("V20c-T — non-string-object-index fires in production", () => {
  it("rejects a non-`string` object index", () => {
    // theta/parse/non-string-object-index — `obj[0]` on an object receiver.
    const codes = codesOf(
      [
    ...
    expect(codes).toContain("theta/parse/non-string-object-index");
  });
});

// ===========================================================================
// theta/parse/non-string-array-join — (owned V3g), integration witness.
// ===========================================================================

describe("V20c-T — non-string-array-join fires in production", () => {
  it("rejects `array.join(<non-string element type>)`", () => {
    // theta/parse/non-string-array-join — `.join` on an `array<integer>`.
    const codes = codesOf(["let xs = [1, 2, 3]", 'let s = xs.join(",")'].join("\n"));
    expect(codes).toContain("theta/parse/non-string-array-join");
  });
});
...
describe("V20c-T — runtime string-index correction", () => {
  it("surfaces the not-indexable error for `s[0]` rather than returning a char", () => {
    ...
    // §"Supported forms"; `theta/parse/non-indexable-receiver`). At runtime the
