...
- **`src/runtime/forwarding-listener-trap.ts`** — V17b throw-trap around the three forwarding listeners' `loomAbort.abort(source.reason)`. ⛔ ACCEPTED CUT — spec-vs-runtime divergence, NOT realizable at the specified seam (investigated; intentionally left unwired). The trap's `catch` fires only if `loomAbort.abort(...)` throws *to the forwarding-listener frame*. In production `loomAbort` is ALWAYS a raw `AbortController` (`createLoomAbort`/`deriveChildLoomAbort` = `new AbortController()`), whose `.abort()` never throws to its caller — a throwing `"abort"` listener is reported WHATWG **out-of-band** (Node raises `uncaughtException`; `.abort()` returns normally; empirically confirmed Node v24). The one hypothesized realistic throw — subagent `session.abort()` (registered by `attachSubagentAbortForwarding` as a listener ON `loomAbort.signal`, dispatched INSIDE `.abort()`) — is therefore uncatchable by any `try`/`catch` at the `forwardSignalReason` seam. The spec-cited `ctx.abort()` override (host-interfaces-core.md:85) that could make `loomAbort.abort()` throw to the wrapped frame is UNIMPLEMENTED (code-side tools receive only a `signal`, never a synthesised ctx with overridden `abort()`). The module's tests pass solely via a `ThrowingLoomAbort` double modelling a non-existent wrapped controller. cancellation.md:15 mandates the trap, but its premise contradicts WHATWG dispatch semantics (which the module header itself acknowledges). If synchronous `AgentSession.abort()`-throw hardening is wanted, it belongs at the PIC-41 subagent seam (`attachSubagentAbortForwarding`), a separate item — NOT this trap. `forwardToolExposedCancel` additionally has no production caller (independent pre-existing CANCEL-5 gap).
