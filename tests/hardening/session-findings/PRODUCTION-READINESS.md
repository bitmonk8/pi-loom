...
**✅ 1.0 CLOSE-OUT COMPLETE (this session).** The maintainer selected the close-out path: the full live-probe regression sweep was run against a real provider and the program is marked complete. Result: all 16 `session-*.test.ts` live probes GREEN. The sweep surfaced TWO deterministic probe failures — BOTH classified (verified against source) as stale-probe drift, NOT production regressions — and realigned in `19d17678` (test-support only; `npm test` 1745 + conformance 26 unchanged-green; source untouched):
- **session-discodyn** (5 assertions): read the wrong channel. The shipped load path routes error-severity load diagnostics onto the `loom-system-note` channel, not `ctx.ui.notify` (V4e, `b5a858a1`); the probe read `probe.diagnostics` and passed no drives. Added an additive load-phase `systemNotes` channel to the harness and moved the assertions to it (substance preserved).
- **session-promptstream** (QTL-1): stale fixture. Two BARE `@`-queries — the first (non-tail) now trips QRY-19 `loom/parse/discarded-query-result` (wired this program in `2c9725b3`), un-registering the loom so `/twostream` fell through as literal text. Bound the first query to `_` (explicit discard, still dispatches) and pinned the SLSH-2/QTL-1 dispatch claim on the deterministic `userTexts` channel + a fall-through guard, dropping the flaky model-obedience `assistantText` assertion.

The named backlog was already EXHAUSTED at entry; no new feature work was performed. Everything remaining is a documented defensible 1.0 cut, a not-realizable item, or a spec-sanctioned DEFER (each recorded with rationale below). **The pi-loom production-readiness program is complete for 1.0 as defined by the spec.**

**Baseline (close-out):** HEAD `19d17678`; `npm test` **1745**, `npm run test:conformance` **26**, `npm run typecheck` + `npm run lint` clean, all 16 live probes green (deterministic), on `main`. (Prior handoffs: `178f1542`/1712, `e1702c87`/1619.)

**Latest session (14 commits, `1dccc833`→`178f1542`, all pushed) — the named backlog is now EXHAUSTED.** Wired (10 feature/fix): drain-state routing (PIC-29..32, `1dccc833`); query-schema-inference indirect positions (V13b, `f2b9f216`); `runSessionShutdown` full five-step teardown across 3 increments (Decision 6: factory wire `37c50def`, ActiveInvocationRegistry sub-steps 2/3 `e615c842`, forwarding-listener detach sub-step 5 `d73de9ae`); subagent-mode transport-error parity (PIC-50/51, `fc10b8ba`); ceiling-#4 on code-driven (`2731dd99`) + model-driven (`bfee36db`) tool-call args — ceiling-#4 table now fully enforced; tool-return shape validation + non-settling-await race (V14c, `5e371c70`); discarded-query-result diagnostic (QRY-19, `2c9725b3`); load-phase pre-eval note routing (V4e spec violation fix, `b5a858a1`). Documented as accepted cuts / not-realizable / already-covered (4 doc commits, with proof): forwarding-listener-trap V17b (`580deb9d`, WHATWG out-of-band — spec-vs-runtime divergence), tool-registration residues PIC-8/19/44 + `ToolDefinition.label` (`19e97843`, wire-none), subagent→prompt invoke cell (`178f1542`, not-realizable + spec-conformant via the spawn path).

**More fixes landed after the RESUME summary above (all pushed):** subagent `dispose()`/`forwarding.detach()`-on-`executeBody`-throw leak ✅ FIXED (PIC-9, `bf58f412` — single idempotent `teardown()` in the drive `finally` on every exit); top-level runtime-defect / panic note surface ✅ FIXED (`339f3ef0` — `composeLoomFixture.run` outer catch frames a `LoomPanic`/internal-error defect as ONE `loom-system-note` via `emitPanicNote` instead of escaping uncaught to the host; `HostFatal` re-raised; top-level-only). NOTE: the tool-return-shape DIAGNOSTIC sink-note (interim `1c1a1d70`) was SUPERSEDED + reverted by `339f3ef0` — the framed carrier note (top-level) / `Err`-to-parent (invoke) is the correct single surface, so the code-side lowering sink is back to `noopSink()` (the diagnostic rides the `ToolReturnShapeDefectError` carrier).

**Remaining documented deferrals/follow-ups (none are named-backlog bugs; each recorded with rationale):** the operator-facing always-log `RuntimeEvent` layer (incl. QRY-20) ⛔ investigated → DEFER (spec-sanctioned). The channel is spec-declared **write-only with NO loom-1.0 consumer** (runtime-event-channel.md:94 — aggregation + read API "deferred to Future Considerations"; QRY-20's named consumers log-scrapers/replay/`/tree` are Pi-host surfaces not built here); the user-facing top-level cascade note is ALREADY wired with correct content (`emitTopLevelErrNote`) — only its structured `details.event` is a `{}` stub nothing reads; the 5 group-A always-log event kinds are **never CONSTRUCTED** in production (the layer is UNBUILT, not merely unwired), and the `outcome.event` `runQueryEffect` drops is a `validation` event deliberately EXCLUDED from the always-log set (dropping it is arguably spec-correct). Wiring = build 5 event kinds + thread `SystemNoteChannelDeps`+`Clock`+`invocationId`+`loomSlashName` through the executor-wide `ExecuteBodyDeps`/`EffectfulStatementHostDeps` + void-tail detection + a per-invocation dedup store — disproportionate blast radius on the cka-47/ERR-8..12 hot path for write-only output no shipped code consumes. Cheapest future increment if forced: populate the `emitTopLevelErrNote` `details:{event:{}}` stub (no executor-deps change). Builders/emitters stay unit-proven; model-callable-`.loom` lowering (SUBAG-2 residual) ✅ WIRED this session (subagent-mode: `.loom` in `tools:` now exposed to the MODEL via the loom-owned `complete()` loop tool schemas + a `defineTool` customTool on `createAgentSession`; model-tool dispatch maps object args → positional `params:` order and drives `#driveCallee`; seam test `tests/subagent-model-loom-tool.test.ts` + live probe `tests/hardening/session-subagent-loom-tool.test.ts`); drain-state two-arm reconciliation (delete the dead-in-prod `degraded` arm + rewrite `drain-state-contract.test.ts` — pure hygiene, BEHAVIOR already spec-correct since the arm is unreachable; maintainer chose Option B to defer it in phase 1); `dispatchSiteSetup`/`runPerInvocationFinally` remain unwired (NO value — B1's inline entry bookkeeping is the shipped path; wiring them would relocate the producer's session ownership for zero benefit). Genuine 1.0 cuts / not-realizable (documented with proof): `tool-batch` (sequential safe per CLAUDE.md), `argument-hint` (no Pi SDK slot), binder forced-tool (`anyOf` not a valid Anthropic `input_schema`), forwarding-listener-trap V17b (WHATWG out-of-band), tool-registration residues PIC-8/19/44 + `ToolDefinition.label`, subagent→prompt invoke cell (spec-conformant via spawn).
...
**Under the unsupervised full-spec-compliance mandate, the realizable B2 items are being WIRED** (not left as cuts). Done so far: `tool-call-off-surface` ✅ (below). Genuinely-defensible cuts kept: `tool-batch` (sequential is safe; matches CLAUDE.md "sequential by default"; only timing-observable), `argument-hint` (no Pi `RegisteredCommand` autocomplete slot — SDK gap, not realizable), `ToolDefinition.label` (internal tool-bridge label; low-severity). Done: `query-discard` QRY-19 ✅ (QRY-20 deferred to the always-log RuntimeEvent layer), `load-pre-eval` ✅ (below). `forwarding-listener-trap` (V17b) ⛔ investigated → ACCEPTED CUT / spec-vs-runtime divergence: the trap wraps `loomAbort.abort()`, but a throwing downstream abort-listener (the only realistic source, subagent `session.abort()`) is WHATWG out-of-band (uncatchable at that seam), and production `loomAbort` is always a raw `AbortController` (never throws to caller); the spec-cited `ctx.abort()` override is unimplemented. Recorded with proof in dead-code-audit.md. `tool-registration` residues PIC-8/19/44 + `ToolDefinition.label` ⛔ investigated → WIRE-NONE (all not-realizable / already-covered / defensive-only, proof in dead-code-audit.md): PIC-44 has no live consumer (`pi.registerTool` superseded by `customTools` + typed-query text-extraction) — note `ToolDefinition.label` is NOW WIRED (the SUBAG-2 model-callable-`.loom` `defineTool` customTool is `deriveToolLabel`'s first live consumer); PIC-8's transient-`setActiveTools`-failure premise can't hold (synchronous in-memory swap at the 1.0 pin); PIC-19 is already routed to `loom/runtime/internal-error` by the outer runtime-defect boundary (no restore owed) — an explicit seam route would perturb the CANCEL/PIC-17 hot path. Remaining: subagent→prompt invoke attach (INV-9 deferred cell).

- **`load-pre-eval` (V4e). ✅ FIXED (commit pending) — was a SPEC VIOLATION, not a cut.** error-model.md line 13 mandates the `loom-system-note` channel (`triggerTurn:false`) for all 8 pre-eval failures (ERR-1..6, ERR-16); the shipped load path used a transient `ctx.ui.notify` toast + stderr (acknowledged only in `notes.md`, NOT the spec/README). Wired the shipped `composeExtensionInstance` load path through `createLoadFailurePreEvalRouter` so error-severity load-phase failures route to the channel with the SAME envelope as the already-wired reload path (`emitErr7 = emitLoadNote` unifies them); the toast is retained only as the channel's PIC-54 delivery-failure fallback. Error-only filter preserved (matches the prior `makeLoadEmit`; no warning-visibility change). Load path stays non-abort (a note drops the loom, not the session). The non-shipped legacy `discoverAndComposeFixtures` (H8a) path keeps toast+stderr (its tests assert it; not the default export). Tests: `load-phase-pre-eval-routing.test.ts` (2).

- **`tool-call-off-surface` (V14c). ✅ FIXED (commit pending).** Routings (a) tool-return-shape + (b) non-settling-await were REAL unwired gaps (the live path bound GARBAGE for `entry-missing-type`/`entry-missing-text` and threw a raw `TypeError` for `resolved-not-object`/`content-not-iterable`, and had NO abort race so a non-settling tool hung forever). Wired at `runCodeSideToolCall`: `awaitToolSettlementOrAbort` races `dispatch()` vs the signal (NOCEIL-1); `routeToolReturnShape` inspects the resolved envelope before binding — a non-conforming shape routes to `loom/runtime/internal-error{tool-return-shape}` (off `CodeToolError`) via a new `return-shape-defect` outcome → `ToolReturnShapeDefectError` (top-level: runtime defect; invoke parent: `Err(InvokeInfraError{cause:"internal_error"})`). Routing (c) post-cancel discard was ALREADY covered (CNCL-1/2/3 + the race's swallowing handler) — pinned, not duplicated. Adversarial-reviewed SOUND (conforming returns bind identically; no CANCEL regression; no unhandled rejection). Tests: `tool-calls-off-surface-live-wiring.test.ts` (13). **Follow-up:** production threads `noopSink()`, so the shape-defect DIAGNOSTIC is discarded in prod (behavioral correctness intact; a pre-existing sink-wiring limitation).
