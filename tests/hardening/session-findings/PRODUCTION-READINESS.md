...
- **Increment B. ⏭️ NEXT.** Producer-side wire so sub-steps 2/3/5 become REAL: thread a shared `ActiveInvocationRegistry` + a shared forwarding-listener list through `composeExtensionInstance` → the producer dispatch path (`production-loom-producer.ts`) so each invocation registers its `loomAbort`/`disposeBarrier`/`shutdownReason` entry and its `ctx.signal`/tool/parent-invoke forwarding listeners; expose both on `ExtensionInstanceWiring`; feed them into the shutdown `deps` so in-flight invocations are cancelled + awaited and listeners detached on shutdown (registration-steps.md:17 — no stranded subagent connections / AbortController listeners). NOTE: `ActiveInvocationRegistry`, `dispatchSiteSetup`, and the `forwarding-listener-trap` traps currently have ZERO production call sites — this is the largest single wire in the program; land carefully.
...
| 5r-2 | V13b query-schema-inference (Decision 5 remainder, Option 1 full bounded) | New post-parse pass `query-schema-resolve.ts` (tree-rebuild) infers response schema at the 4 spec indirect positions + paren/`?`; QRY-4 explicit-schema-mismatch warning + schema-subset.md step-4 `$defs` pruning wired. Documented parse-time divergences: match-arm opaque (spec-correct), tool/invoke arg sinks + object/union sinks not statically inferable | pending |
