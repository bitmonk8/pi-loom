...
**Remaining documented deferrals/follow-ups (none are named-backlog bugs; each recorded with rationale):** QRY-20 discard-observability event (belongs with the whole operator-facing always-log `RuntimeEvent` layer — thread `SystemNoteChannelDeps`+`Clock`+`invocationId` into `ExecuteBodyDeps`, stop dropping `outcome.event` in `runQueryEffect`, + void-tail detection — a LAYER, not a point fix); model-callable-`.loom` lowering (`TODO(SUBAG-2)` — a larger unimplemented feature; under investigation this session); drain-state two-arm reconciliation (delete the dead-in-prod `degraded` arm + rewrite `drain-state-contract.test.ts` — pure hygiene, BEHAVIOR already spec-correct since the arm is unreachable; maintainer chose Option B to defer it in phase 1); `dispatchSiteSetup`/`runPerInvocationFinally` remain unwired (NO value — B1's inline entry bookkeeping is the shipped path; wiring them would relocate the producer's session ownership for zero benefit). Genuine 1.0 cuts / not-realizable (documented with proof): `tool-batch` (sequential safe per CLAUDE.md), `argument-hint` (no Pi SDK slot), binder forced-tool (`anyOf` not a valid Anthropic `input_schema`), forwarding-listener-trap V17b (WHATWG out-of-band), tool-registration residues PIC-8/19/44 + `ToolDefinition.label`, subagent→prompt invoke cell (spec-conformant via spawn).
