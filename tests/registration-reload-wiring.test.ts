...
describe("V9b-T — registry-swap atomicity (PIC-36)", () => {
  ...
  it("PIC-36: a failed rebuild discards the staging set, keeps the prior snapshot live, and fires theta/runtime/registry-swap-failed", () => {
    ...
    expect(diag.code).toBe("theta/runtime/registry-swap-failed");
    expect(diag.message).toBe("registry swap failed: /x/foo.theta");
  ...
  it("PIC-36: the ReloadFailureInjector routes a synthetic registry-swap / re-parse failure onto the registry-swap-failed surfacing path", () => {
    ...
    injector.injectReloadFailure("theta-thetalib-reparse", new Error("parse boom"));
...
describe("V9b-T — getCommands() snapshot read-only-by-convention (PIC-39)", () => {
  it("PIC-39: the collision pass drops colliding thetas and leaves the snapshot array unmutated in length and order", () => {
    ...
    const result = dropCollidingThetas(pending, snapshot);
...
describe("V9b-T — structural-change watcher note (PIC-37 / PIC-38)", () => {
  it("PIC-37: an empty debounce window (added.length + removed.length === 0) emits no note", () => {
    // Including a settings edit whose post-merge `thetaPaths` is byte-identical
    // and a burst that nets no added/removed `.theta`/`.thetalib` file — all resolve
  ...
  it("PIC-38: a same-window rename of path P (removed P then added P, summing to 2) emits the note with the fixed content and display:true", () => {
    const note = structuralChangeNote(["/x/p.theta"], ["/x/p.theta"]);
    ...
    expect(note?.details).toEqual({
      structural: { added: ["/x/p.theta"], removed: ["/x/p.theta"] },
    });
  });
});
...
describe("V9b-T — model-reference-matcher production wiring (model-registry surface)", () => {
  ...
  it("constructs the matcher once at the load pass and injects that single instance into every V6a parse call (single-source-of-construction, instance identity)", () => {
    ...
    // That injected instance is theta's registry-backed exact-match resolver.
