...
| `loom/parse/bare-object-literal` | E | parse | Bare `{ field: expr }` (no schema name) used in expression position outside the two documented carve-outs (`params:` defaults; single argument of a Pi-tool call). | [Expressions — Object construction](./expressions.md) | Name the schema: `Schema { field: expr }`. |
| `loom/parse/default-not-literal` | E | parse | A `params:` default RHS contains a form outside the [Loom literal sublanguage](./grammar.md#loom-literal-sublanguage) (operator, function call, identifier reference other than `Enum.Variant`, `${...}` interpolation, or `@`...`` template). | [Frontmatter — Defaults](./frontmatter.md) | Replace the offending sub-expression with a literal value, or move the computation into loom body code. |
| `loom/parse/tool-arg-not-literal` | E | parse | The single-argument bare-object literal of a Pi-tool call contains a form outside the [Loom literal sublanguage](./grammar.md#loom-literal-sublanguage). | [Tool Calls — Argument shape](./tool-calls.md) | Bind the computed value via `let` first, then pass through a typed `params:` callee or `invoke(...)`. |
| `loom/parse/tool-arg-arity` | E | parse | A Pi-tool call site uses more than one positional argument (e.g. `read({...}, {...})`). | [Tool Calls — Argument shape](./tool-calls.md) | Pi tools take exactly one input object; merge the arguments. |
| `loom/parse/let-without-initialiser` | E | parse | `let x: T` (annotation, no initialiser). Loom has no `undefined` value and no definite-assignment analysis. | [Grammar Appendix — `let` form](./grammar.md#let-form) | Initialise the binding at declaration, or restructure to bind once at the point a value is available. |
| `loom/parse/statement-in-arm-body` | E | parse | A `match` arm body is a bare `if` / `for` / `while` / `let` / assignment statement. Arm bodies are expressions; the ternary is the expression form of conditional. | [Grammar Appendix — `match` arm body](./grammar.md#match-arm-body) | Wrap the statements in a block expression `{ ... }` whose tail expression is the arm's value. |
| `loom/parse/by-on-object-schema` | E | parse | `schema X by f { ... }` declaration with an object body. The `by` clause applies only to discriminated-union schemas. | [Grammar Appendix — `schema X by <field>`](./grammar.md#schema-x-by-field) | Use `schema X by f = A \| B \| …`, or drop the `by` clause if `X` is an object schema. |
| `loom/parse/doc-comment-misplaced` | E | parse | A `///` doc comment appears above a production other than `schema`, `enum`, schema field, enum variant, or `fn`. | [Grammar Appendix — `///` placement](./grammar.md#-placement) | Move the description to a legal anchor, or replace with a regular `//` comment. |
...
| `loom/parse/invoke-return-type-mismatch` | E | type | `invoke<Schema>(...)` annotation is structurally incompatible with the callee's inferred return type, where both annotation and callee are statically resolvable. | [Invocation — Typed return](./invocation.md) | Widen the annotation, narrow the callee, or drop the annotation and let the runtime AJV check decide. |
| `loom/parse/invoke-arity-too-few` | E | parse | `invoke(...)` or registered-loom call passes fewer arguments than the callee's count of non-defaulted `params:`, where the callee is statically resolvable. | [Invocation — Argument arity](./invocation.md#argument-binding) | Provide the missing argument(s) or default the corresponding `params:` field on the callee. |
| `loom/parse/invoke-arity-too-many` | E | parse | `invoke(...)` or registered-loom call passes more arguments than the callee's total `params:` count. Always parse-time — no runtime safety net is possible. | [Invocation — Argument arity](./invocation.md#argument-binding) | Drop the extra argument(s); positional binding has no destination for them. |
...
| `loom/load/params-null` | E | load | Frontmatter declares `params: null`. Absent `params:` and `params: {}` are the two equivalent ways to declare a no-params loom. | [Frontmatter — Field contract](./frontmatter.md) | Remove the `params:` line entirely, or write `params: {}`. |
| `loom/load/bind-echo-without-params` | W | load | `bind_echo: true` declared on a no-params loom (`params:` absent or `params: {}`). The binder does not run, so no echo is produced. | [Binder — Binder bypass](./binder.md) | Remove `bind_echo:` or accept that the field has no effect. |
...
| `loom/load/invoke-path-escape` | E | load | An `invoke(...)` literal or a `tools:` `.loom` entry resolves (post-realpath) to a path that lies outside every active discovery root. | [Invocation — Resolution](./invocation.md) | Move the callee under one of the active discovery roots (see [Directory Convention — Discovery roots](./discovery.md#discovery-roots)), or add the callee's directory as a settings `looms` entry. |
| `loom/load/binder-model-unresolved` | E | load | A non-bypass loom is in scope but neither `bind_model:` frontmatter nor the `looms.binderModel` setting resolves to a model identifier. | [Binder — Binder model](./binder.md) | Set `bind_model:` in the loom's frontmatter, or set `looms.binderModel` in `~/.pi/agent/settings.json` or `.pi/settings.json`. |
| `loom/load/binder-model-not-strict-capable` | E | load | The resolved binder model is not flagged by Pi's model registry as supporting strict structured-output / strict tool-input. | [Binder — Binder model](./binder.md) | Choose a strict-capable model (Claude Haiku, GPT-4o-mini, Gemini Flash all qualify on supported providers). When Pi's registry does not surface a strict flag, the load-time check degrades to best-effort; runtime envelope-malformed failures surface as `loom/runtime/binder-malformed-envelope`. |
| `loom/load/argument-hint-not-displayed` | W | load | A loom declares `argument-hint:` but no `description:`. The binder still receives the hint, but Pi's autocomplete dropdown will show only an empty entry (Pi extension-registered commands have no `argumentHint` slot). | [Frontmatter — Field contract](./frontmatter.md) | Add a `description:` so the autocomplete entry is not empty, or accept the empty entry. |
| `loom/load/callee-has-errors` | E/W | load | A `.loom` callee referenced by an `invoke(...)` literal or a `tools:` `.loom` entry failed to parse, lower, or pass its own structural checks during the parent's per-load-pass static-resolution walk (per [Invocation — Static resolution](./invocation.md#static-resolution)). The diagnostic carries the callee's own diagnostic codes via `related`. Severity is per surface: `tools:` `.loom` entries are `E` (the callable cannot be created and the parent does not register); `invoke(...)` literals are `W` (the parent registers; static checks against the unresolvable callee are skipped; the runtime AJV check is the safety net). | [Invocation — Static resolution](./invocation.md#static-resolution) | Open the callee and fix the listed errors. |
...
| `loom/load/cross-format-collision` | E | load | Two or more candidates at the same priority derive the same slash name. Covers both the same-format case (two `.loom` files at the same priority — e.g. two packages, two settings entries, two `--loom` components) and the cross-format case (`.loom` vs Pi-owned `.md` prompt / `.md` subagent / another extension's command). The diagnostic lists every colliding path; for cross-format, the Pi-owned entry survives and only the loom(s) drop; for same-format, none of the colliding looms register. | [Discovery — Slash-name collisions at the same priority](./discovery.md) | Rename one of the colliding files so the slash names diverge, or remove the duplicate configuration entry. |
...
V1 has exactly six panic sources. Each surfaces through the channels defined in [Errors and Results — Runtime panics](./errors-and-results.md) (slash-command system note for top-level looms; `Err(QueryError { kind: "invoke_failure", reason: "panic", ... })` to an `invoke` parent). Two additional `loom/runtime/*` codes — `system-note-delivery-failed` and `subagent-dispose-failure` — are not panics but delivery-failure diagnostics emitted, respectively, by the system-note fallback path and the subagent session lifecycle teardown defined in [Pi Integration Contract](./pi-integration-contract.md).
...
| `loom/runtime/invoke-depth` | E | runtime | `invoke` chain (counting direct `invoke(...)`, registered-loom calls, and `.warp` `fn` invokes) exceeds depth 32 on a single chain. | [Invocation — Invocation depth bound](./invocation.md) | `invoke chain depth exceeded: <depth> > 32`. |
