...
- `coercion` controls how typed queries recover from schema-validation failures (see the [Query](./query.md) section). `attempts` bounds the number of follow-up coercion turns; `methodology` selects the phrasing strategy. The block configures **coercion** — the response-repair mechanism specified in [Query — Schema-validation coercion](./query.md). The verb "retry" is reserved for genuinely retry-shaped behaviour elsewhere in the spec (`TransportError.retryable`, the binder's single-shot transport retry); they are different mechanisms. Recognised methodologies (V1) — the literal user-turn text each non-`none` methodology emits is fixed by [Query — Follow-up turn templates (normative)](./query.md#follow-up-turn-templates-normative); the bullets below name the methodology and its summary intent, the templates carry the wording:
