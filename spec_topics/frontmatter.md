...
| `respond_repair` | no | `{ attempts: 3, methodology: validator_error }` | Typed queries get the default respond-repair budget; documented under the `respond_repair` prose below. |
| `tool_loop` | no | `{ max_iterations: 25 }` | Every query (untyped, typed, and any respond-repair follow-up) runs its tool-call loop under the default cap; documented under the `tool_loop` prose below. |
