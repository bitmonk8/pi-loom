...
**Unknown-key policy.** Unknown keys at the top level of the frontmatter mapping are tolerated for forward compatibility: they are ignored at the value level (the runtime does not propagate them into `params`, the binder, or any other surface) and surface as a single warning per key — `loom/load/unknown-frontmatter-field` for unrecognised keys, or `loom/load/deferred-frontmatter-field` for keys reserved for deferred V1 features named in [Future Considerations](./future-considerations.md). The loom continues to load and register in both cases. The current V1 vocabulary is the table below; future minor versions extend it additively. The warning-not-error posture is deliberate: an author who hand-edits frontmatter to set a key from a newer loom minor than the runtime supports must still see their loom load (with a visible warning), so that downgrading the runtime never silently breaks slash commands. The seam is what allows the deferred `binder_temperature` and user-overridable binder-prompt extensions in [Future Considerations](./future-considerations.md) to land additively without a frontmatter-schema bump.
...
| `tool_loop` | no | `{ max_iterations: 25 }` | Every query (untyped, typed, and any respond-repair follow-up) runs its tool-call loop under the default cap; documented under the `tool_loop` prose below. `tool_loop: {}` (block present, `max_iterations` absent) is equivalent to omitting `tool_loop:` entirely; the default `25` applies. |
...
- `respond_repair` controls how typed queries recover from schema-validation failures (see the [Query](./query.md) section). `attempts` bounds the number of follow-up respond-repair turns; `methodology` selects the phrasing strategy. The block configures **respond-repair** — the response-repair mechanism specified in [Query — Schema-validation respond-repair](./query.md). The verb "retry" is reserved for genuinely retry-shaped behaviour elsewhere in the spec (`TransportError.retryable`, the binder's single-shot transport retry); they are different mechanisms. Recognised methodologies (V1) — the literal user-turn text each non-`none` methodology emits is fixed by [Query — Follow-up turn templates (normative)](./query.md#follow-up-turn-templates-normative); the bullets below name the methodology and its summary intent, the templates carry the wording:
...
- `tool_loop` bounds the model's tool-call loop (see [Query — Tool-call loop bound](./query.md)). `max_iterations` is a non-negative integer counting tool-call *rounds*, not individual tool calls (V1 imposes no upper bound); one round is the model emitting one or more `tool_use` blocks, the runtime executing them all (in parallel where the provider supports parallel tool calls), and feeding the results back. The forced respond turn that terminates a typed query also consumes one slot. The cap applies independently to each query and, separately, to each respond-repair follow-up (a follow-up does **not** inherit a depleted budget from the original turn) — and to every query inside an `invoke`d callee, which uses the callee's own `tool_loop` frontmatter; the parent's budget is not debited by `invoke`. When the cap is reached without the model producing a terminating turn, the query returns `Err(QueryError { kind: "tool_loop_exhausted", ... })`. `max_iterations: 0` disables model-driven tool calls entirely — the model cannot use any frontmatter `tools:` entry during the query, while loom code's bare `<name>(...)` calls remain unaffected. Cancellation via `AbortSignal` preempts the loop at any iteration boundary; the cap is a ceiling, not a floor. Out-of-range values (negative integers, non-integer numbers such as `25.5`, non-number scalars such as `"25"`, or `null`) are rejected at frontmatter-parse time as `loom/load/frontmatter-value-out-of-range`; YAML integer-ness is judged on the parsed numeric value, not the lexical form (`25` and `25.0` are both accepted). The same out-of-range rule applies to `respond_repair.attempts`. V1 provides no operator-level override for `max_iterations`; the value is per-loom only. Adding a `looms.toolLoopMaxIterations` settings key is deferred per [Future Considerations](./future-considerations.md).
