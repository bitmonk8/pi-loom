...
**Error shape.** A depth violation surfaces as a standard validation failure on the same channel as any other AJV error at that boundary. For typed queries, this is `Err(QueryError { kind: "validation", cause: "schema_validation", ... })` carrying a single `ValidationIssue` whose `path` is the JSON Pointer to the first too-deep node, `message` is `"JSON document depth exceeds 5"`, and `schema_keyword` is `"maxDepth"` (the only `schema_keyword` value Loom emits that is not a literal AJV keyword; `query.md`'s `ValidationIssue.schema_keyword` enumeration is open). Depth violations always carry `cause: "schema_validation"` even though the depth walk short-circuits before AJV runs (see [Errors and Results — ValidationError](./errors-and-results.md)). For tool-call argument validation and `params` validation the violation surfaces as the analogous variant on those boundaries (`CodeToolError` cause / `InvokeInfraError`; see [Tool Calls](./tool-calls.md) and [Invocation](./invocation.md)). Because depth violations are `validation` failures, typed-query respond-repair follow-ups apply per [Query — Schema-validation respond-repair](./query.md); the depth walk re-runs on each follow-up's response.
