...
- **No formal source-language migration mechanism for major-version transitions.** V1 promises that a `.loom` or `.warp` file loading cleanly under V1.0 loads and behaves identically under every V1.x release (per [Governance — GOV-13](./governance.md#gov-13)); migration across major versions is out of V1 scope and will require a separate mechanism when V2 ships.
