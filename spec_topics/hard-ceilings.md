...
- At the typed-query response / model-driven `tool_use` args / `params` merge / `invoke<T>` return boundary (ceiling #4's site, CIO-3) only ceiling #2 can co-fire (ceiling #3 is load-time per CIO-1 / CIO-5, ceiling #1 is checked at `invoke` entry per CIO-2 before the callee body runs).
- At the code-driven `<name>(args)` tool-arg boundary (ceiling #4's code-driven row, CIO-3) the reachable set is empty: the call site is not inside a tool-call-round counter context, so ceiling #2's slot accounting is not reachable at this site (the emptiness is by absence of counter context, not by the CIO-2 / CIO-1 / CIO-5 ordering that explains the typed-query case); ceilings #1 and #3 are unreachable for the same reasons given in the typed-query case (ceiling #3 is load-time, ceiling #1 is checked at `invoke` entry before any callee body runs).
...
*Five-site list co-edit obligation.* Ceiling #4's per-boundary table at [Per-boundary destination/surface table (ceiling #4)](#ceiling-4-table) above is the canonical enumeration of the AJV enforcement points; a future V1 leaf that adds, removes, or splits a row of that table MUST update the five-site enumerations in CIO-3 and CIO-6's masked-domain bullets above (in this page) and the parenthetical site-list in [Pi Integration Contract — Hard-ceiling co-fire (`masked`)](./pi-integration-contract.md) in the same commit, in addition to the GOV-12 aggregator update covered above.
