...
- **Original to loom**: `schema` and `enum` declarations targeting the [Schema Subset](./schema-subset.md), the `@`...`` query template as the primitive that crosses code → model in the *current* conversation, frontmatter-declared execution mode, the `.loom` / `.warp` split, and the unified callable surface where Pi tools and registered subagent looms share one declaration list (`tools:`) and one call syntax (`<name>(...)`).
